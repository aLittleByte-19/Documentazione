...
\begin{center}
    ...
    \begin{tcolorbox}[
        ...
        \begin{tabularx}{\linewidth}{>{\bfseries}p{3.5cm} X}
            ...
            Versione    & 1.0.0 \\
            Redattori   & V. Y. K. Fernandes, D. Belluz, A. Oloni\\
            Verificatori&  A. Maule, L. Soligo \\
...
\begin{center}
    ...
    \begin{longtable}{|p{1.4cm}|p{1.8cm}|p{5cm}|p{2.4cm}|p{2.3cm}|p{2.3cm}|}
        ...
        \hline
        \endhead
        1.0.0 & 03/09/2026 & Approvazione manuale utente & - & - & Eleonora Bellet\\
        \hline
        0.9.0 & 18/08/2026 & Aggiunta Appendice, sezione problemi frequenti e metriche. & V. Y. K. Fernandes & L. Soligo & - \\
        \hline
        0.8.0 & 16/08/2026 & Stesura sezione AI Co-Pilot (revisione, dettaglio e messaggio di invio). & A. Oloni & A. Maule & - \\
        \hline
        0.7.0 & 14/08/2026 & Stesura sezione AI Co-Pilot (caricamento, avanzamento e storico). & D. Belluz & L. Soligo & - \\
        \hline
        0.6.0 & 12/08/2026 & Stesura sezione AI Assistant (revisione, valutazioni e storico). & V. Y. K. Fernandes & A. Maule & - \\
        \hline
        0.5.0 & 10/08/2026 & Stesura sezione AI Assistant (composizione bozza e avanzamento). & A. Oloni & L. Soligo & - \\
        \hline
        0.4.0 & 08/08/2026 & Stesura interfaccia generale, navigazione e Overview operativa. & D. Belluz & A. Maule & - \\
        \hline
        0.3.0 & 06/08/2026 & Stesura guida all'installazione, avvio e credenziali AI. & V. Y. K. Fernandes & L. Soligo & - \\
        \hline
        0.2.0 & 04/08/2026 & Stesura requisiti hardware, software e convenzioni tipografiche. & A. Oloni & A. Maule & - \\
        \hline
        0.1.0 & 02/08/2026 & Strutturazione iniziale, scopo del documento e perimetro. & A. Oloni, \newline V. Y. K. Fernandes & L. Soligo & - \\
        \hline
    \end{longtable}
    }
\end{center}
...
Il perimetro è quello concordato con il \glslink{committente-proponente}{committente} il 15 luglio 2026. Per evitare attese errate, le esclusioni rilevanti per chi usa il prodotto sono le seguenti.
...
\begin{itemize}
    ...
    \item \textbf{Nessun accesso nominativo.} Non c'è login, non ci sono ruoli distinti in interfaccia e non esiste un revisore dedicato. Lo stato \ui{Approvata} di una comunicazione significa che il \glslink{redattore}{redattore} l'ha salvata nello storico, non che un altro utente l'ha validata.
    \item \textbf{Nessun deploy su infrastruttura cloud del \glslink{committente-proponente}{committente}.} Lo stack gira in locale (\glslink{docker}{Docker}, \glslink{localstack}{LocalStack}). I servizi \glslink{aws}{AWS} reali usati dalla \glslink{mvp-minimum-viable-product}{MVP} restano \glslink{amazon-bedrock}{Amazon Bedrock} e, se abilitato, Amazon \glslink{textract}{Textract} insieme a uno storage \glslink{s3-simple-storage-service}{S3} reale.
...
\begin{itemize}
    ...
    \item \textbf{\glslink{repository}{Repository} della \glslink{mvp-minimum-viable-product}{MVP}:}\\
...
L'applicazione è una \glslink{spa-single-page-application}{SPA} consultabile da browser. Per eseguirla in locale (unico modo previsto dalla \glslink{mvp-minimum-viable-product}{MVP}) si consiglia un computer con:

\begin{itemize}
    \item processore con almeno 2 vCPU;
    \item 8~GB di RAM (16~GB consigliati: lo stack comprende database, emulazione \glslink{aws}{AWS}, \glslink{worker}{worker} e osservabilità);
    \item almeno 15~GB di spazio libero su disco per immagini \glslink{docker}{Docker} e volumi;
...
\begin{itemize}
    ...
    \item \textbf{\glslink{docker}{Docker}} e \textbf{\glslink{docker-compose}{Docker Compose}}, in versione recente (si consiglia \glslink{docker}{Docker} Desktop 4.3 o superiore, oppure un motore \glslink{docker}{Docker} equivalente).
...
Il certificato \glslink{tls-transport-layer-security}{TLS} locale è self-signed. Al primo accesso il browser segnala che la connessione non è attendibile: è il comportamento previsto. Per eliminare l'avviso si può generare un certificato fidato con \texttt{make trusted-local-\glslink{tls-transport-layer-security}{tls}} (richiede \texttt{mkcert} installato sulla macchina).

\subsection{Credenziali AI}

La generazione dei testi, delle copertine e l'analisi documentale usano \glslink{amazon-bedrock}{Amazon Bedrock}. \glslink{localstack}{LocalStack} non emula Bedrock: anche in ambiente locale servono credenziali \glslink{aws}{AWS} reali con accesso \glslink{intelligenza-artificiale-ai}{ai} modelli abilitato nella console della region configurata.

Senza un modello di testo raggiungibile la generazione della \glslink{bozza-draft}{bozza} fallisce e il sistema mostra un errore applicativo, senza contenuti sostitutivi. Senza un modello di immagini raggiungibile il testo viene comunque prodotto: la copertina risulta non disponibile, con un messaggio esplicito, e la \glslink{bozza-draft}{bozza} resta utilizzabile.

L'\glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR} via Amazon \glslink{textract}{Textract} è disabilitato di default. Se lo si attiva, il documento deve risiedere su un bucket \glslink{s3-simple-storage-service}{S3} reale (\glslink{textract}{Textract} non legge lo storage emulato).
...
Copiare il file \texttt{.env.example} in \texttt{.env} e compilare almeno le credenziali \glslink{aws}{AWS} reali usate da Bedrock:

\begin{itemize}
    \item \texttt{\glslink{aws}{AWS}\_REAL\_ACCESS\_KEY\_ID} e \texttt{\glslink{aws}{AWS}\_REAL\_SECRET\_ACCESS\_KEY};
    \item \texttt{\glslink{aws}{AWS}\_REAL\_SESSION\_TOKEN}, solo se le credenziali sono temporanee;
...
Non inserire commenti sulla stessa riga di un valore: il file li tratterebbe come parte del valore. Dopo ogni modifica alle variabili \texttt{\glslink{aws}{AWS}\_REAL\_*} o Bedrock, una volta avviato lo stack, eseguire \texttt{make refresh-runtime} perché i nuovi valori arrivino ai processi.

I valori di default coprono \glslink{localstack}{LocalStack}, \glslink{postgresql}{PostgreSQL}, \glslink{redis}{Redis} e l'osservabilità. Non vanno modificati se non si sta cambiando deliberatamente l'ambiente.
...
Il comando genera il certificato \glslink{tls-transport-layer-security}{TLS} locale se manca, costruisce le immagini, avvia i servizi, applica \glslink{terraform}{Terraform} su \glslink{localstack}{LocalStack}, esegue le migrazioni e mette in marcia applicazione, \glslink{worker}{worker} e ingress HTTPS.

Al termine, aprire il browser su:

\url{https://localhost:8443}

La \glslink{spa-single-page-application}{SPA} è servita dall'emulatore \glslink{cdn-content-delivery-network}{CDN} locale; le \glslink{api-application-programming-interface}{API} rispondono sullo stesso host. Non è richiesto alcun accesso: si atterra direttamente sulla \textit{Overview operativa}.
...
Una risposta positiva indica che il \glslink{backend}{backend} è in ascolto. I log dello stack si consultano con \texttt{make logs}. Un reset completo dell'ambiente (distruttivo: cancella i volumi locali) si ottiene con \texttt{make reset-all}.
...
\begin{itemize}
    \item \textbf{Barra laterale sinistra.} In alto il logo aLittleByte. Sotto, tre gruppi: \ui{Panoramica}, \ui{\glslink{ai-assistant-generativo-ai-assistant}{AI Assistant}}, \ui{Co-Pilot \glslink{ai-co-pilot-per-consulenti-del-lavoro-cdl}{CdL}}. Ogni gruppo ha una voce principale e dei collegamenti alle sezioni della pagina (evidenziati mentre si scorre).
    \item \textbf{Intestazione.} L'etichetta \ui{\glslink{nexum}{NEXUM}} e il titolo della vista corrente (\ui{Overview operativa}, \ui{\glslink{ai-assistant-generativo}{AI Assistant Generativo}} o \ui{AI Co-Pilot per i \glslink{ai-co-pilot-per-consulenti-del-lavoro-cdl}{CdL}}). A destra, il pulsante per passare dal tema chiaro a quello scuro e viceversa.
...
\begin{itemize}
    \item \ui{Crea contenuto} \glslink{porta}{porta} alla sezione di generazione dell'Assistant.
    \item \ui{Carica documenti} \glslink{porta}{porta} alla sezione di caricamento del Co-Pilot.
\end{itemize}
...
La sezione \ui{Priorità essenziali} mostra tre conteggi sul \glslink{tenant}{tenant}, non sulla sola pagina visibile:

\begin{itemize}
    \item \ui{Bozze generate}: comunicazioni ancora in stato di \glslink{bozza-draft}{bozza}.
...
Il modulo prepara comunicazioni interne a partire da un \glslink{prompt}{prompt}, da un \glslink{tono-comunicativo}{tono} e da uno \glslink{stile-editoriale}{stile}. La generazione è asincrona: l'interfaccia accetta la richiesta subito e aggiorna l'avanzamento in tempo reale. Il testo arriva per primo, la copertina dopo. Senza testo la \glslink{bozza-draft}{bozza} è fallita; senza copertina la \glslink{bozza-draft}{bozza} resta valida.
...
La sezione \ui{Crea una \glslink{bozza-draft}{bozza}} contiene:

\begin{itemize}
    \item il campo \ui{\glslink{prompt}{Prompt}}, obbligatorio, da 12 a 5000 caratteri. Deve descrivere cosa comunicare (destinatari interni, obiettivo, vincoli di contenuto). Un testo troppo corto impedisce l'invio;
    ...
    \item il pulsante \ui{Genera \glslink{bozza-draft}{bozza}}. Durante l'elaborazione l'etichetta diventa \ui{Generazione} e il pulsante si disabilita.
\end{itemize}
...
Sotto il form, \ui{Salva configurazione} memorizza \glslink{prompt}{prompt}, tono e stile come preset riutilizzabile, senza avviare una generazione. Si può assegnare un nome (al massimo 150 caratteri) oppure lasciarlo vuoto: in quel caso il sistema attribuisce un'etichetta progressiva del tipo \texttt{Senza nome (1)}. Se il nome è già usato, viene comunque assegnata un'etichetta progressiva. Il pulsante è disabilitato finché il \glslink{prompt}{prompt} non è valido.
...
\begin{enumerate}
    ...
    \item \textit{\glslink{bozza-draft}{Bozza} generata correttamente.}
\end{enumerate}

Se la richiesta resta in lavorazione a lungo, può comparire \textit{``Generazione ancora in corso. Lo stato verrà aggiornato.''} In caso di errore (modello irraggiungibile, credenziali, risposta non valida) lo stato diventa un messaggio di fallimento e la \glslink{bozza-draft}{bozza} non è utilizzabile. Non vengono mai mostrati testi di ripiego.
...
La sezione \ui{Controlla il contenuto} mostra la \glslink{bozza-draft}{bozza} corrente, oppure il record dello storico selezionato. In assenza di \glslink{bozza-draft}{bozza} compare \textit{``La \glslink{bozza-draft}{bozza} generata apparira qui.''}

Quando la \glslink{bozza-draft}{bozza} è pronta si vedono, dall'alto:

\begin{itemize}
    \item l'immagine di copertina, se presente. Durante la generazione compare \textit{``Generazione copertina in corso.''}; se manca o è stata tolta, \textit{``Nessuna copertina per questa \glslink{bozza-draft}{bozza}.''}; se la generazione dell'immagine è fallita, un avviso con il motivo;
    \item i pulsanti \ui{Cambia immagine} e \ui{Rimuovi immagine} (assenti se la \glslink{bozza-draft}{bozza} è stata scartata). L'immagine sostitutiva deve essere PNG, JPEG o WebP, massimo 5~MB;
    ...
    \item il blocco \ui{Documento finale}, visibile solo a generazione conclusa e se la \glslink{bozza-draft}{bozza} non è scartata: \ui{Apri anteprima} (nuova scheda) e \ui{Scarica PDF}. Ogni pagina del documento riporta il \glslink{marcatore-di-trasparenza}{marcatore di trasparenza} \textit{``Creato da \glslink{ai-assistant-generativo-ai-assistant}{AI Assistant}''};
    \item la valutazione a stelle (paragrafo successivo);
    \item lo stato della \glslink{bozza-draft}{bozza} (\ui{\glslink{bozza-draft}{Bozza}}, \ui{Approvata} o \ui{Scartata}) e la dicitura \textit{``Creato da \glslink{ai-assistant-generativo-ai-assistant}{AI Assistant}''};
    \item le azioni \ui{Salva nello storico}, \ui{Rigenera \glslink{bozza-draft}{bozza}} e \ui{Scarta \glslink{bozza-draft}{bozza}}.
\end{itemize}
...
\ui{Modifica} abilita i due campi. \ui{Salva} persiste le correzioni; \ui{Annulla} le scarta e ripristina i valori precedenti. La modifica è consentita finché la \glslink{bozza-draft}{bozza} non è \ui{Scartata}, anche dopo il salvataggio nello storico. Una rigenerazione sostituisce titolo, testo e copertina e chiude un'eventuale modifica non salvata.
...
\begin{itemize}
    \item \ui{Salva nello storico} sposta la \glslink{bozza-draft}{bozza} nello stato \ui{Approvata} e la fa comparire nello storico. Finché non si preme, la generazione resta nell'area di lavoro e non compare nell'elenco. Il salvataggio non congela il contenuto: titolo, testo e copertina restano modificabili e la \glslink{bozza-draft}{bozza} resta rigenerabile. Il pulsante scompare quando lo stato è già \ui{Approvata}.
    \item \ui{Rigenera \glslink{bozza-draft}{bozza}} chiede una nuova variante con gli stessi \glslink{prompt}{prompt}, tono e stile. Sostituisce testo e copertina della \glslink{bozza-draft}{bozza} corrente. È disponibile anche dopo il salvataggio nello storico, non su una \glslink{bozza-draft}{bozza} scartata.
    \item \ui{Scarta \glslink{bozza-draft}{bozza}} chiede conferma: \textit{``Sei sicuro di voler scartare questa \glslink{bozza-draft}{bozza}? Non sarà più modificabile né rigenerabile.''} Con \ui{Conferma eliminazione} lo stato diventa \ui{Scartata}: la \glslink{bozza-draft}{bozza} resta tracciata ma esce dallo storico attivo. \ui{Annulla} chiude la richiesta di conferma.
\end{itemize}
...
La sezione \ui{Storico contenuti} elenca solo le comunicazioni in stato \ui{Approvata}. Una \glslink{bozza-draft}{bozza} appena generata, o una \glslink{bozza-draft}{bozza} scartata, non vi compare. Accanto al titolo della sezione è indicato il numero di record.
...
\begin{itemize}
    \item \ui{Parola chiave}: cerca nel \glslink{prompt}{prompt} (e, per i preset, anche nel nome);
...
\begin{itemize}
    \item \ui{Usa} ricompila il form di generazione con \glslink{prompt}{prompt}, tono e stile del preset. Non avvia una nuova generazione. Non si applica a una \glslink{bozza-draft}{bozza} già salvata nello storico: su quella restano modifica e rigenerazione dirette.
...
\hline
\ui{\glslink{bozza-draft}{Bozza}} & Generazione conclusa (o in corso) nell'area di lavoro. Non compare nello storico. \\
\hline
\ui{Approvata} & Il \glslink{redattore}{redattore} l'ha salvata nello storico. Resta modificabile e rigenerabile. Non è una validazione da parte di un altro ruolo. \\
\hline
...
\subsubsection{Qualità della generazione}

La sezione \ui{Qualità della generazione} riepiloga l'andamento dell'Assistant su un intervallo scelto dal \glslink{redattore}{redattore}. In alto è disponibile \ui{Esporta report}, che genera un PDF con le stesse metriche e i feedback recenti visibili a schermo.

\paragraph{Filtri}

\begin{itemize}
    \item \ui{Tono} e \ui{Stile}: gli stessi elenchi del form di generazione, più l'opzione ``tutti'';
    \item \ui{Da} e \ui{A}: intervallo di date sulla generazione;
    \item \ui{Azzera filtri} ripulisce tutti i campi e riporta le metriche sull'intero storico.
\end{itemize}

Questo filtro è indipendente da quello dello \ui{Storico contenuti}: agisce solo sulle card di questa sezione.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{immagini/assistant-metriche.png}
\caption{Filtri e mosaico delle metriche dell'AI Assistant}
\label{fig:assistant-metriche}
\end{figure}

\paragraph{Card metriche}

\begin{itemize}
    \item \ui{Contenuti generati}: totale delle generazioni nel periodo filtrato, con andamento giornaliero;
    \item \ui{Configurazioni di \glslink{prompt}{prompt} salvate}: numero di preset presenti;
    \item \ui{Bozze valutate}: percentuale di generazioni con un voto, su quante ancora senza;
    \item \ui{Media stelle}: media aritmetica dei punteggi 1--5;
    \item \ui{Esito delle bozze}: barra che ripartisce le generazioni fra \ui{In \glslink{bozza-draft}{bozza}} e \ui{Decise} (approvate o scartate).
\end{itemize}

\paragraph{Feedback recenti}

Sotto il mosaico compare l'elenco delle valutazioni più recenti: titolo della comunicazione, voto in stelle, data e, se presente, il commento lasciato in fase di valutazione. Senza valutazioni compare \textit{``Nessun feedback ricevuto finora.''}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{immagini/assistant-feedback-recenti.png}
\caption{Elenco dei feedback recenti}
\label{fig:assistant-feedback-recenti}
\end{figure}
...
La sezione \ui{Carica nel \glslink{repository}{repository}} accetta un solo file per volta. Prima del file si possono indicare dei \glslink{metadati}{metadati} facoltativi. Quello che viene dichiarato qui \textbf{fa fede}: l'\glslink{intelligenza-artificiale-ai}{AI} continua a produrre la propria estrazione, consultabile, ma tipologia, mese, anno e azienda indicati a mano non vengono sovrascritti.
...
\begin{itemize}
    ...
    \item \ui{Mese di riferimento}: elenco dei mesi, oppure \ui{Automatico (\glslink{intelligenza-artificiale-ai}{AI})}.
    \item \ui{Anno di riferimento}: numero compreso fra 1900 e 2100.
    \item \ui{Azienda}: testo libero, massimo 500 caratteri.
\end{itemize}

L'area tratteggiata \ui{Seleziona o trascina un documento} apre il selettore file. È accettato solo \texttt{application/pdf}. Durante l'upload i \glslink{metadati}{metadati} si disabilitano.
...
\begin{enumerate}
    ...
    \item \textit{Analisi \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR} del documento in corso;}
...
La sezione \ui{Storico documenti analizzati} elenca i sotto-documenti del \glslink{tenant}{tenant}. Ogni riga mostra:
...
I criteri sono applicati dal \glslink{backend}{backend}, sul solo \glslink{tenant}{tenant} corrente:
...
\begin{itemize}
    ...
    \item \ui{\glslink{confidenza-confidence-score}{Confidenza}}: \ui{Minore di} o \ui{Maggiore di} una \ui{Soglia (\%)} da 0 a 100;
...
\begin{itemize}
    \item \textbf{a sinistra, \ui{Anteprima documento}:} il PDF del sotto-documento in un riquadro, con collegamento \ui{Apri in una nuova scheda}. Se lo storage non è raggiungibile compare \textit{``Storage documento non raggiungibile.''}; se il file manca, \textit{``Anteprima non disponibile.''}; in attesa, \textit{``Caricamento anteprima in corso.''} Indipendentemente da questo stato, il pulsante \ui{Documento originale} apre in una nuova scheda il PDF originale caricato (prima dello split), utile per confrontare il sotto-documento con il file di partenza (RF56-OB).
    \item \textbf{a destra, \ui{Dati estratti dall'\glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR}}:} i campi strutturati, con una legenda \ui{Modificabile} / \ui{Sola lettura}.
\end{itemize}

Se la \glslink{confidenza-confidence-score}{confidenza} è sotto l'80\% il pannello dei dati è evidenziato come da controllare con priorità.
...
\begin{itemize}
    \item nome file;
    \item numero pagine;
    \item \glslink{confidenza-confidence-score}{confidenza};
    \item stato revisione;
    \item email destinatario rilevata in sola lettura (con pulsante di copia negli appunti, se valorizzata). Questa email \textbf{non} è compilata dalla pipeline \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR}/Bedrock: va inserita a mano nel campo modificabile omonimo;
    \item data e ora di caricamento.
\end{itemize}
...
\begin{itemize}
    ...
    \item azienda;
    \item data documento;
    \item tipologia (l'elenco comprende le sei tipologie predefinite; se l'\glslink{intelligenza-artificiale-ai}{AI} ha proposto un valore fuori elenco, quel valore resta selezionabile);
    \item descrizione;
...
La \glslink{confidenza-confidence-score}{confidenza} non è un'auto-valutazione del modello. Misura la leggibilità \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR} ponderata sulla completezza dei campi lasciati all'\glslink{intelligenza-artificiale-ai}{AI}. I \glslink{metadati}{metadati} dichiarati in caricamento restano fuori dal calcolo: dichiarare tipologia o azienda non ``gonfia'' il punteggio. La \glslink{soglia-di-confidenza}{soglia} operativa di default è 80\%.
...
\hline
\ui{Validato automaticamente} & I campi automatici superano la \glslink{soglia-di-confidenza}{soglia di confidenza}. \\
\hline
...
Il recapito al dipendente avviene fuori da \glslink{nexum}{NEXUM} (posta elettronica dello studio, altri canali). La piattaforma si ferma alla preparazione del plico.
...
La sezione \ui{Qualità e performance \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR}} riporta i volumi del \glslink{tenant}{tenant}:

\begin{itemize}
    \item \ui{Documenti analizzati}: file originali caricati, con andamento giornaliero;
    \item \ui{Tempo medio di elaborazione}: durata media della pipeline \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR}, dalla ricezione al risultato;
    \item \ui{Classificazioni corrette senza intervento}: percentuale di sotto-documenti validati automaticamente, sul totale di quelli arrivati a una validazione (UC-56.2). Chi è ancora da revisionare o in quarantena non entra nel conto;
    \item \ui{Sotto la \glslink{soglia-di-confidenza}{soglia di confidenza}}: percentuale di sotto-documenti da revisionare perché sotto la soglia, sul totale analizzato;
    \item \ui{Destinatari riconosciuti in automatico}: percentuale di destinatari confermati che l'\glslink{intelligenza-artificiale-ai}{AI} aveva già riconosciuto correttamente, mai corretti a mano (UC-56.4).
\end{itemize}

Non esiste una vista analytics con filtri temporali o grafici di accuratezza nel dettaglio: quelle elaborazioni restano fuori perimetro. Le stesse metriche, in forma compatta, compaiono anche in Overview.
...
Nell'Overview, sotto \ui{Qualità degli strumenti}, il riquadro \ui{\glslink{ai-assistant-generativo}{AI Assistant}} mostra in forma compatta solo la \ui{Media stelle} (qualità percepita delle bozze) e il collegamento \ui{Vedi tutte le metriche}, che \glslink{porta}{porta} alla sezione \ui{Qualità della generazione} descritta in \ref{fig:assistant-metriche}: lì si trovano il totale dei contenuti generati, le configurazioni di \glslink{prompt}{prompt} salvate, la percentuale di bozze valutate, l'esito delle bozze e i feedback recenti, con filtro per tono, stile e intervallo di date. Il segnale di qualità percepita resta la valutazione 1--5, unica per generazione.
...
\hline
Il browser segnala un certificato non attendibile. & È atteso con il certificato self-signed. Procedere comunque al sito, oppure generare un certificato fidato con \texttt{make trusted-local-\glslink{tls-transport-layer-security}{tls}} e riavviare \glslink{traefik}{Traefik}. \\
\hline
...
\hline
\ui{Genera \glslink{bozza-draft}{bozza}} fallisce subito. & Controllare credenziali e region Bedrock nel \texttt{.env}, poi \texttt{make refresh-runtime}. Senza modello di testo non esiste contenuto sostitutivo. \\
\hline
La \glslink{bozza-draft}{bozza} c'è ma manca la copertina. & Comportamento previsto se il modello immagini non è raggiungibile. Si può caricare un'immagine propria o procedere senza. \\
\hline
Il pulsante di generazione non parte. & Il \glslink{prompt}{prompt} deve avere almeno 12 caratteri. Completare tono e stile (hanno già un default). \\
\hline
...
\hline
\ui{Inviato} non corrisponde a una mail partita. & Lo stato cambia allo scaricamento del PDF del messaggio. L'invio reale è fuori dalla \glslink{mvp-minimum-viable-product}{MVP}. \\
\hline
\end{tabularx}
\end{center}
...
Le immagini vanno salvate in \texttt{\glslink{pb-product-baseline}{PB}/Manuale Utente/immagini/} con i nomi indicati, a larghezza desktop, tema chiaro, senza dati personali reali. Dopo il salvataggio, sostituire nel sorgente \LaTeX{} ogni \texttt{\textbackslash screenshottodo} con un \texttt{\textbackslash includegraphics} dello stesso file.
...
\hline
\texttt{header-tema.png} & Header \glslink{nexum}{NEXUM} con pulsante tema. \\
\hline
...
\hline
\texttt{assistant-composizione.png} & Form ``Crea una \glslink{bozza-draft}{bozza}'' compilato. \\
\hline
\texttt{assistant-avanzamento.png} & Barra di generazione in corso. \\
\hline
\texttt{assistant-revisione.png} & Anteprima completa di una \glslink{bozza-draft}{bozza} con copertina. \\
\hline
...
\hline
\texttt{assistant-metriche.png} & Filtri (tono/stile/data) e mosaico \ui{Qualità della generazione}. \\
\hline
\texttt{assistant-feedback-recenti.png} & Esito delle bozze e lista feedback recenti. \\
\hline
\texttt{copilot-caricamento.png} & Upload con \glslink{metadati}{metadati}. \\
\hline
...
\hline
\texttt{copilot-dettaglio.png} & Anteprima PDF e dati estratti affiancati, con pulsante \ui{Documento originale}. \\
\hline
...
\hline
\texttt{copilot-metriche.png} & Mosaico \ui{Qualità e performance \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR}} (classificazioni, \glslink{confidenza-confidence-score}{confidenza}, destinatari). \\
\hline
\end{longtable}
}
\end{center}
