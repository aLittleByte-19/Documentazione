...
\begin{itemize}
    ...
    \item \textbf{Procedure operative:} per ciascuna attività, la sequenza numerata dei passi da eseguire, con indicazione del ruolo responsabile di ogni passo.
\end{itemize}

All'interno delle procedure, il ruolo indicato all'inizio di ogni passo è il responsabile della sua esecuzione. Le formulazioni con ``deve'' individuano obblighi vincolanti per tutti i membri del gruppo, mentre quelle con ``può'' individuano facoltà il cui esercizio è rimesso al giudizio del ruolo indicato.
...
\begin{itemize}
    ...
    \item \textbf{\glslink{latex}{LaTeX}:} per la redazione dei documenti contrattuali e di pianificazione.
...
\paragraph{Strumenti a supporto}
\begin{itemize}
    \item \textbf{\glslink{laravel}{Laravel} 12 e \glslink{filament}{Filament}:} per la realizzazione rispettivamente del backend applicativo e delle interfacce amministrative.
...
\begin{enumerate}
    ...
    \item L'\glslink{analista}{Analista} identifica ogni caso d'uso con il codice \texttt{UC-[Numero]} e ne indica le scomposizioni logiche nella forma \texttt{[Caso Padre].[\glslink{sottocaso}{Sottocaso}]}, ad esempio \textit{UC-1.1} per un \glslink{sottocaso}{sottocaso} di \textit{UC-1}.
...
Ogni documento attraversa una sequenza definita di stati, che rende immediatamente riconoscibile il grado di affidabilità del contenuto e la figura responsabile del passo successivo:
...
\begin{itemize}
    ...
    \item \textbf{Stile prescrittivo:} nelle sezioni che normano un'attività, indicare il ruolo responsabile e l'azione richiesta anziché limitarsi a descrivere la prassi in corso.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\glslink{github}{GitHub} Projects:} fornisce i dati di avanzamento e di consuntivo necessari alla misurazione delle metriche di processo.
...
\begin{itemize}
    ...
    \item \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}:} per la scrittura e l'esecuzione dei test di unità e di integrazione.
...
\begin{enumerate}
    ...
    \item Il \glslink{responsabile}{Responsabile} monitora l'avanzamento della board e ridistribuisce i \textit{task} qualora rilevi il rischio di superare le scadenze fissate.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Il \glslink{responsabile}{Responsabile} revisiona il Registro dei Rischi documentando, per ogni criticità emersa o \glslink{rischio}{rischio} inatteso manifestatosi, la causa scatenante, l'impatto effettivo e l'azione di \glslink{mitigazione-dei-rischi}{mitigazione} prevista, che deve essere assegnata a un membro.
\end{enumerate}
...
Il \glslink{progettista}{Progettista} deve ideare l'architettura del sistema in modo che soddisfi i requisiti funzionali e di qualità concordati con la \glslink{committente-proponente}{proponente}. Nello specifico deve:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\glslink{issue}{Issue} e \glslink{pull-request}{Pull Request}:} costituiscono rispettivamente l'\textit{issue tracking system} e lo strumento di revisione del gruppo, secondo le modalità definite nel processo di gestione della configurazione.
...
\begin{itemize}
    ...
    \item \textbf{Framework applicativi:} il backend è realizzato con \glslink{laravel}{Laravel} 12 e le interfacce amministrative con \glslink{filament}{Filament}; le funzionalità di \glslink{intelligenza-artificiale-ai}{Intelligenza Artificiale} sono erogate tramite \glslink{amazon-bedrock}{Amazon Bedrock}.
...
\begin{itemize}
    ...
    \item \textbf{Verifica:} \glslink{pest}{Pest} \glslink{php}{PHP} per i test di unità e di integrazione, Larastan e \glslink{laravel}{Laravel} Pint per l'\glslink{analisi-statica}{analisi statica} e la formattazione.
...
Le metriche seguenti misurano l'efficienza nell'impiego delle risorse e l'efficacia della previsione dei rischi, ovvero gli esiti delle attività di \glslink{pianificazione}{pianificazione} dell'iterazione e di revisione e valutazione del processo di gestione dei processi. I dati di ingresso sono il foglio di rendicontazione oraria e il Registro dei Rischi del \glslink{piano-di-progetto}{Piano di Progetto}.
