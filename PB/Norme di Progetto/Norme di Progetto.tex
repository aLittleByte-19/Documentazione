...
\begin{center}
    ...
    \begin{tcolorbox}[
        ...
        \begin{tabularx}{\linewidth}{>{\bfseries}p{3.5cm} X}
            ...
            Versione    & 2.0.0 \\
            Redattori   & Eleonora Bellet, Diego Belluz, Alex Oloni, Angelica Gastaldello \\
...
{\footnotesize
    ...
    \begin{longtable}{|p{1.1cm}|p{2.0cm}|p{4.3cm}|p{2.8cm}|p{2.0cm}|p{2.0cm}|}
        ...
        \hline
        \endlastfoot
        2.0.0 & 04/09/2026 & Approvazione versione 2.0.0 & - & - & Leonardo Soligo \\
        \hline
        1.2.0 & 25/08/2026 &aggiornamento e correzione di errori testuali & Angelica Gastaldello & Alex Oloni & - \\
        \hline
        1.1.0 & 14/08/2026 & Aggiunta della redazione di Specifica Tecnica e Manuale Utente in fase PB a cura del Progettista & Angelica Gastaldello & Alex Oloni & - \\
        \hline
...
\begin{itemize}
    \item \textbf{\glslink{laravel}{Laravel} Pint:} Tool ufficiale di formattazione per \glslink{laravel}{Laravel}, basato su \glslink{php}{PHP} CS Fixer. Applica automaticamente PSR-12 al codebase. Deve essere eseguito prima di ogni commit; il codice non conforme non può essere integrato nel \glslink{branch}{branch} principale.
...
\begin{itemize}
    ...
    \item \textbf{\glslink{github}{GitHub} Actions e \glslink{github}{GitHub} Pages:} Impiegati per l'automazione (\glslink{ci-cd-continuous-integration-continuous-deployment}{Continuous Integration/Continuous Deployment}) e per la pubblicazione automatica e trasparente del sito web documentale.
...
\begin{itemize}
    ...
    \item \textbf{Specifica Tecnica:} Documento di design che descrive l'architettura di sistema (\glslink{modello-c4}{modello C4}), i diagrammi delle classi UML e i design pattern scelti per l'implementazione, a cura del \glslink{progettista}{Progettista} a partire dalla fase \glslink{pb-product-baseline}{PB}.
    \item \textbf{Manuale Utente:} Guida operativa destinata agli utenti finali per facilitare l'onboarding sulla piattaforma \glslink{nexum}{Nexum} e spiegare l'utilizzo degli assistenti \glslink{intelligenza-artificiale-ai}{AI}, a cura del \glslink{progettista}{Progettista} a partire dalla fase \glslink{pb-product-baseline}{PB}.
...
Il framework adottato è \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}}, incluso nativamente in \glslink{laravel}{Laravel} 12. I test risiedono nella directory \texttt{tests/Unit/} del progetto.
...
\begin{enumerate}
    ...
    \item Calcolare la \textit{code coverage} con \texttt{./vendor/bin/\glslink{pest}{pest} -{}-coverage} (richiede il driver \textbf{Xdebug} attivo nel \glslink{container}{container} \glslink{php-fpm}{PHP-FPM}). La copertura minima richiesta per ogni unità è dell'\textbf{80\%}, in conformità con la metrica \textbf{MPC-CC}.
...
I \glslink{test-di-integrazione}{test di integrazione} sono scritti con \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}} e risiedono in \texttt{tests/Feature/}. A differenza dei \glslink{test-di-unita}{test di unità}, questi test eseguono richieste HTTP reali contro l'applicazione \glslink{laravel}{Laravel} utilizzando il database \glslink{postgresql}{PostgreSQL} di test (definito in \texttt{.env.testing}), verificando l'interazione tra controller, service layer e database. Anche per questi test vale il requisito di copertura minima dell'\textbf{80\%} e l'obbligo di esecuzione prima di aprire una \textit{\glslink{pull-request}{Pull Request}}.
...
Vengono eseguiti sull'applicazione deployata nell'ambiente \glslink{docker-compose}{Docker Compose} completo (\glslink{nginx}{Nginx}, \glslink{php-fpm}{PHP-FPM}, \glslink{postgresql}{PostgreSQL}, \glslink{redis}{Redis}, \glslink{minio}{MinIO}), simulando scenari end-to-end che attraversano tutti i livelli dello stack. \glslink{minio}{MinIO} viene utilizzato nell'ambiente di test come object storage compatibile con le \glslink{api-application-programming-interface}{API} \glslink{s3-simple-storage-service}{S3}. Ogni \glslink{test-di-sistema}{test di sistema} è tracciato ai \glslink{requisito-funzionale}{requisiti funzionali} dell'\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti} e, ove applicabile, al relativo \glslink{caso-d-uso-uc}{caso d'uso}, tramite i codici di riferimento riportati nel \textit{\glslink{piano-di-qualifica}{Piano di Qualifica}}. I risultati e lo stato di copertura dei requisiti sono riportati nel \textit{\glslink{piano-di-qualifica}{Piano di Qualifica}}.
...
\begin{itemize}
    ...
    \item A partire dalla fase \glslink{pb-product-baseline}{PB}, è sua competenza diretta la stesura e il costante aggiornamento dei seguenti documenti:
    \begin{itemize}
