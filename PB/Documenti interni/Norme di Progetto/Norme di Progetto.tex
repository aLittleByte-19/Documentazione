...
\begin{itemize}
    ...
    \item \textbf{\glslink{docker-compose}{Docker Compose} e \glslink{localstack}{LocalStack}:} per l'esecuzione locale dello stack applicativo (Traefik, nginx, app, worker, PostgreSQL 16, Redis 7, emulazione S3/SQS/Step Functions/Secrets).
...
Il codice \glslink{php}{PHP} prodotto deve essere conforme allo standard \textbf{PSR-12} (\textit{\glslink{php}{PHP} Standards Recommendations 12}), seguito da \glslink{laravel}{Laravel} 12. La formattazione automatica è applicata da \glslink{laravel}{Laravel} Pint con il preset \texttt{laravel} (derivato da PSR-12). PSR-12 definisce regole su indentazione (4 spazi), apertura delle parentesi, dichiarazione dei tipi e struttura delle classi, garantendo uniformità in tutto il codebase indipendentemente dal membro che scrive il codice.
...
\begin{itemize}
    ...
    \item \textbf{Variabili d'ambiente:} UPPER\_SNAKE\_CASE — es. \texttt{AWS\_BUCKET}, \texttt{DB\_CONNECTION}
\end{itemize}

\subparagraph{Strumenti di linting e formattazione}
\begin{itemize}
    \item \textbf{\glslink{laravel}{Laravel} Pint:} tool ufficiale di formattazione per \glslink{laravel}{Laravel}, basato su \glslink{php}{PHP} CS Fixer. In assenza di \texttt{pint.json} si applica il preset \texttt{laravel} (derivato da PSR-12). Il codice non conforme è bloccato dalla CI prima dell'integrazione nel \glslink{branch}{branch} principale.
...
\begin{enumerate}
    ...
    \item Verificare che i test \glslink{pest}{Pest} pertinenti passino in locale con il comando \texttt{./vendor/bin/pest}; per il frontend, eseguire la suite \glslink{jest}{Jest}.
    \item La copertura è misurata in CI rispetto a \texttt{coverage-thresholds.json}: backend linee $\geq 80\%$ e rami $\geq 70\%$; frontend statements e functions $\geq 80\%$ e rami $\geq 70\%$; diff coverage sulle linee modificate $\geq 80\%$. Il gate è globale sulla suite, non per singolo tipo di test.
...
I \glslink{test-di-unita}{test di unità} verificano in isolamento singole componenti software. Sul \glslink{backend}{backend} il framework è \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}}: 86 casi in \texttt{tests/Unit/} e 138 in \texttt{tests/DomainUnit/}, senza bootstrap completo di \glslink{laravel}{Laravel} dove non necessario. Sul frontend i test unitari sono scritti con \textbf{\glslink{jest}{Jest}} 30 e \texttt{jest-preset-angular} (jsdom): 510 casi in 53 file.

\subparagraph{Test di integrazione}
I \glslink{test-di-integrazione}{test di integrazione} confermano che le componenti del software operino tra loro in modo sinergico e privo di errori. Sono scritti con \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}}, risiedono in \texttt{tests/Feature/} (271 test in 21 file) e usano il kernel di test di \glslink{laravel}{Laravel} con database SQLite in memoria; i client \glslink{aws}{AWS} sono simulati. La copertura minima non è un gate per tipo di test: vale il \glslink{quality-gate}{quality gate} globale della suite backend.

\subparagraph{Test di sistema}
I \glslink{test-di-sistema}{test di sistema} accertano che il comportamento globale del prodotto rispetti i requisiti stabiliti nell'\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti}. Non esiste una suite e2e automatica con asserzioni di scenario. In CI lo stack \glslink{docker-compose}{Docker Compose} (Traefik 3.4, nginx 1.29, app, worker, PostgreSQL 16, Redis 7, LocalStack 4.5) è esercitato da smoke HTTPS/CSP, da axe e da pa11y; i percorsi end-to-end di prodotto sono verificati manualmente. Ogni \glslink{test-di-sistema}{test di sistema} è tracciato nel \glslink{piano-di-qualifica}{Piano di Qualifica}. I \glslink{test-di-sistema}{test di sistema} e il collaudo appartengono al processo di validazione.

\subparagraph{Test di regressione}
I \glslink{test-di-regressione}{test di regressione} accertano che gli interventi correttivi o evolutivi non abbiano introdotto difetti nelle funzionalità già esistenti. Non richiedono la scrittura di test dedicati: sono garantiti dalla riesecuzione automatica dell'intera suite \glslink{pest}{Pest} e \glslink{jest}{Jest} (insieme a contratto OpenAPI, smoke e a11y) nella pipeline di \textit{Continuous Integration} a ogni \textit{push} o apertura di \textit{\glslink{pull-request}{Pull Request}}. Un test precedentemente superato che risulti fallito segnala una regressione e blocca l'integrazione fino alla risoluzione.
...
\begin{itemize}
    \item \textbf{Ambiente di esecuzione:} l'applicativo viene eseguito localmente tramite \glslink{docker-compose}{Docker Compose} con Traefik 3.4, nginx 1.29, app, worker, PostgreSQL 16, Redis 7 e LocalStack 4.5 (S3, SQS, Step Functions, Secrets Manager).
    \item \textbf{Framework applicativi:} il \glslink{backend}{backend} è realizzato con \glslink{laravel}{Laravel} 12 e il frontend con una \glslink{spa-single-page-application}{SPA} \glslink{angular}{Angular} 21; le funzionalità di \glslink{intelligenza-artificiale-ai}{Intelligenza Artificiale} sono erogate tramite \glslink{amazon-bedrock}{Amazon Bedrock} e l'OCR tramite Amazon Textract.
    \item \textbf{Strumenti di qualità:} \glslink{laravel}{Laravel} Pint (preset laravel), Larastan livello 3, \glslink{pest}{Pest} \glslink{php}{PHP} e \glslink{jest}{Jest}, secondo le soglie e le procedure fissate nel processo di verifica.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\glslink{docker-compose}{Docker Compose} e LocalStack:} \url{https://docs.docker.com/compose/} e \url{https://docs.localstack.cloud/}
...
\begin{itemize}
    \item \textbf{Descrizione:} Percentuale di codice sorgente eseguita durante i test automatici, distinta per stack. Sul backend si misurano linee e rami (Pest, report Cobertura sul package \texttt{app/}); sul frontend si misurano statement, functions, rami e linee (Jest). Si misura inoltre la \textit{diff coverage}, cioè la percentuale di linee modificate coperte dai test. La metrica è applicabile in \glslink{pb-product-baseline}{PB} e i valori sono rilevati dalla CI. Le soglie che bloccano l'integrazione sono: backend linee $\geq 80\%$ e rami $\geq 70\%$; frontend statements e functions $\geq 80\%$ e rami $\geq 70\%$; diff coverage $\geq 80\%$.
    \item \textbf{Formula:}
    $$\text{CC} = \frac{\text{Unità di codice eseguite dai test}}{\text{Unità di codice totali}} \times 100$$
    dove l'unità è la linea (backend), lo statement o il ramo (frontend), secondo il report dello stack.
    \item \textbf{Unità di misura:} percentuale (\%)
\end{itemize}
