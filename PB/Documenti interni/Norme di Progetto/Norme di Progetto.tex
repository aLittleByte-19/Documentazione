...
\paragraph{Strumenti a supporto}
\begin{itemize}
    \item \textbf{\glslink{laravel}{Laravel} 12 e \glslink{angular}{Angular} 21:} per la realizzazione rispettivamente del \glslink{backend}{backend} applicativo e della \glslink{spa-single-page-application}{SPA} frontend in \glslink{typescript}{TypeScript}.
    ...
    \item \textbf{\glslink{docker-compose}{Docker Compose} e \glslink{localstack}{LocalStack}:} per l'esecuzione locale dello stack applicativo (\glslink{traefik}{Traefik}, \glslink{nginx}{nginx}, app, \glslink{worker}{worker}, \glslink{postgresql}{PostgreSQL} 16, \glslink{redis}{Redis} 7, emulazione \glslink{s3-simple-storage-service}{S3}/\glslink{sqs-simple-queue-service}{SQS}/\glslink{step-functions}{Step Functions}/Secrets).
    \item \textbf{\glslink{laravel}{Laravel} Pint, Larastan, \glslink{pest}{Pest} \glslink{php}{PHP} e \glslink{jest}{Jest}:} per la formattazione, l'\glslink{analisi-statica}{analisi statica} e i test del codice prodotto.
...
\begin{enumerate}
    ...
    \item Il \glslink{programmatore}{Programmatore} esegue \glslink{laravel}{Laravel} Pint e Larastan in locale quando possibile e corregge le segnalazioni prima del \textit{commit}. Il vincolo di integrazione resta la pipeline CI.
    ...
    \item Il \glslink{programmatore}{Programmatore} apre la \textit{\glslink{pull-request}{Pull Request}} verso il ramo di integrazione assegnandola al \glslink{verificatore}{Verificatore} designato. Gli esiti di test e copertura sono gli artifact della CI, non un allegato obbligatorio della PR.
\end{enumerate}
...
Il codice \glslink{php}{PHP} prodotto deve essere conforme allo standard \textbf{PSR-12} (\textit{\glslink{php}{PHP} Standards Recommendations 12}), seguito da \glslink{laravel}{Laravel} 12. La formattazione automatica è applicata da \glslink{laravel}{Laravel} Pint con il preset \texttt{\glslink{laravel}{laravel}} (derivato da PSR-12). PSR-12 definisce regole su indentazione (4 spazi), apertura delle parentesi, dichiarazione dei tipi e struttura delle classi, garantendo uniformità in tutto il codebase indipendentemente dal membro che scrive il codice.

\subparagraph{Documentazione del codice}
Classi e metodi pubblici la cui firma non è autoesplicativa riportano un blocco di commento con scopo, parametri e valore di ritorno. Le annotazioni di tipo sono obbligatorie dove il tipo non è inferibile dalla firma e vengono validate dall'\glslink{analisi-statica}{analisi statica} in \textit{pipeline}.
...
\begin{itemize}
    ...
    \item \textbf{Variabili d'ambiente:} UPPER\_SNAKE\_CASE — es. \texttt{\glslink{aws}{AWS}\_BUCKET}, \texttt{DB\_CONNECTION}
\end{itemize}

\subparagraph{Strumenti di linting e formattazione}
\begin{itemize}
    \item \textbf{\glslink{laravel}{Laravel} Pint:} tool ufficiale di formattazione per \glslink{laravel}{Laravel}, basato su \glslink{php}{PHP} CS Fixer. In assenza di \texttt{pint.\glslink{json}{json}} si applica il preset \texttt{\glslink{laravel}{laravel}} (derivato da PSR-12). Il codice non conforme è bloccato dalla CI prima dell'integrazione nel \glslink{branch}{branch} principale.
    \item \textbf{Larastan:} estensione di PHPStan ottimizzata per \glslink{laravel}{Laravel}. Esegue l'\glslink{analisi-statica}{analisi statica} del codice per rilevare errori di tipo, chiamate a metodi inesistenti e potenziali bug a runtime, senza eseguire il codice. Il livello minimo di analisi adottato è il \textbf{livello 3} (paths \texttt{app}, \texttt{routes}, \texttt{config}).
\end{itemize}

\paragraph{Integrazione e rilascio}
\begin{enumerate}
    ...
    \item Il \glslink{verificatore}{Verificatore} approva la \textit{\glslink{pull-request}{Pull Request}} dopo la \glslink{code-review}{code review}; l'integrazione nei rami protetti resta comunque bloccata finché i job CI obbligatori non sono verdi.
...
\begin{enumerate}
    ...
    \item Il \glslink{verificatore}{Verificatore} verifica che ogni requisito dichiarato soddisfatto abbia il relativo \glslink{test-di-sistema}{test di sistema} in stato S nel \glslink{piano-di-qualifica}{Piano di Qualifica} (gli opzionali non implementati restano NI).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{\glslink{github}{GitHub} Actions:} pipeline di Continuous Integration che esegue, tra gli altri, composer validate/audit, Larastan livello 3, \glslink{laravel}{Laravel} Pint, \glslink{pest}{Pest}, ESLint 9 con regole a11y sui template, typecheck, \glslink{jest}{Jest} 30, \glslink{redocly}{Redocly} e \glslink{orval}{Orval}, diff-cover, \glslink{trivy}{Trivy}, \glslink{npm}{npm} audit, \glslink{terraform}{terraform} fmt/validate, \glslink{axe}{axe} e \glslink{pa11y}{pa11y}, smoke HTTPS/CSP dello stack.
    \item \textbf{Larastan e \glslink{laravel}{Laravel} Pint:} per l'\glslink{analisi-statica}{analisi statica} automatica e la verifica di conformità stilistica del codice \glslink{php}{PHP}.
    \item \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}:} per i \glslink{test-di-unita}{test di unità} e di integrazione del \glslink{backend}{backend}.
    \item \textbf{\glslink{jest}{Jest}:} per i \glslink{test-di-unita}{test di unità} del frontend \glslink{angular}{Angular}.
    \item \textbf{\glslink{docker-compose}{Docker Compose}:} per lo smoke dello stack completo in CI.
\end{itemize}
...
In affiancamento alle tecniche manuali, il team adotta \textbf{Larastan} (PHPStan per \glslink{laravel}{Laravel}) come strumento di \glslink{analisi-statica}{analisi statica} automatica del codice. Larastan viene eseguito al livello di analisi \textbf{3} sui path \texttt{app}, \texttt{routes} e \texttt{config}. A questo livello non sostituisce la metrica \textbf{MPD12} (code smell, coupling, complessità ciclomatica), che resta non misurata in assenza di uno strumento dedicato. L'esecuzione locale è raccomandata; il vincolo di integrazione è la pipeline di \textit{Continuous Integration}: il codice che non supera l'analisi non può essere integrato nel \glslink{branch}{branch} principale.
...
\begin{enumerate}
    ...
    \item Verificare che i test \glslink{pest}{Pest} pertinenti passino in locale con il comando \texttt{./vendor/bin/\glslink{pest}{pest}}; per il frontend, eseguire la suite \glslink{jest}{Jest}.
    \item La copertura è misurata in CI rispetto a \texttt{coverage-thresholds.\glslink{json}{json}}: \glslink{backend}{backend} linee $\geq 80\%$ e rami $\geq 70\%$; frontend statements e functions $\geq 80\%$ e rami $\geq 70\%$; diff coverage sulle linee modificate $\geq 80\%$. Il gate è globale sulla suite, non per singolo tipo di test.
    \item Gli esiti (report di copertura, log dei job) restano negli artifact della CI, consultabili dalla \textit{\glslink{pull-request}{Pull Request}}.
\end{enumerate}

La pipeline di \textit{Continuous Integration} verifica automaticamente questi requisiti a ogni \textit{push}, sulle \textit{\glslink{pull-request}{Pull Request}}, sui tag \texttt{v*} e in esecuzione manuale. Una modifica non può essere integrata nei rami \texttt{main} e \texttt{develop} finché i job CI obbligatori non sono verdi (ruleset: una approvazione e quattro check).

\subparagraph{Test di unità}
I \glslink{test-di-unita}{test di unità} verificano in isolamento singole componenti software. Sul \glslink{backend}{backend} il framework è \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}}: 86 casi in \texttt{tests/Unit/} e 138 in \texttt{tests/DomainUnit/}, senza \glslink{bootstrap}{bootstrap} completo di \glslink{laravel}{Laravel} dove non necessario. Sul frontend i test unitari sono scritti con \textbf{\glslink{jest}{Jest}} 30 e \texttt{jest-preset-\glslink{angular}{angular}} (jsdom): 510 casi in 53 file.

\subparagraph{Test di integrazione}
I \glslink{test-di-integrazione}{test di integrazione} confermano che le componenti del software operino tra loro in modo sinergico e privo di errori. Sono scritti con \textbf{\glslink{pest}{Pest} \glslink{php}{PHP}}, risiedono in \texttt{tests/Feature/} (271 test in 21 file) e usano il kernel di test di \glslink{laravel}{Laravel} con database SQLite in memoria; i client \glslink{aws}{AWS} sono simulati. La copertura minima non è un gate per tipo di test: vale il \glslink{quality-gate}{quality gate} globale della suite \glslink{backend}{backend}.

\subparagraph{Test di sistema}
I \glslink{test-di-sistema}{test di sistema} accertano che il comportamento globale del prodotto rispetti i requisiti stabiliti nell'\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti}. Non esiste una suite e2e automatica con asserzioni di scenario. In CI lo stack \glslink{docker-compose}{Docker Compose} (\glslink{traefik}{Traefik} 3.4, \glslink{nginx}{nginx} 1.29, app, \glslink{worker}{worker}, \glslink{postgresql}{PostgreSQL} 16, \glslink{redis}{Redis} 7, \glslink{localstack}{LocalStack} 4.5) è esercitato da smoke HTTPS/CSP, da \glslink{axe}{axe} e da \glslink{pa11y}{pa11y}; i percorsi end-to-end di prodotto sono verificati manualmente. Ogni \glslink{test-di-sistema}{test di sistema} è tracciato nel \glslink{piano-di-qualifica}{Piano di Qualifica}. I \glslink{test-di-sistema}{test di sistema} e il collaudo appartengono al processo di validazione.

\subparagraph{Test di regressione}
I \glslink{test-di-regressione}{test di regressione} accertano che gli interventi correttivi o evolutivi non abbiano introdotto difetti nelle funzionalità già esistenti. Non richiedono la scrittura di test dedicati: sono garantiti dalla riesecuzione automatica dell'intera suite \glslink{pest}{Pest} e \glslink{jest}{Jest} (insieme a contratto \glslink{openapi}{OpenAPI}, smoke e a11y) nella pipeline di \textit{Continuous Integration} a ogni \textit{push} o apertura di \textit{\glslink{pull-request}{Pull Request}}. Un test precedentemente superato che risulti fallito segnala una regressione e blocca l'integrazione fino alla risoluzione.
...
\begin{enumerate}
    ...
    \item Esecuzione dei \glslink{test-di-sistema}{test di sistema} e collaudo.
...
\begin{enumerate}
    ...
    \item Un requisito è dichiarato soddisfatto solo quando il relativo \glslink{test-di-sistema}{test di sistema} nel \glslink{piano-di-qualifica}{Piano di Qualifica} è in stato S. Gli opzionali non implementati restano NI e non contano come soddisfatti.
\end{enumerate}

\paragraph{Test di sistema e collaudo}
I \glslink{test-di-sistema}{test di sistema} e il collaudo appartengono alla validazione. Lo stato S poggia sulla suite Feature \glslink{pest}{Pest}, sui test \glslink{jest}{Jest} e sulla verifica manuale dello stack; TS-Q e TS-V poggiano sui \glslink{quality-gate}{quality gate} della CI. Non risulta un verbale di collaudo con la \glslink{committente-proponente}{proponente}.
...
Le ore sono catalogate secondo tre dimensioni: \textbf{per ruolo}, tra \glslink{responsabile}{Responsabile}, \glslink{amministratore}{Amministratore}, \glslink{analista}{Analista}, \glslink{progettista}{Progettista}, \glslink{programmatore}{Programmatore} e \glslink{verificatore}{Verificatore}; \textbf{per membro}, in modo che sia sempre noto il monte ore residuo individuale per ciascuna figura professionale; \textbf{per categoria}, tra attività produttive, verifica, riunioni e coordinamento, studio e overhead, usate per il calcolo di MPC-TE e MPC-LE nel \glslink{piano-di-qualifica}{Piano di Qualifica}. Le ore dedicate alla formazione sono registrate a carico del ruolo per cui la formazione è funzionale. La ripartizione per categoria è consolidata in chiusura di \glslink{sprint}{sprint}, a partire dai consuntivi per ruolo.
...
\begin{itemize}
    \item \textbf{Ambiente di esecuzione:} l'applicativo viene eseguito localmente tramite \glslink{docker-compose}{Docker Compose} con \glslink{traefik}{Traefik} 3.4, \glslink{nginx}{nginx} 1.29, app, \glslink{worker}{worker}, \glslink{postgresql}{PostgreSQL} 16, \glslink{redis}{Redis} 7 e \glslink{localstack}{LocalStack} 4.5 (\glslink{s3-simple-storage-service}{S3}, \glslink{sqs-simple-queue-service}{SQS}, \glslink{step-functions}{Step Functions}, Secrets Manager).
    \item \textbf{Framework applicativi:} il \glslink{backend}{backend} è realizzato con \glslink{laravel}{Laravel} 12 e il frontend con una \glslink{spa-single-page-application}{SPA} \glslink{angular}{Angular} 21; le funzionalità di \glslink{intelligenza-artificiale-ai}{Intelligenza Artificiale} sono erogate tramite \glslink{amazon-bedrock}{Amazon Bedrock} e l'\glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR} tramite Amazon \glslink{textract}{Textract}.
    \item \textbf{Strumenti di qualità:} \glslink{laravel}{Laravel} Pint (preset \glslink{laravel}{laravel}), Larastan livello 3, \glslink{pest}{Pest} \glslink{php}{PHP} e \glslink{jest}{Jest}, secondo le soglie e le procedure fissate nel processo di verifica.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Sviluppo:} \glslink{php}{PHP} 8.4 con \glslink{laravel}{Laravel} 12 e Pint (preset \glslink{laravel}{laravel}), \glslink{angular}{Angular} 21 e \glslink{typescript}{TypeScript} 5.9 per la \glslink{spa-single-page-application}{SPA}, \glslink{postgresql}{PostgreSQL} 16 per la persistenza, \glslink{redis}{Redis} per cache, sessioni e rate limiting, Amazon \glslink{sqs-simple-queue-service}{SQS} per le code, \glslink{localstack}{LocalStack} per l'emulazione dei servizi \glslink{aws}{AWS} e \glslink{docker-compose}{Docker Compose} per l'orchestrazione dell'ambiente locale.
    \item \textbf{Verifica:} \glslink{pest}{Pest} \glslink{php}{PHP} e \glslink{jest}{Jest} per i test automatici, Larastan livello 3 e \glslink{laravel}{Laravel} Pint per l'\glslink{analisi-statica}{analisi statica} e la formattazione, più i \glslink{quality-gate}{quality gate} della CI.
...
\begin{itemize}
    \item \textbf{\glslink{laravel}{Laravel} e \glslink{angular}{Angular}:} \url{https://laravel.com/docs} e \url{https://angular.dev}
    \item \textbf{\glslink{php}{PHP} e PSR-12:} \url{https://www.php.net/docs.php} e \url{https://www.php-fig.org/psr/psr-12/}
    \item \textbf{\glslink{pest}{Pest} \glslink{php}{PHP} e \glslink{jest}{Jest}:} \url{https://pestphp.com/docs/installation} e \url{https://jestjs.io/docs/getting-started}
    ...
    \item \textbf{\glslink{docker-compose}{Docker Compose} e \glslink{localstack}{LocalStack}:} \url{https://docs.docker.com/compose/} e \url{https://docs.localstack.cloud/}
...
Per definire e misurare la qualità intrinseca del software, lo standard di riferimento adottato è la serie di norme ISO/IEC 9126. Questo standard internazionale organizza la qualità in tre categorie: qualità interna, qualità esterna e qualità in uso. In questo documento si adottano le sei caratteristiche di prodotto (funzionalità, affidabilità, efficienza, usabilità, mantenibilità, portabilità) e la qualità in uso, che valuta la capacità del software di permettere agli utenti di raggiungere i propri obiettivi con efficacia, produttività, sicurezza e soddisfazione in specifici contesti d'uso.
...
Le metriche seguenti misurano l'estensione e l'esito dell'attività di \glslink{analisi-dinamica}{analisi dinamica} del processo di verifica. Sono rilevate automaticamente dalla pipeline di \textit{Continuous Integration} a ogni \textit{push}; i report restano negli artifact CI, consultabili dalla \textit{\glslink{pull-request}{Pull Request}}.

\paragraph{MPC-CC --- Code Coverage}
\begin{itemize}
    \item \textbf{Descrizione:} Percentuale di codice sorgente eseguita durante i test automatici, distinta per stack. Sul \glslink{backend}{backend} si misurano linee e rami (\glslink{pest}{Pest}, report Cobertura sul package \texttt{app/}); sul frontend si misurano statement, functions, rami e linee (\glslink{jest}{Jest}). Si misura inoltre la \textit{diff coverage}, cioè la percentuale di linee modificate coperte dai test. La metrica è applicabile in \glslink{pb-product-baseline}{PB} e i valori sono rilevati dalla CI. Le soglie che bloccano l'integrazione sono: \glslink{backend}{backend} linee $\geq 80\%$ e rami $\geq 70\%$; frontend statements e functions $\geq 80\%$ e rami $\geq 70\%$; diff coverage $\geq 80\%$.
    \item \textbf{Formula:}
    $$\text{CC} = \frac{\text{Unità di codice eseguite dai test}}{\text{Unità di codice totali}} \times 100$$
    dove l'unità è la linea (\glslink{backend}{backend}), lo statement o il ramo (frontend), secondo il report dello stack.
    \item \textbf{Unità di misura:} percentuale (\%)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Descrizione:} Numero di code smell rilevati nel codice sorgente tramite uno strumento di \glslink{analisi-statica}{analisi statica} dedicato. I code smell sono indicatori di progettazione debole (metodi troppo lunghi, classi troppo grandi, duplicazione di codice). Larastan al livello 3 non sostituisce questa misura; in \glslink{pb-product-baseline}{PB} la metrica non è valorizzata.
    \item \textbf{Formula:}
