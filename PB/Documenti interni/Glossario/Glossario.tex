...
\begin{center}
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|p{2cm}|p{2cm}|p{2cm}|}
        ...
        3.0.0 & 04/06/2026 & Approvazione versione 3.0.0 & - & - & Diego Belluz \\
        2.2.0 & 27/08/2026 & Aggiunti termini del Manuale Utente. & Angelica Gastaldello & Angela Maule & - \\
        2.1.0 & 26/08/2026 & Aggiunti termini della Specifica Tecnica. & Angelica Gastaldello & Angela Maule & - \\
...
\subsection{AI Assistant Generativo (AI Assistant)}
...
Registro cronologico delle operazioni eseguite all'interno del sistema, utile per la tracciabilità e la sicurezza.

\subsection{AuditLogger}
Componente responsabile della registrazione degli eventi nel audit trail, garantendo la tracciabilità delle operazioni.
...
\subsection{axe}
Strumento di testing automatico per l'accessibilità (a11y) dell'interfaccia utente, integrato nella pipeline 
...
Servizio AWS per il monitoraggio e la gestione dei log e delle metriche delle risorse cloud.
...
\subsection{EMF}
Embedded Metric Format, Specifica JSON utilizzata per generare metriche Amazon CloudWatch personalizzate in modo asincrono tramite eventi di log strutturati.
...
\subsection{Flysystem}
Libreria PHP per l'astrazione del filesystem, che permette di interagire con diversi tipi di storage in modo uniforme.
...
Framework di politiche, tecnologie e processi che permette di gestire le identità digitali e controllare l'accesso degli utenti alle risorse aziendali.
...
\subsection{Jest}
 è un popolare framework di testing JavaScript sviluppato per garantire la correttezza del codice.
...
\subsection{NPM}
comando integrato in Node.js che analizza le dipendenze di un progetto alla ricerca di vulnerabilità di sicurezza note
...
\subsection{pa11y}
Strumenti gratuiti e open-source progettata per automatizzare i test di accessibilità delle pagine web
...
Framework per l'automazione di un browser reale.
...
\subsection{Redocly}
Piattaforma di documentazione API e gestione del ciclo di vita delle API che aiuta i team di sviluppo a creare portali per sviluppatori professionali, validare specifiche e pubblicare guide interattive.
...
\subsection{Traefik}
Proxy inverso e load balancer per microservizi, che gestisce automaticamente il routing del traffico.

\subsection{Trigger}
Evento che innesca un caso d'uso.

\subsection{Trivy}
Scanner di sicurezza open source sviluppato da Aqua Security, semplice e veloce, progettato per trovare problemi di sicurezza nei sistemi cloud-native.
