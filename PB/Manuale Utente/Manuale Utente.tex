...
La sezione \ui{Qualità della generazione} riepiloga l'andamento dell'Assistant su un intervallo scelto dal redattore. In alto è disponibile \ui{Esporta report}, che genera un PDF con le stesse metriche e i feedback recenti visibili a schermo.
...
\begin{itemize}
    ...
    \item \ui{Configurazioni di prompt salvate}: numero di preset presenti;
    ...
    \item \ui{Esito delle bozze}: barra che ripartisce le generazioni fra \ui{In bozza} e \ui{Decise} (approvate o scartate).
\end{itemize}
...
\begin{itemize}
    ...
    \item \ui{Tempo medio di elaborazione}: durata media della pipeline OCR, dalla ricezione al risultato;
    ...
    \item \ui{Sotto la soglia di confidenza}: percentuale di sotto-documenti da revisionare perché sotto la soglia, sul totale analizzato;
    \item \ui{Destinatari riconosciuti in automatico}: percentuale di destinatari confermati che l'AI aveva già riconosciuto correttamente, mai corretti a mano (UC-56.4).
\end{itemize}
...
Nell'Overview, sotto \ui{Qualità degli strumenti}, il riquadro \ui{AI Assistant} mostra in forma compatta solo la \ui{Media stelle} (qualità percepita delle bozze) e il collegamento \ui{Vedi tutte le metriche}, che porta alla sezione \ui{Qualità della generazione} descritta in \ref{fig:assistant-metriche}: lì si trovano il totale dei contenuti generati, le configurazioni di prompt salvate, la percentuale di bozze valutate, l'esito delle bozze e i feedback recenti, con filtro per tono, stile e intervallo di date. Il segnale di qualità percepita resta la valutazione 1--5, unica per generazione.
...
\hline
\texttt{copilot-metriche.png} & Mosaico \ui{Qualità e performance OCR} (classificazioni, confidenza, destinatari). \\
\hline
\end{longtable}
}
\end{center}
