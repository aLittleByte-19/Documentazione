...
\screenshottodo{shell-completa}{Cornice dell'applicazione con barra laterale e intestazione}{Catturare l'intera finestra del browser sulla vista Overview, con la sidebar visibile a sinistra (gruppi Panoramica, AI Assistant, Co-Pilot CdL), il titolo ``Overview operativa'' e il selettore di tema in alto a destra. Preferire il tema chiaro, a larghezza desktop.}
...
\screenshottodo{sidebar-navigazione}{Barra laterale di navigazione}{Ritaglio della sola sidebar, con un gruppo espanso (ad esempio Assistant) e la voce attiva evidenziata.}
...
\screenshottodo{header-tema}{Intestazione con selettore del tema}{Ritaglio dell'header NEXUM con il titolo della pagina e il pulsante tema. Eventualmente una coppia chiaro/scuro affiancata.}
...
\screenshottodo{overview-hero}{Overview: avvio rapido}{Schermata della parte alta della Overview, con il testo introduttivo e i due pulsanti ``Crea contenuto'' e ``Carica documenti''.}
...
\screenshottodo{overview-metriche}{Overview: priorità, metriche e attività recenti}{Schermata completa sotto l'hero: le tre card di priorità, i due pannelli metriche (AI Assistant e Co-Pilot) e la tabella Attività recenti, con almeno una riga di esempio.}
...
\screenshottodo{assistant-composizione}{Modulo di generazione della bozza}{Sezione ``Crea una bozza'' compilata con un prompt di esempio, tono e stile selezionati, pulsante ``Genera bozza'' visibile e, sotto, il pulsante ``Salva configurazione''.}
...
\screenshottodo{assistant-avanzamento}{Barra di avanzamento della generazione}{Stesso modulo durante una generazione in corso: barra parzialmente piena e messaggio di stato (ad esempio ``Generazione del testo in corso'').}
...
\screenshottodo{assistant-revisione}{Anteprima della bozza generata}{Sezione ``Controlla il contenuto'' con copertina, titolo, testo, badge di stato ``Bozza'', azioni sulla copertina e pulsanti in basso. Usare una generazione conclusa con copertina visibile.}
...
\screenshottodo{assistant-modifica}{Bozza in modalità modifica}{Stessa anteprima con i campi titolo e testo editabili e i pulsanti ``Annulla'' e ``Salva'' al posto di ``Modifica''.}
...
\screenshottodo{assistant-valutazione}{Valutazione a stelle della bozza}{Ritaglio del blocco ``Valuta questa bozza'': stelle, campo commento con contatore caratteri e pulsante ``Invia valutazione''. In alternativa, lo stato successivo alla registrazione.}
...
\screenshottodo{assistant-azioni}{Azioni sulla bozza: storico, rigenerazione, scarto}{Ritaglio dei pulsanti ``Salva nello storico'', ``Rigenera bozza'' e ``Scarta bozza''. In una seconda cattura, il messaggio di conferma dello scarto.}
...
\screenshottodo{assistant-filtri}{Filtri dello storico contenuti}{Riga dei filtri (parola chiave, tono, stile, data, Azzera filtri) sopra l'elenco.}
...
\screenshottodo{assistant-configurazioni}{Preset di prompt salvati}{Blocco ``Configurazioni salvate'' con almeno un preset, pulsante ``Usa'' e icona di eliminazione.}
...
\screenshottodo{assistant-storico}{Elenco dello storico contenuti}{Due o tre card nello storico, di cui una selezionata (bordo evidenziato), con stella preferiti e icona elimina. Includere se possibile una stella attiva.}
...
\screenshottodo{copilot-caricamento}{Pannello di caricamento documenti}{Sezione ``Carica nel repository'' con tipologia selezionata, mese/anno/azienda compilati e area tratteggiata di upload, senza elaborazione in corso.}
...
\screenshottodo{copilot-avanzamento}{Avanzamento dell'analisi documentale}{Stesso pannello durante l'elaborazione, con barra di progresso e messaggio (ad esempio ``Analisi OCR del documento in corso'').}
...
\screenshottodo{copilot-storico}{Tabella dello storico documenti}{Tabella con almeno due righe, colonne visibili (Destinatario, Documento, File, Date, Conf., Stato, Invio, Azioni) e una riga con ``Consulta'' attivo.}
...
\screenshottodo{copilot-filtri}{Filtri dello storico documenti}{Riga completa dei filtri sopra la tabella: ricerca, stato invio, confidenza, soglia, mese, anno, Azzera filtri.}
...
\screenshottodo{copilot-dettaglio}{Dettaglio documento: anteprima e dati estratti}{Vista affiancata: PDF a sinistra, ispettore campi a destra, con legenda modificabile/sola lettura e stato di revisione visibile. Usare un documento già elaborato.}
...
\screenshottodo{copilot-revisione}{Modifica dei dati estratti}{Ispettore in modalità modifica, con almeno un campo evidenziato da errore (email o codice fiscale) e i pulsanti ``Annulla'' e ``Salva''.}
...
\screenshottodo{copilot-messaggio}{Pannello del messaggio di invio}{Blocco ``Messaggio di invio'' aperto, con destinatario, oggetto e testo precompilati, più i collegamenti ``Apri anteprima'' e ``Scarica PDF''.}
...
\screenshottodo{copilot-metriche}{Pannello metriche del Co-Pilot}{Sezione ``Qualità e performance OCR'' con le card delle metriche popolate.}
