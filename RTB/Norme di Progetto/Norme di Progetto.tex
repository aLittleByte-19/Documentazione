...
La stesura del seguente documento segue un approccio \glslink{incrementale}{incrementale}: le norme qui descritte verranno costantemente aggiornate e affinate nel tempo, supportando il miglioramento continuo dei processi organizzativi, di sviluppo e di supporto del gruppo.


\subsection{Scopo del prodotto}
Il prodotto da realizzare consiste in una piattaforma software \textit{stand-alone} sviluppata su richiesta dell'azienda \glslink{committente-o-proponente}{proponente} \textit{Eggon}, che si affianca a \textit{\glslink{nexum}{Nexum}} — piattaforma digitale dedicata alla comunicazione interna e alla gestione HR — comunicando con essa tramite API. Il sistema è architetturalmente indipendente da \glslink{nexum}{Nexum} e comprende due moduli basati sull'\glslink{ai-intelligenza-artificiale}{Intelligenza Artificiale}:
\begin{itemize}
    \item \textbf{\glslink{ai-intelligenza-artificiale}{AI} Assistant Generativo:} Un modulo dedicato agli utenti amministrativi (\glslink{dashboard}{dashboard}) che permette la creazione autonoma di contenuti aziendali (titolo, testo, immagini di copertina) a partire da un \glslink{prompt}{prompt}, garantendo l'adeguamento automatico del tono e dello stile del messaggio.
    \item \textbf{\glslink{ai-co-pilot}{AI Co-Pilot} per Consulenti del Lavoro (\glslink{consulente-del-lavoro-cdl}{CdL}):} Un sistema avanzato in grado di riconoscere automaticamente la tipologia di documenti caricati (es. cedolini, comunicazioni), estrarne i destinatari tramite tecniche di \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR} ed \textbf{\glslink{entity-resolution}{Entity Resolution}}, e automatizzare lo smistamento e il \glslink{dispaccio-dispatch}{dispaccio} massivo.
\end{itemize}
...
Al fine di prevenire qualsiasi ambiguità linguistica o concettuale tra i membri del team e gli \glslink{stakeholder}{stakeholder}, è stato redatto un documento separato denominato Glossario. Esso contiene la spiegazione puntuale di tutti i termini tecnici, di dominio (es. \glslink{llm-large-language-model}{LLM}, \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR}, \glslink{dispaccio-dispatch}{Dispaccio}, ecc.) e degli acronimi utilizzati all'interno della documentazione di progetto.
...
\begin{itemize}
    \item \textbf{Glossario:} Documento interno del progetto contenente la definizione dei termini tecnici, di dominio e degli acronimi utilizzati nella documentazione.
\end{itemize}
...
Il processo di fornitura definisce le procedure e le attività che il gruppo mette in atto per stabilire un accordo operativo con la \glslink{committente-o-proponente}{proponente} (\textit{Eggon}). L'obiettivo è trasformare le necessità espresse nel \glslink{capitolato}{capitolato} C5 in un insieme di requisiti tecnici e vincoli contrattuali ben definiti. In questa fase si stabiliscono le basi metodologiche del \textit{"\glslink{way-of-working-wow}{Way of Working}''} del team, garantendo che ogni attività sia pianificata, tracciabile e verificabile.
...
\begin{itemize}
    ...
    \item \textbf{Piano di Qualifica:} Specifica la strategia per garantire che il software sia privo di difetti critici. Definisce le metriche (es. \textit{\glslink{code-coverage}{Code Coverage}} $\geq$ 80\%) e i test (unità, integrazione, sistema) necessari alla validazione.
    ...
    \item \textbf{Specifica Tecnica:} Documento di design che descrive l'architettura di sistema (\glslink{modello-c4}{modello C4}), i diagrammi delle classi UML e i \glslink{design-pattern}{design pattern} scelti per l'implementazione.
    \item \textbf{Manuale Utente:}  Guida operativa destinata agli utenti finali per facilitare l'onboarding sulla piattaforma \glslink{nexum}{Nexum} e spiegare l'utilizzo degli assistenti \glslink{ai-intelligenza-artificiale}{AI}.
...
\begin{itemize}
    \item \textbf{GitHub:} Per il versionamento della documentazione tramite \textit{\glslink{pull-request}{Pull Request}} e per l'integrazione con il codice preesistente.
    ...
    \item \textbf{Tracciamento:} Ogni decisione rilevante viene convertita in una \textit{\glslink{issue}{issue}} su GitHub per garantirne l'assegnazione e il completamento.
\end{itemize}
...
Il ciclo di sviluppo per l'integrazione dei modelli di \glslink{ai-intelligenza-artificiale}{Intelligenza Artificiale} richiede una sequenza logica di operazioni ingegneristiche specifiche:
\begin{itemize}
    ...
    \item \textbf{Validazione "\glslink{human-in-the-loop}{Human-in-the-loop}":} Implementazione di flussi di sviluppo che prevedano sempre un passaggio di validazione umana per le operazioni \glslink{ai-intelligenza-artificiale}{AI} a bassa \glslink{confidenza-confidence-score}{confidenza} (es. revisione dei destinatari estratti o modifica del testo generato).
    \item \textbf{Collaudo e Rilascio:} Esecuzione di test formali sulle metriche di qualità (es. tempi di risposta, tasso di successo dell'\glslink{ai-intelligenza-artificiale}{AI}) prima dell'approvazione finale.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item UC-[Numero] identifica i casi d'uso relativi all'\textbf{\glslink{ai-intelligenza-artificiale}{AI} Assistant Generativo} (es. \textit{UC-1}).
        \item UC-CO-[Numero] identifica i casi d'uso relativi all'\textbf{\glslink{ai-co-pilot}{AI Co-Pilot}} (es. \textit{UC-CO-1}).
    ...
    \end{itemize}
    \item \textbf{Struttura Descrittiva:} Ogni \glslink{caso-d-uso-uc}{caso d'uso} documentato deve presentare una scheda strutturata contenente: Attori coinvolti, Pre e Post-condizioni, \glslink{scenario-principale}{Scenario Principale} (sequenza ottimale numerata), Scenari Alternativi o di Errore (identificati con codici specifici come UC-1.E1), e Relazioni (Inclusioni, Estensioni).
\end{itemize}

\paragraph{Progettazione Architetturale}
La progettazione traduce le specifiche in un'infrastruttura flessibile e scalabile.
\begin{itemize}
    \item \textbf{\glslink{disaccoppiamento}{Disaccoppiamento} e Asincronia:} Data la natura \textit{\glslink{data-driven}{data-driven}} e i tempi di latenza dell'\glslink{ai-intelligenza-artificiale}{AI}, l'architettura deve garantire l'\glslink{esecuzione-asincrona}{esecuzione asincrona} delle operazioni pesanti (es. elaborazione \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR} o \glslink{entity-resolution}{entity resolution}).
...
\begin{itemize}
    \item \textbf{\glslink{modularita}{Modularità} e Riuso:} Suddivisione del codice in moduli con responsabilità singola (es. libreria dei \glslink{prompt}{prompt} separata dal motore di generazione). Applicazione di pattern architetturali per evitare dipendenze dirette e favorire la manutenibilità.
    \item \textbf{Gestione degli Errori e \glslink{resilienza}{Resilienza}:} Implementazione di meccanismi robusti per la gestione di anomalie, retry di rete e fallback nel caso in cui i servizi \glslink{ai-intelligenza-artificiale}{AI} esterni non siano temporaneamente disponibili (come mappato negli scenari alternativi dei casi d'uso).
    \item \textbf{Tracciabilità e Qualità:} Versionamento continuo del codice, revisione tra pari (\textit{\glslink{code-review}{Code Review}}) e test automatizzati per validare non solo le logiche applicative, ma anche l'integrazione corretta dei parametri (tono, stile, \glslink{metadati}{metadati}) passati \glslink{ai-intelligenza-artificiale}{ai} core di \glslink{ai-intelligenza-artificiale}{intelligenza artificiale}.
\end{itemize}

\subparagraph{Standard Stilistici}
Il codice PHP prodotto deve essere conforme allo standard \textbf{PSR-12} (\textit{PHP Standards Recommendations 12}), adottato ufficialmente dalla comunità PHP e seguito nativamente da Laravel 12. PSR-12 definisce regole precise su indentazione (4 spazi), apertura delle parentesi, dichiarazione dei tipi e struttura delle classi, garantendo uniformità in tutto il codebase indipendentemente dal membro che scrive il codice.

\subparagraph{Convenzioni di Naming nel Codice}
Le convenzioni seguono gli standard Laravel e PSR-12:
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Laravel Pint:} Tool ufficiale di formattazione per Laravel, basato su PHP CS Fixer. Applica automaticamente PSR-12 al codebase. Deve essere eseguito prima di ogni commit; il codice non conforme non può essere integrato nel \glslink{branch-ramo}{branch} principale.
    \item \textbf{Larastan:} \glslink{estensione}{Estensione} di PHPStan ottimizzata per Laravel. Esegue \glslink{analisi-statica}{analisi statica} del codice per rilevare errori di tipo, chiamate a metodi inesistenti e potenziali bug a runtime, senza eseguire il codice. Il livello minimo di analisi adottato è il \textbf{livello 5}.
\end{itemize}

\subsection{Processi di supporto}
I processi di supporto comprendono tutte le attività, i metodi e gli strumenti che non sono direttamente legati alla scrittura del codice, ma che risultano indispensabili per sostenere i processi primari di sviluppo e fornitura. Per il nostro team, definire questi processi significa stabilire delle regole di base condivise, necessarie per collaborare in modo efficiente ed evitare disorganizzazione durante le varie fasi del progetto didattico. Ciò \glslink{integrazione-continua-continuous-integration-ci}{ci} permette, a livello pratico, di garantire la tracciabilità di ogni decisione tecnica e organizzativa presa dal gruppo e di assicurare che tutto il materiale prodotto (documenti, codice, \glslink{issue}{issue}) sia costantemente coerente, aggiornato e facilmente accessibile a tutti i membri del team e al \glslink{committente-o-proponente}{committente}.

\subsubsection{Documentazione}

\paragraph{Scopo}
Lo scopo del processo di documentazione è stabilire regole e metodologie pratiche per la creazione, la gestione e l'aggiornamento continuo dei documenti di progetto. Per il nostro gruppo, impegnarci a produrre materiale che si mantenga costantemente chiaro, accessibile e aggiornato \glslink{integrazione-continua-continuous-integration-ci}{ci} permette di agevolare e rendere trasparente la comunicazione interna tra i membri del gruppo. Inoltre, una repository ben strutturata facilita notevolmente il confronto e l'allineamento con gli \glslink{stakeholder}{stakeholder} esterni e l'azienda \textit{Eggon}. Questo approccio rigoroso garantisce un'accurata tracciabilità delle decisioni organizzative e progettuali prese nel corso dello sviluppo, assicurando al contempo che l'intero progetto venga gestito in piena conformità agli standard di qualità che \glslink{integrazione-continua-continuous-integration-ci}{ci} siamo prefissati di rispettare.
...
\begin{itemize}
    \item \textbf{GitHub:} Rappresenta la nostra piattaforma principale per l'archiviazione e il tracciamento storico dei file. Sfruttiamo le funzionalità di branching e \glslink{pull-request}{pull request} per garantire che ogni modifica venga revisionata in modo asincrono.
    \item \textbf{GitHub Projects:} Utilizzato come strumento principale di project management. Il team gestisce il flusso di lavoro tramite una board Kanban dedicata (''aLittleByte - \glslink{dashboard}{Dashboard}'') che permette di visualizzare lo stato di tutte le attività.
    \item \textbf{GitHub Actions e GitHub Pages:} Impiegati per l'automazione (\glslink{integrazione-continua-continuous-integration-ci}{Continuous Integration}/Continuous Deployment) e per la pubblicazione automatica e trasparente del sito web documentale.
...
\begin{itemize}
    ...
    \item \textbf{Norme di progetto:} Il documento in esame, avente lo scopo di codificare il nostro \glslink{way-of-working-wow}{Way of Working}, illustrando nel dettaglio le metodologie operative, gli strumenti scelti e le regole di collaborazione interne al team.
    ...
    \item \textbf{Lettera di Presentazione:} La documentazione formale di accompagnamento, debitamente firmata, necessaria per certificare e presentare ufficialmente il \glslink{rtb-requirements-and-technology-baseline}{RTB} al \glslink{committente-o-proponente}{committente}.
    \item \textbf{Verbali:} I Verbali (interni ed esterni) tengono traccia degli incontri e delle decisioni sia tra i membri del gruppo e che tra il gruppo e la \glslink{committente-o-proponente}{proponente}.
\end{itemize}

\paragraph{Struttura dei documenti}
Tutti i documenti (eccetto i verbali e i \glslink{diari-di-bordo}{diari di bordo}) hanno la seguente struttura generale: 
\begin{itemize}
...
Questa suddivisione in sezioni permette di ottimizzare i tempi della presentazione, garantendo che i messaggi chiave arrivino in modo chiaro e che \glslink{integrazione-continua-continuous-integration-ci}{ci} si possa concentrare immediatamente sulla discussione e risoluzione dei problemi.
...
\begin{itemize}
    ...
    \item \textbf{\glslink{diari-di-bordo}{Diari di Bordo}:} Per i \glslink{diari-di-bordo}{Diari di Bordo} viene adottato il seguente standard di denominazione: \texttt{Diario\_di\_Bordo\_AAAA\_MM\_GG.pdf}. Ad esempio, \texttt{Diario\_di\_Bordo\_2026\_04\_13.pdf} rappresenta il diario di bordo presentato il giorno 13/04/2026.
...
\begin{itemize}
    ...
    \item \textbf{Creazione del \glslink{branch-ramo}{branch}:} Prima di iniziare la stesura, viene aperto un \textit{\glslink{branch-ramo}{branch}} dedicato nel repository GitHub. Il gruppo \textit{aLittleByte} ha stabilito di creare un \glslink{branch-ramo}{ramo} separato per ogni singolo file, inclusi i verbali e i \glslink{diari-di-bordo}{diari di bordo}, al fine di garantire una gestione granulare delle modifiche e un isolamento totale del lavoro rispetto al \glslink{branch-ramo}{ramo} principale.
    
    \item \textbf{Redazione:} Il \glslink{redattore}{redattore} incaricato sviluppa il contenuto seguendo i template e le direttive stilistiche definite nelle presenti Norme di Progetto. La scrittura avviene tramite gli strumenti appropriati (ad esempio \textit{\glslink{latex}{LaTeX}}), assicurando un output professionale e coerente.
    
    \item \textbf{Revisione:} Terminata la stesura, il documento viene sottoposto a una verifica asincrona. Il \glslink{verificatore}{verificatore} analizza il file tramite una \textit{\glslink{pull-request}{pull request}} su GitHub, segnalando eventuali correzioni necessarie o suggerimenti direttamente nei commenti della piattaforma.
    
    \item \textbf{Integrazione e Approvazione:} Una volta superata la revisione e ottenuta l'approvazione formale dal \glslink{responsabile}{Responsabile}, il contenuto viene integrato nel \textit{\glslink{branch-ramo}{branch} main} del repository. In questa fase viene consolidata la versione finale nel registro delle modifiche.
...
\begin{itemize}
    \item \textbf{\glslink{git}{Git}:} Software per il controllo di versione distribuito che permette di monitorare ogni variazione apportata \glslink{ai-intelligenza-artificiale}{ai} file sorgente e alla documentazione.
    \item \textbf{GitHub:} Servizio di hosting basato su cloud utilizzato per la gestione centralizzata del repository. Viene impiegato non solo per l'archiviazione, ma anche per la \glslink{pianificazione}{pianificazione} operativa tramite le \textit{\glslink{issue}{Issue}} e per la revisione del lavoro attraverso le \textit{\glslink{pull-request}{Pull Request}}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\glslink{pull-request}{Pull Request}:} Rappresenta il meccanismo esclusivo per l'integrazione di qualsiasi modifica all'interno del repository. Al fine di mantenere un elevato standard qualitativo e prevenire errori, il team ha stabilito che nessun componente è autorizzato a effettuare modifiche dirette sul \glslink{branch-ramo}{ramo} principale (\texttt{main}). Perciò ogni integrazione deve necessariamente passare per una \textit{\glslink{pull-request}{Pull Request}}, la quale richiede obbligatoriamente un processo di revisione incrociata e può essere inserita nel main solo a seguito di almeno una revisione positiva da parte di altri membri del gruppo. 
\end{itemize}
...
Per assicurare che la documentazione fornita agli \glslink{stakeholder}{stakeholder} sia costantemente aggiornata e pubblica, il gruppo ha implementato un processo di \textit{Continuous Deployment} tramite GitHub Actions.
Il \glslink{workflow}{workflow} principale, nominato \texttt{Site Deploy}, regola il ciclo di vita della pubblicazione del sito web e si attiva in automatico ad ogni evento di push o \glslink{pull-request}{pull request} verso il \glslink{branch-ramo}{branch} \texttt{main} (ignorando i file non critici come \texttt{README.md} e \texttt{.gitignore}).
...
A differenza dell'utilizzo di identificativi numerici, il gruppo \textit{aLittleByte} adotta una convenzione di nomenclatura descrittiva per i \glslink{branch-ramo}{branch}, in modo da rendere immediatamente esplicito l'oggetto del lavoro.
...
L'alberatura della \glslink{repository-documentale}{repository documentale} è strutturata in modo logico e gerarchico per separare le fasi del progetto didattico. Le directory principali includono:
\begin{itemize}
...
Le attività di verifica hanno lo scopo di garantire l'assenza di difetti e la totale aderenza di ogni componente prodotta (sia essa documentale o codice sorgente) rispetto alle specifiche e \glslink{ai-intelligenza-artificiale}{ai} vincoli stabiliti. Applicando i principi dell'Ingegneria del Software, questa pratica metodologica mira a soddisfare la seguente domanda: \textit{''Did I build the system right?''}.
...
In questa prima parte del lavoro, il gruppo \textit{aLittleByte} si è dedicato principalmente alla verifica della documentazione. Tramite revisioni incrociate tra i membri, \glslink{integrazione-continua-continuous-integration-ci}{ci} siamo assicurati che i contenuti fossero corretti e che i testi rispettassero le regole grammaticali, sintattiche e di formattazione che \glslink{integrazione-continua-continuous-integration-ci}{ci} siamo dati nelle presenti Norme di Progetto.
...
In affiancamento alle tecniche manuali, il team adotta \textbf{Larastan} (PHPStan per Laravel) come strumento di \glslink{analisi-statica}{analisi statica} automatica del codice. Larastan viene eseguito al livello di analisi \textbf{5} e si occupa di rilevare automaticamente i \textit{code smell} tracciati dalla metrica \textbf{MPD12} (es. metodi troppo complessi, variabili non tipizzate, chiamate a metodi inesistenti). L'esecuzione avviene in locale prima di ogni \textit{commit} e nella pipeline di \textit{\glslink{integrazione-continua-continuous-integration-ci}{Continuous Integration}}: il codice che non supera l'analisi non può essere integrato nel \glslink{branch-ramo}{branch} principale.
...
Il framework adottato è \textbf{\glslink{pest}{Pest} PHP}, incluso nativamente in Laravel 12. I test risiedono nella directory \texttt{tests/Unit/} del progetto.
...
\begin{enumerate}
    ...
    \item Calcolare la \textit{\glslink{code-coverage}{code coverage}} con \texttt{./vendor/bin/\glslink{pest}{pest} -{}-coverage} (richiede il driver \textbf{Xdebug} attivo nel container PHP-FPM). La copertura minima richiesta per ogni unità è dell'\textbf{80\%}, in conformità con la metrica \textbf{MPC-CC}.
    \item Allegare l'esito del report nella descrizione della \textit{\glslink{pull-request}{Pull Request}}.
\end{enumerate}
La pipeline di \glslink{integrazione-continua-continuous-integration-ci}{CI} verifica automaticamente questi requisiti a ogni push; una PR con copertura inferiore alla soglia o con test falliti non può essere approvata.
...
Non richiedono la scrittura di test ad hoc: sono garantiti dall'esecuzione automatica dell'intera suite \glslink{pest}{Pest} PHP (unità + integrazione) nella pipeline di \glslink{integrazione-continua-continuous-integration-ci}{CI} ad ogni \textit{push} o apertura di \textit{\glslink{pull-request}{Pull Request}}. Un test precedentemente verde che diventa rosso segnala una regressione e blocca il merge fino alla risoluzione.
...
Vengono eseguiti sull'applicazione deployata nell'ambiente Docker Compose completo (Nginx, PHP-FPM, \glslink{postgresql}{PostgreSQL}, Redis, \glslink{minio}{MinIO}), simulando scenari end-to-end che attraversano tutti i livelli dello stack. Ogni test di sistema è tracciato a un \glslink{requisito-funzionale}{requisito funzionale} dell'Analisi dei Requisiti tramite il codice \glslink{caso-d-uso-uc}{UC} corrispondente. I risultati e lo stato di copertura dei requisiti sono riportati nel \textit{Piano di Qualifica}.

\subsection{Validazione}
La validazione rappresenta il processo finale che conferma in modo definitivo che le caratteristiche del prodotto software siano pienamente conformi \glslink{ai-intelligenza-artificiale}{ai} bisogni dell'utilizzo finale e agli obiettivi d'uso previsti. L'obiettivo principale è rispondere al quesito: \textit{''Did I build the right system?''} 

Mentre la verifica analizza la correttezza tecnica dell'esecuzione, la validazione si sposta sul piano dell'efficacia, garantendo che le funzionalità sviluppate siano effettivamente idonee a risolvere le problematiche operative individuate e siano allineate con le aspettative del \glslink{committente-o-proponente}{committente}.

\paragraph{Processo di Validazione}
Il gruppo \textit{aLittleByte} ha provveduto a raccogliere, analizzare e formalizzare tutte le necessità espresse dall'azienda \glslink{committente-o-proponente}{proponente} \textit{Eggon} all'interno del documento di \textit{Analisi dei Requisiti}. Questa operazione di classificazione è fondamentale per garantire un tracciamento rigoroso delle specifiche e per predisporre i futuri test di accettazione, i quali avranno il compito di accertare che il prodotto software sviluppato rispecchi le richieste del \glslink{committente-o-proponente}{committente}.

Gli esiti derivanti da queste procedure di validazione saranno riportati e discussi in modo esaustivo nel \textit{Piano di Qualifica}, all'interno della specifica sezione dedicata \glslink{ai-intelligenza-artificiale}{ai} Test di Accettazione.
...
\begin{itemize}
    ...
    \item \textbf{Organizzazione del team:} Assegnazione trasparente di ruoli e responsabilità \glslink{ai-intelligenza-artificiale}{ai} membri del gruppo, garantendo al contempo il mantenimento di canali di comunicazione efficaci.
\end{itemize}
...
\begin{itemize}
    \item \textbf{\glslink{pianificazione}{Pianificazione}:} Il team seleziona gli obiettivi operativi da raggiungere, scompone il lavoro in \textit{task} gestibili e li assegna \glslink{ai-intelligenza-artificiale}{ai} vari componenti, cercando di bilanciare equamente i carichi di lavoro.
    
    \item \textbf{\glslink{sprint}{Sprint} Review:} Si analizza lo \glslink{sprint}{sprint} appena concluso per la verifica concreta dei risultati prodotti. Costituisce l'occasione per valutare il lavoro completato, discutere le criticità tecniche emerse e misurare l'avanzamento effettivo rispetto al piano generale.
...
Per garantire un controllo visivo e immediato sullo stato di avanzamento delle singole \glslink{issue}{Issue} associate al progetto, il gruppo gestisce i task tramite la board GitHub Projects ''aLittleByte - \glslink{dashboard}{Dashboard}''.
...
\begin{itemize}
    ...
    \item \textbf{In Progress:} Attività su cui il team sta lavorando attivamente (le \glslink{issue}{issue} sono formalmente assegnate \glslink{ai-intelligenza-artificiale}{ai} membri tramite il campo \textit{Assignee}).
    ...
    \item \textbf{Done:} Lavoro verificato, approvato e unito definitivamente al \glslink{branch-ramo}{branch} principale.
\end{itemize}

\subsubsection{Ruoli}

\paragraph{Responsabile}
Il membro che assume il ruolo di \glslink{responsabile}{Responsabile} agisce come vero e proprio \textit{Project Manager} del gruppo. Ha la responsabilità di coordinare il lavoro del team, pianificare le iterazioni di sviluppo, ottimizzare l'uso delle risorse e fungere da referente principale per le comunicazioni ufficiali con l'azienda \glslink{committente-o-proponente}{proponente} e il corpo docente.

Dal punto di vista strettamente operativo, il \glslink{responsabile}{Responsabile} si fa carico di distribuire e assegnare i compiti (tramite le \textit{\glslink{issue}{Issue}}) \glslink{ai-intelligenza-artificiale}{ai} vari componenti del gruppo. Inoltre, è sua competenza diretta la stesura e il costante aggiornamento dei seguenti documenti:
...
Oltre a monitorare le scadenze amministrative e a fornire supporto costante \glslink{ai-intelligenza-artificiale}{ai} colleghi, l'\glslink{amministratore}{Amministratore} si fa carico delle seguenti responsabilità operative:
...
\begin{itemize}
    ...
    \item \textbf{Norme di Progetto:} Curare la stesura, la revisione e il continuo aggiornamento del presente documento (\textit{Norme di Progetto}), assicurandosi che le regole interne e il \textit{\glslink{way-of-working-wow}{Way of Working}} siano sempre allineati alle reali necessità del gruppo.
\end{itemize}

\paragraph{Progettista}

Il \glslink{progettista}{Progettista} è la mente tecnica dietro alla struttura del prodotto software. Il suo compito principale è ideare l'architettura del sistema, assicurandosi che sia in grado di soddisfare tutti i requisiti (sia funzionali che di qualità) concordati con il \glslink{committente-o-proponente}{proponente}. Lavora a stretto contatto con il resto del gruppo per trasformare le richieste astratte in soluzioni pratiche, efficienti e facili da mantenere nel tempo.
...
\begin{itemize}
    ...
    \item \textbf{Gestione dei dubbi e delle modifiche:} Interfacciarsi direttamente con il \glslink{committente-o-proponente}{proponente} per risolvere eventuali incertezze, chiedere chiarimenti o concordare variazioni \glslink{ai-intelligenza-artificiale}{ai} requisiti iniziali.
...
Al termine di ogni riunione, il membro designato redige il verbale seguendo la struttura definita nella sezione~\ref{sec:documentazione}. Ogni compito emerso viene immediatamente trasformato in una \textit{\glslink{issue}{Issue}} su GitHub e assegnato al membro o \glslink{ai-intelligenza-artificiale}{ai} membri incaricati. Qualora il componente assegnato riscontri difficoltà, può utilizzare la sezione commenti della \textit{\glslink{issue}{Issue}} per chiedere chiarimenti al resto del team.
...
\begin{itemize}
    ...
    \item \textbf{Discussioni GitHub:} Viene utilizzato per discussioni all'interno delle \textit{\glslink{issue}{Issue}} e delle \textit{\glslink{pull-request}{Pull Request}}, per garantire il tracciamento delle decisioni. Sono utili in quanto permettono di comunicare solo con i membri del gruppo relativi alla discussione. 
\end{itemize}

\paragraph{Comunicazioni Esterne}

Per interagire con l'azienda \glslink{committente-o-proponente}{proponente} e con il corpo docente, il gruppo ha definito dei canali di comunicazione specifici, differenziati in base al livello di formalità e all'urgenza della richiesta:
...
\begin{itemize}
    ...
    \item \textbf{Telegram:} Impiegato per scambi di messaggi rapidi, chiarimenti immediati e per il coordinamento logistico direttamente con il referente dell'azienda \glslink{committente-o-proponente}{proponente}.
\end{itemize}
...
I colloqui con l'azienda \glslink{committente-o-proponente}{proponente} sono essenziali per garantire il continuo allineamento sugli obiettivi e raccogliere \textit{feedback} sul lavoro svolto.
...
Lo standard ISO/IEC 12207:1997 viene adattato al contesto di un team accademico che sviluppa un \glslink{mvp-minimum-viable-product}{MVP} stand-alone con moduli \glslink{ai-intelligenza-artificiale}{AI}. Sono mantenuti i processi di \textbf{Fornitura} e \textbf{Sviluppo} (adattati con Agile/SCRUM), mentre Acquisizione, Operazione e Manutenzione sono esclusi. La documentazione è ridotta \glslink{ai-intelligenza-artificiale}{ai} soli artefatti richiesti dalla baseline; la gestione della configurazione è delegata a GitHub. L'efficacia del tailoring è monitorata tramite la metrica \textbf{\glslink{quality-metrics-satisfied-mpc-qms}{MPC-QMS}}: un valore sotto soglia segnala la necessità di rivedere le scelte nell'iterazione successiva.
...
\begin{itemize}
    \item \textbf{Descrizione:} Valore economico del lavoro pianificato per lo \glslink{sprint}{sprint}, estratto dai preventivi del Piano di Progetto. Rappresenta il costo previsto per le attività programmate.
    \item \textbf{Formula:}
...
\begin{itemize}
    \item \textbf{Descrizione:} Costo effettivamente sostenuto per le ore lavorate nello \glslink{sprint}{sprint}, ricavato dai consuntivi del Piano di Progetto.
    \item \textbf{Formula:}
...
\begin{itemize}
    \item \textbf{Descrizione:} Percentuale di metriche di qualità (di processo e di prodotto) che rientrano nel range accettabile definito nel Piano di Qualifica. Rappresenta un indicatore sintetico della salute complessiva del progetto al termine di ciascuno \glslink{sprint}{sprint}.
    \item \textbf{Formula:}
...
\begin{itemize}
    \item \textbf{Descrizione:} Percentuale di rischi effettivamente manifestatisi che non erano stati identificati nel registro dei rischi del Piano di Progetto. Valori bassi indicano una \glslink{pianificazione}{pianificazione} del rischio efficace e un registro dei rischi completo.
    \item \textbf{Formula:}
...
\begin{itemize}
    \item \textbf{Descrizione:} Percentuale di generazioni \glslink{ai-intelligenza-artificiale}{AI} valutate dagli utenti con un \glslink{rating}{rating} pari o superiore a 3 su 5 rispetto al totale delle generazioni valutate. Misura la qualità percepita delle risposte prodotte dai moduli \glslink{ai-intelligenza-artificiale}{AI} Assistant e \glslink{ai-co-pilot}{AI Co-Pilot} del sistema.
    \item \textbf{Formula:}
    $$\text{\glslink{ai-acceptance-rate-mpd03}{MPD03}} = \frac{\text{Generazioni con \glslink{rating}{rating}} \geq 3/5}{\text{Generazioni totali valutate}} \times 100$$
    \item \textbf{Unità di misura:} percentuale (\%)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Formula:}
    $$\text{\glslink{branch-coverage-mpd04}{MPD04}} = \frac{\text{Rami Decisionali Eseguiti}}{\text{Rami Decisionali Totali}} \times 100$$
    \item \textbf{Unità di misura:} percentuale (\%)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Descrizione:} Tempo di risposta del sistema a un'interazione dell'utente sull'interfaccia, misurato come intervallo tra l'invio della richiesta e la ricezione della risposta completa per le operazioni generali dell'applicazione (escluse le chiamate \glslink{ai-intelligenza-artificiale}{ai} moduli \glslink{ai-intelligenza-artificiale}{AI}).
    \item \textbf{Formula:}
...
\begin{itemize}
    \item \textbf{Descrizione:} Tempo intercorso tra l'invio del \glslink{prompt}{prompt} al modulo \glslink{ai-post-generator}{AI Post Generator} e la ricezione del testo della \glslink{bozza-draft}{bozza} completa generata dal servizio \glslink{ai-intelligenza-artificiale}{AI}.
    \item \textbf{Formula:}
    $$\text{MPD07} = T_{\text{risposta \glslink{ai-intelligenza-artificiale}{AI}}} - T_{\text{invio \glslink{prompt}{prompt}}}$$
    \item \textbf{Unità di misura:} secondi (s)
\end{itemize}

\paragraph{MPD08 --- Latenza Generazione Immagine AI Assistant}
\begin{itemize}
    \item \textbf{Descrizione:} Tempo intercorso tra la richiesta di generazione dell'immagine di copertina al modulo \glslink{ai-intelligenza-artificiale}{AI} Assistant e la ricezione dell'immagine generata e pronta per la visualizzazione.
    \item \textbf{Formula:}
...
\begin{itemize}
    ...
    \item \textbf{Formula:}
    $$\text{\glslink{learning-time-mpd11}{MPD11}} = \frac{\displaystyle\sum_{i=1}^{n} T_{\text{apprendimento}_i}}{n}$$
...
\begin{itemize}
    ...
    \item \textbf{Formula:}
    $$\text{\glslink{coefficient-of-coupling-mpd13}{MPD13}} = \frac{\text{Dipendenze Effettive tra Moduli}}{n \times (n - 1)}$$
    ...
    \item \textbf{Interpretazione:}
    \begin{itemize}
        \item \glslink{coefficient-of-coupling-mpd13}{MPD13} $= 0$: nessun accoppiamento (moduli completamente indipendenti)
        \item \glslink{coefficient-of-coupling-mpd13}{MPD13} $= 1$: accoppiamento massimo (ogni modulo dipende da tutti gli altri)
    \end{itemize}
