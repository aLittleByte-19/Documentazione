...
Il prodotto da realizzare consiste in una piattaforma software \textit{stand-alone} sviluppata su richiesta dell'azienda proponente \textit{Eggon}, che si affianca a \textit{Nexum} — piattaforma digitale dedicata alla comunicazione interna e alla gestione HR — comunicando con essa tramite API. Il sistema è architetturalmente indipendente da Nexum e comprende due moduli basati sull'Intelligenza Artificiale:
\begin{itemize}
...
I processi di fornitura comprendono tutte le attività preliminari che permettono di avviare lo sviluppo del progetto, in particolar modo la pianificazione: suddivisione del lavoro in fasi e milestone come \textit{RTB} e \textit{PB}, l’analisi: come vengono analizzati i capitolati, la stima: come si stimano vari costi e stime.
...
\begin{itemize}
    \item \textbf{GitHub:} Per il versionamento della documentazione tramite \textit{Pull Request} e per l'integrazione con il codice preesistente.
...
\begin{itemize}
    \item \textbf{Laravel Pint:} Tool ufficiale di formattazione per Laravel, basato su PHP CS Fixer. Applica automaticamente PSR-12 al codebase. Deve essere eseguito prima di ogni commit; il codice non conforme non può essere integrato nel branch principale.
    \item \textbf{Larastan:} Estensione di PHPStan ottimizzata per Laravel. Esegue analisi statica del codice per rilevare errori di tipo, chiamate a metodi inesistenti e potenziali bug a runtime, senza eseguire il codice. Il livello minimo di analisi adottato è il \textbf{livello 5}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{To-Do List:} Sezione conclusiva dedicata al tracciamento delle azioni da intraprendere a seguito della riunione. Per ogni task viene indicato il responsabile e viene inserito il collegamento ipertestuale diretto alla rispettiva \textit{Issue} GitHub, garantendo tracciabilità completa tra decisione e attività operativa.
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    Mentre \texttt{<Data>} è la data della riunione scritta nel formato \texttt{AAAA\_MM\_GG}. Ad esempio, \texttt{Verbale\_Interno\_2026\_04\_13.pdf} rappresenta il verbale interno del giorno 13/04/2026.
    \\
    
    \item \textbf{Diari di Bordo:} Per i Diari di Bordo viene adottato il seguente standard di denominazione: \texttt{Diario\_di\_Bordo\_AAAA\_MM\_GG.pdf}. Ad esempio, \texttt{Diario\_di\_Bordo\_2026\_04\_13.pdf} rappresenta il diario di bordo presentato il giorno 13/04/2026.
...
In affiancamento alle tecniche manuali, il team adotta \textbf{Larastan} (PHPStan per Laravel) come strumento di analisi statica automatica del codice. Larastan viene eseguito al livello di analisi \textbf{5} e si occupa di rilevare automaticamente i \textit{code smell} tracciati dalla metrica \textbf{MPD12} (es. metodi troppo complessi, variabili non tipizzate, chiamate a metodi inesistenti). L'esecuzione avviene in locale prima di ogni \textit{commit} e nella pipeline di \textit{Continuous Integration}: il codice che non supera l'analisi non può essere integrato nel branch principale.
...
Il framework adottato è \textbf{Pest PHP}, incluso nativamente in Laravel 12. I test risiedono nella directory \texttt{tests/Unit/} del progetto.

Il programmatore è tenuto a seguire questa procedura prima di aprire una \textit{Pull Request}:
\begin{enumerate}
    \item Scrivere i test di unità che coprono la logica del codice prodotto.
    ...
    \item Allegare l'esito del report nella descrizione della \textit{Pull Request}.
\end{enumerate}
...
Non richiedono la scrittura di test ad hoc: sono garantiti dall'esecuzione automatica dell'intera suite Pest PHP (unità + integrazione) nella pipeline di CI ad ogni \textit{push} o apertura di \textit{Pull Request}. Un test precedentemente verde che diventa rosso segnala una regressione e blocca il merge fino alla risoluzione.
...
I test di integrazione sono scritti con \textbf{Pest PHP} e risiedono in \texttt{tests/Feature/}. A differenza dei test di unità, questi test eseguono richieste HTTP reali contro l'applicazione Laravel utilizzando il database PostgreSQL di test (definito in \texttt{.env.testing}), verificando l'interazione tra controller, service layer e database. Anche per questi test vale il requisito di copertura minima dell'\textbf{80\%} e l'obbligo di esecuzione prima di aprire una \textit{Pull Request}.
...
Vengono eseguiti sull'applicazione deployata nell'ambiente Docker Compose completo (Nginx, PHP-FPM, PostgreSQL, Redis, MinIO), simulando scenari end-to-end che attraversano tutti i livelli dello stack. Ogni test di sistema è tracciato a un requisito funzionale dell'Analisi dei Requisiti tramite il codice UC corrispondente. I risultati e lo stato di copertura dei requisiti sono riportati nel \textit{Piano di Qualifica}.
...
Ogni membro del gruppo aggiorna il proprio registro orario \textbf{entro la fine di ogni settimana}. Il Responsabile di turno effettua una revisione complessiva del bilancio orario al \textbf{termine di ogni Sprint}, verificando che i dati siano completi e coerenti con le attività svolte.

\paragraph{Catalogazione delle ore}
Le ore sono catalogate secondo due dimensioni:
\begin{itemize}
    \item \textbf{Per ruolo:} Responsabile, Amministratore, Analista, Progettista, Programmatore, Verificatore.
...
Il Responsabile ha il compito di controllare, a fine Sprint, che nessun membro abbia esaurito prematuramente le ore assegnate a un determinato ruolo. In caso di criticità, provvede a ridistribuire i task nelle iterazioni successive.

\subsubsection{Tracciamento delle Azioni}

Al termine di ogni riunione, il membro designato redige il verbale seguendo la struttura definita nella sezione~\ref{sec:documentazione}. Ogni compito emerso viene immediatamente trasformato in una \textit{Issue} su GitHub e assegnato al membro o ai membri incaricati. Qualora il componente assegnato riscontri difficoltà, può utilizzare la sezione commenti della \textit{Issue} per chiedere chiarimenti al resto del team.
...
Lo standard di riferimento per la qualità del processo è l'ISO/IEC 12207. Questo standard fornisce le linee guida per la gestione della qualità nei processi di sviluppo, includendo la pianificazione, il controllo e il miglioramento continuo.
...
Lo standard ISO/IEC 12207:1997 viene adattato al contesto di un team accademico che sviluppa un MVP stand-alone con moduli AI. Sono mantenuti i processi di \textbf{Fornitura} e \textbf{Sviluppo} (adattati con Agile/SCRUM), mentre Acquisizione, Operazione e Manutenzione sono esclusi. La documentazione è ridotta ai soli artefatti richiesti dalla baseline; la gestione della configurazione è delegata a GitHub. L'efficacia del tailoring è monitorata tramite la metrica \textbf{MPC-QMS}: un valore sotto soglia segnala la necessità di rivedere le scelte nell'iterazione successiva.
