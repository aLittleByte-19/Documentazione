...
{\footnotesize
    ...
    \begin{longtable}{|p{1.1cm}|p{2.0cm}|p{4.3cm}|p{2.8cm}|p{2.0cm}|p{2.0cm}|}
        ...
        \hline
        1.1.0 & 19/07/2026 & Aggiunta delle procedure operative ai processi definiti. & Eleonora Bellet & Leonardo Soligo & - \\
        \hline
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Data Ultimo accesso: 17/06/2026} 
    \end{itemize}
...
\begin{itemize}
    \item \textbf{\glslink{glossario}{Glossario} v2.0.0:} Documento interno del progetto contenente la definizione dei termini tecnici, di dominio e degli acronimi utilizzati nella documentazione.
    \begin{itemize}
...
I processi di fornitura comprendono tutte le attività preliminari che permettono di avviare lo sviluppo del progetto, in particolar modo la \glslink{pianificazione}{pianificazione}: suddivisione del lavoro in fasi e \glslink{milestone}{milestone} come \textit{\glslink{rtb-requirements-and-technology-baseline}{RTB}} e \textit{\glslink{pb-product-baseline}{PB}}, l’analisi: come vengono analizzati i capitolati, la stima: come si stimano vari costi e stime.

\paragraph{Scopo e obiettivi}
Il processo di fornitura definisce le procedure e le attività che il gruppo mette in atto per stabilire un accordo operativo con la \glslink{committente-proponente}{proponente} (\textit{Eggon}). L'obiettivo è trasformare le necessità espresse nel \glslink{capitolato}{capitolato} C5 in un insieme di requisiti tecnici e vincoli contrattuali ben definiti. In questa fase si stabiliscono le basi metodologiche del \textit{"\glslink{way-of-working-wow}{Way of Working}''} del team, garantendo che ogni attività sia pianificata, tracciabile e verificabile.

\paragraph{Attività Operative}
Il team si impegna nelle seguenti attività chiave:
\begin{itemize}
    \item \textbf{Analisi e Valutazione del \glslink{capitolato}{Capitolato}:} Studio critico della proposta C5 per identificare rischi tecnologici, opportunità di apprendimento e coerenza con le competenze del team.
    \item \textbf{Stima e \glslink{pianificazione}{Pianificazione}:} Definizione del budget (12.575€) e delle scadenze temporali (consegna finale entro il 10 settembre 2026), con la suddivisione del lavoro in cicli iterativi (\glslink{sprint}{Sprint}).
    \item \textbf{Negoziazione e Definizione dell’\glslink{mvp-minimum-viable-product}{MVP}:} Collaborazione con \textit{Eggon} per identificare il \textit{\glslink{mvp-minimum-viable-product}{Minimum Viable Product}}, garantendo che le funzionalità core (\glslink{intelligenza-artificiale-ai}{AI} Assistant e Co-Pilot) siano prioritizzate correttamente.
...
Il processo di fornitura viene eseguito secondo i seguenti passaggi:

\begin{enumerate}
    \item Analisi delle richieste formulate dal \glslink{committente-proponente}{proponente}.
    \item Definizione degli obiettivi della \glslink{milestone}{milestone} corrente.
    \item \glslink{pianificazione}{Pianificazione} delle attività da svolgere.
    \item Produzione dei deliverable previsti.
    \item Verifica dei deliverable prodotti.
    \item Validazione interna dei risultati ottenuti.
    \item Consegna del materiale al \glslink{committente-proponente}{committente} o ai docenti.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti}:} Documento cardine che formalizza le funzionalità del sistema. Include la modellazione degli attori (\glslink{redattore}{Redattore}, \glslink{analista-dati}{Analista dati}, \glslink{consulente-del-lavoro-cdl}{Consulente del lavoro}), la descrizione dei casi d'uso (\glslink{caso-d-uso-uc}{UC}) e la classificazione dei \glslink{requisiti-funzionali}{requisiti funzionali}, qualitativi e di vincolo.
    \item \textbf{\glslink{norme-di-progetto}{Norme di Progetto}:} Definiscono il regolamento interno del gruppo. Contengono gli standard di codifica, le procedure di gestione dei ticket su \glslink{github}{GitHub}, le convenzioni di naming e le metodologie di lavoro adottate.
    \item \textbf{\glslink{piano-di-progetto}{Piano di Progetto}:} Strumento di gestione manageriale che descrive l'allocazione delle risorse, l'\glslink{analisi-dei-rischi}{analisi dei rischi} e il \glslink{preventivo}{preventivo} economico. Viene aggiornato periodicamente con il \glslink{consuntivo}{consuntivo} delle ore per monitorare lo stato del budget.
    \item \textbf{\glslink{piano-di-qualifica}{Piano di Qualifica}:} Specifica la strategia per garantire che il software sia privo di difetti critici. Definisce le metriche (es. \textit{Code Coverage} $\geq$ 80\%) e i test (unità, integrazione, sistema) necessari alla validazione.
    \item \textbf{\glslink{glossario}{Glossario}:} Raccolta di termini tecnici e acronimi per eliminare ambiguità terminologiche tra sviluppatori, committenti e utenti finali.
    \item \textbf{Specifica Tecnica:} Documento di design che descrive l'architettura di sistema (\glslink{modello-c4}{modello C4}), i diagrammi delle classi UML e i design pattern scelti per l'implementazione.
    \item \textbf{Manuale Utente:}  Guida operativa destinata agli utenti finali per facilitare l'onboarding sulla piattaforma \glslink{nexum}{Nexum} e spiegare l'utilizzo degli assistenti \glslink{intelligenza-artificiale-ai}{AI}.
    \item \textbf{Lettera di Candidatura e Presentazione:}  Documenti formali per l'ufficializzazione della partecipazione all'appalto e per la consegna dei vari rilasci (\glslink{rtb-requirements-and-technology-baseline}{RTB} e \glslink{pb-product-baseline}{PB}).
...
\begin{itemize}
    \item \textbf{\glslink{github}{GitHub}:} Per il versionamento della documentazione tramite \textit{\glslink{pull-request}{Pull Request}} e per l'integrazione con il codice preesistente.
    ...
    \item \textbf{Tracciamento:} Ogni decisione rilevante viene convertita in una \textit{\glslink{issue}{issue}} su \glslink{github}{GitHub} per garantirne l'assegnazione e il completamento.
\end{itemize}

\subsubsection{Processi di Sviluppo}
Questa sezione codifica l'insieme delle pratiche ingegneristiche e operative indispensabili per costruire materialmente la piattaforma software stand-alone richiesta dal \glslink{capitolato}{capitolato}. L'obiettivo è tradurre i requisiti concordati in un applicativo funzionante e architetturalmente indipendente dal sistema originario (\textit{\glslink{nexum}{Nexum}}), passando attraverso fasi rigorose di design, programmazione e collaudo.

\paragraph{Attività Operative e Ciclo di Vita}
Il ciclo di sviluppo per l'integrazione dei modelli di \glslink{intelligenza-artificiale-ai}{Intelligenza Artificiale} richiede una sequenza logica di operazioni ingegneristiche specifiche:
\begin{itemize}
    ...
    \item \textbf{Progettazione Architetturale:} Definizione di un'infrastruttura \textit{stand-alone} che comunichi in modo asincrono con i \glslink{moduli-esterni}{moduli esterni} di \glslink{intelligenza-artificiale-ai}{Intelligenza Artificiale} (\glslink{ai-post-generator}{AI Post Generator} e \glslink{ai-analyst}{AI Analyst}), per non bloccare l'interfaccia utente durante le operazioni \textit{time-consuming}.
    \item \textbf{Sviluppo Modulare:} L'applicativo viene frammentato in macro-componenti indipendenti (es. modulo di \glslink{rating}{rating}/feedback, modulo di split documentale, \glslink{dashboard}{dashboard} analitica).
    \item \textbf{Validazione "\glslink{human-in-the-loop}{Human-in-the-loop}":} Implementazione di flussi di sviluppo che prevedano sempre un passaggio di validazione umana per le operazioni \glslink{intelligenza-artificiale-ai}{AI} a bassa \glslink{confidenza-confidence-score}{confidenza} (es. revisione dei destinatari estratti o modifica del testo generato).
    \item \textbf{Collaudo e Rilascio:} Esecuzione di test formali sulle metriche di qualità (es. tempi di risposta, tasso di successo dell'\glslink{intelligenza-artificiale-ai}{AI}) prima dell'approvazione finale.
\end{itemize}

\paragraph{Procedura Operativa}
Il processo di sviluppo segue il seguente ciclo di vita:

\begin{enumerate}
    \item \glslink{analisi-dei-requisiti-adr}{Analisi dei requisiti}.
    \item Definizione della soluzione architetturale.
    \item \glslink{pianificazione}{Pianificazione} delle attività di sviluppo.
    \item Implementazione delle funzionalità previste.
    \item Esecuzione delle attività di verifica.
    \item Correzione delle eventuali anomalie rilevate.
    \item Validazione del prodotto sviluppato.
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{Utenti Operativi (Primari):} Utenti umani che innescano le operazioni (\glslink{amministratore}{Amministratore}, \glslink{redattore}{Redattore}, \glslink{analista-dati}{Analista dati}, \glslink{consulente-del-lavoro-cdl}{Consulente del lavoro}).
        \item \textit{\glslink{moduli-esterni}{Moduli Esterni} (Secondari):} Entità di sistema chiamate in causa per completare l'operazione (\glslink{ai-post-generator}{AI Post Generator}, \glslink{ai-analyst}{AI Analyst}).
    \end{itemize}
    ...
    \begin{itemize}
        \item UC-[Numero] identifica i casi d'uso.
        \item {} [Caso Padre].[\glslink{sottocaso}{Sottocaso}] indica le scomposizioni logiche (es. \textit{UC-1.1} per un \glslink{sottocaso}{sottocaso} di \textit{UC-1}).
    \end{itemize}
    \item \textbf{Struttura Descrittiva:} Ogni \glslink{caso-d-uso-uc}{caso d'uso} documentato deve presentare una scheda strutturata contenente: Attori coinvolti, Trigger, Pre e Post-condizioni, \glslink{scenario-principale}{Scenario Principale} (sequenza ottimale numerata), Scenario Alternativo.
\end{itemize}

\paragraph{Progettazione Architetturale}
La progettazione traduce le specifiche in un'infrastruttura flessibile e scalabile.
\begin{itemize}
    \item \textbf{Disaccoppiamento e Asincronia:} Data la natura \textit{data-driven} e i tempi di \glslink{latenza}{latenza} dell'\glslink{intelligenza-artificiale-ai}{AI}, l'architettura deve garantire l'\glslink{esecuzione-asincrona}{esecuzione asincrona} delle operazioni pesanti (es. elaborazione \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR} o \glslink{entity-resolution}{entity resolution}).
...
\begin{itemize}
    \item \textbf{Modularità e Riuso:} Suddivisione del codice in moduli con responsabilità singola (es. libreria dei \glslink{prompt}{prompt} separata dal motore di generazione). Applicazione di pattern architetturali per evitare dipendenze dirette e favorire la manutenibilità.
    \item \textbf{Gestione degli Errori e Resilienza:} Implementazione di meccanismi robusti per la gestione di anomalie, retry di rete e fallback nel caso in cui i servizi \glslink{intelligenza-artificiale-ai}{AI} esterni non siano temporaneamente disponibili (come mappato negli scenari alternativi dei casi d'uso).
    \item \textbf{Tracciabilità e Qualità:} Versionamento continuo del codice, revisione tra pari (\textit{\glslink{code-review}{Code Review}}) e test automatizzati per validare non solo le logiche applicative, ma anche l'integrazione corretta dei parametri (tono, stile, \glslink{metadati}{metadati}) passati ai core di \glslink{intelligenza-artificiale-ai}{intelligenza artificiale}.
\end{itemize}

\subparagraph{Standard Stilistici}
Il codice PHP prodotto deve essere conforme allo standard \textbf{PSR-12} (\textit{PHP Standards Recommendations 12}), adottato ufficialmente dalla comunità PHP e seguito nativamente da \glslink{laravel}{Laravel} 12. PSR-12 definisce regole precise su indentazione (4 spazi), apertura delle parentesi, dichiarazione dei tipi e struttura delle classi, garantendo uniformità in tutto il codebase indipendentemente dal membro che scrive il codice.

\subparagraph{Convenzioni di Naming nel Codice}
...
\begin{itemize}
    ...
    \item \textbf{File Blade:} kebab-case — es. \texttt{invoice-list.blade.php}
    \item \textbf{Variabili d'ambiente:} UPPER\_SNAKE\_CASE — es. \texttt{MINIO\_BUCKET}, \texttt{DB\_CONNECTION}
\end{itemize}

\subparagraph{Strumenti di Linting e Formattazione}
\begin{itemize}
    \item \textbf{\glslink{laravel}{Laravel} Pint:} Tool ufficiale di formattazione per \glslink{laravel}{Laravel}, basato su PHP CS Fixer. Applica automaticamente PSR-12 al codebase. Deve essere eseguito prima di ogni commit; il codice non conforme non può essere integrato nel \glslink{branch}{branch} principale.
    \item \textbf{Larastan:} Estensione di PHPStan ottimizzata per \glslink{laravel}{Laravel}. Esegue \glslink{analisi-statica}{analisi statica} del codice per rilevare errori di tipo, chiamate a metodi inesistenti e potenziali bug a runtime, senza eseguire il codice. Il livello minimo di analisi adottato è il \textbf{livello 5}.
\end{itemize}

\subsection{Processi di supporto}
I processi di supporto comprendono tutte le attività, i metodi e gli strumenti che non sono direttamente legati alla scrittura del codice, ma che risultano indispensabili per sostenere i processi primari di sviluppo e fornitura. Per il nostro team, definire questi processi significa stabilire delle regole di base condivise, necessarie per collaborare in modo efficiente ed evitare disorganizzazione durante le varie fasi del progetto didattico. Ciò ci permette, a livello pratico, di garantire la tracciabilità di ogni decisione tecnica e organizzativa presa dal gruppo e di assicurare che tutto il materiale prodotto (documenti, codice, \glslink{issue}{issue}) sia costantemente coerente, aggiornato e facilmente accessibile a tutti i membri del team e al \glslink{committente-proponente}{committente}.

\subsubsection{Documentazione}

\paragraph{Scopo}
Lo scopo del processo di documentazione è stabilire regole e metodologie pratiche per la creazione, la gestione e l'aggiornamento continuo dei documenti di progetto. Per il nostro gruppo, impegnarci a produrre materiale che si mantenga costantemente chiaro, accessibile e aggiornato ci permette di agevolare e rendere trasparente la comunicazione interna tra i membri del gruppo. Inoltre, una \glslink{repository}{repository} ben strutturata facilita notevolmente il confronto e l'allineamento con gli \glslink{stakeholder}{stakeholder} esterni e l'azienda \textit{Eggon}. Questo approccio rigoroso garantisce un'accurata tracciabilità delle decisioni organizzative e progettuali prese nel corso dello sviluppo, assicurando al contempo che l'intero progetto venga gestito in piena conformità agli standard di qualità che ci siamo prefissati di rispettare.
...
\begin{itemize}
    \item \textbf{\glslink{github}{GitHub}:} Rappresenta la nostra piattaforma principale per l'archiviazione e il tracciamento storico dei file. Sfruttiamo le funzionalità di branching e \glslink{pull-request}{pull request} per garantire che ogni modifica venga revisionata in modo asincrono.
    \item \textbf{\glslink{github}{GitHub} Projects:} Utilizzato come strumento principale di project management. Il team gestisce il flusso di lavoro tramite una board Kanban dedicata (''aLittleByte - \glslink{dashboard}{Dashboard}'') che permette di visualizzare lo stato di tutte le attività.
    \item \textbf{\glslink{github}{GitHub} Actions e \glslink{github}{GitHub} Pages:} Impiegati per l'automazione (Continuous Integration/Continuous Deployment) e per la pubblicazione automatica e trasparente del sito web documentale.
    \item \textbf{\glslink{latex}{LaTeX}:} Scelto per la stesura di tutti i documenti tecnici e formali, garantendo un'impaginazione professionale.
...
I documenti ufficiali previsti per il progetto, i quali saranno costantemente aggiornati e mantenuti sotto controllo di versione all'interno del \glslink{repository}{repository}, sono i seguenti: 
\begin{itemize}
    \item \textbf{\glslink{analisi-dei-requisiti-adr}{Analisi dei requisiti}:} Il documento in cui vengono estrapolati, classificati e descritti tutti i requisiti del sistema (funzionali e non), i casi d'uso individuati e i rispettivi criteri per l'accettazione finale.
    \item \textbf{\glslink{norme-di-progetto}{Norme di progetto}:} Il documento in esame, avente lo scopo di codificare il nostro \glslink{way-of-working-wow}{Way of Working}, illustrando nel dettaglio le metodologie operative, gli strumenti scelti e le regole di collaborazione interne al team.
    \item \textbf{\glslink{piano-di-progetto}{Piano di progetto}:} Il piano strategico che scandisce le tempistiche di sviluppo, regola l'allocazione delle risorse umane ed economiche e pianifica le varie fasi di avanzamento del lavoro.
    \item \textbf{\glslink{piano-di-qualifica}{Piano di qualifica}:} Il documento volto a fissare le direttive per l'assicurazione della qualità. Al suo interno vengono delineate le metriche di valutazione e le procedure di testing, con l'obiettivo specifico di garantire una copertura del codice non inferiore all'80\%.
    \item \textbf{\glslink{glossario}{Glossario}:} Uno strumento di supporto testuale che definisce in modo inequivocabile la terminologia specifica, tecnica e di dominio, oltre agli acronimi impiegati in tutta la documentazione.
    \item \textbf{Lettera di Presentazione:} La documentazione formale di accompagnamento, debitamente firmata, necessaria per certificare e presentare ufficialmente il \glslink{rtb-requirements-and-technology-baseline}{RTB} al \glslink{committente-proponente}{committente}.
    \item \textbf{Verbali:} I Verbali (interni ed esterni) tengono traccia degli incontri e delle decisioni sia tra i membri del gruppo e che tra il gruppo e la \glslink{committente-proponente}{proponente}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Registro delle modifiche:} Tabella posta all'inizio del file per il tracciamento delle versioni. Per ogni aggiornamento indica: numero di versione, data, descrizione della modifica, \glslink{redattore}{redattore} e approvatore/\glslink{verificatore}{verificatore}.
    ...
    \item \textbf{Introduzione:} Contiene lo scopo del documento, \glslink{glossario}{glossario} e riferimenti. 
...
\paragraph{Struttura dei verbali}\label{sec:documentazione}
\begin{itemize}
    \item \textbf{Copertina:} Riporta gli elementi visivi e identificativi del gruppo, il titolo (specificando se \glslink{verbale-interno}{verbale interno} o esterno e la data), i ruoli assunti per la stesura (\glslink{redattore}{redattore} e \glslink{verificatore}{verificatore}) e l'elenco dei destinatari.
    \item \textbf{Indice:} Elenco cliccabile delle sezioni per facilitare la navigazione del documento.
    \item \textbf{Informazioni generali:} Tabella riassuntiva contenente i dettagli logistici dell'incontro: luogo o piattaforma utilizzata (\glslink{discord}{Discord}), orario di inizio e fine, e l'elenco puntuale dei partecipanti.
    ...
    \item \textbf{To-Do List:} Sezione conclusiva dedicata al tracciamento delle azioni da intraprendere a seguito della riunione. Per ogni task viene indicato il \glslink{responsabile}{responsabile} e viene inserito il collegamento ipertestuale diretto alla rispettiva \textit{\glslink{issue}{Issue}} \glslink{github}{GitHub}, garantendo tracciabilità completa tra decisione e attività operativa.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cosa faremo:} La \glslink{pianificazione}{pianificazione} operativa per i giorni successivi, indicando i prossimi \textit{task} da completare.
...
Il gruppo \textit{aLittleByte} ha adottato una rigorosa politica di denominazione univoca per tutta la documentazione. Tale scelta metodologica è volta a preservare l'uniformità del \glslink{repository}{repository} e a ottimizzare le procedure di ricerca e consultazione dei file.

\begin{itemize}
    \item \textbf{Verbali:} I verbali hanno la seguente denominazione: \\
    \\
    \centerline{\texttt{<Tipo>\_<Data>}} \\
    Dove \texttt{<Tipo>} rappresenta:
    \begin{itemize}
        \item \textbf{Verbale\_Interno}
        \item \textbf{Verbale\_Esterno}
    \end{itemize}
    Mentre \texttt{<Data>} è la data della riunione scritta nel formato \texttt{AAAA\_MM\_GG}. Ad esempio, \texttt{Verbale\_Interno\_2026\_04\_13.pdf} rappresenta il \glslink{verbale-interno}{verbale interno} del giorno 13/04/2026.
    \\
    
    \item \textbf{Diari di Bordo:} Per i Diari di Bordo viene adottato il seguente standard di denominazione: \texttt{Diario\_di\_Bordo\_AAAA\_MM\_GG.pdf}. Ad esempio, \texttt{Diario\_di\_Bordo\_2026\_04\_13.pdf} rappresenta il \glslink{diario-di-bordo}{diario di bordo} presentato il giorno 13/04/2026.
    
    \item \textbf{Documenti:} I restanti documenti ufficiali seguono una convenzione di denominazione esplicita basata sul loro contenuto, come ad esempio \textit{\glslink{norme-di-progetto}{Norme di Progetto}.pdf} o \textit{\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti}.pdf}. 
\end{itemize}
...
\begin{itemize}
    \item \textbf{\glslink{pianificazione}{Pianificazione}:} In corrispondenza delle scadenze di progetto, il gruppo stabilisce quali documenti produrre o aggiornare. Per ogni compito vengono designati un \glslink{redattore}{redattore} e un \glslink{verificatore}{verificatore}, scelti nel rispetto dei ruoli previsti dal \textit{\glslink{way-of-working-wow}{Way of Working}} e garantendo l'assenza di conflitti d'interesse.
    
    \item \textbf{Creazione del \glslink{branch}{branch}:} Prima di iniziare la stesura, viene aperto un \textit{\glslink{branch}{branch}} dedicato nel \glslink{repository}{repository} \glslink{github}{GitHub}. Il gruppo \textit{aLittleByte} ha stabilito di creare un ramo separato per ogni singolo file, inclusi i verbali e i diari di bordo, al fine di garantire una gestione granulare delle modifiche e un isolamento totale del lavoro rispetto al ramo principale.
    
    \item \textbf{Redazione:} Il \glslink{redattore}{redattore} incaricato sviluppa il contenuto seguendo i template e le direttive stilistiche definite nelle presenti \glslink{norme-di-progetto}{Norme di Progetto}. La scrittura avviene tramite gli strumenti appropriati (ad esempio \textit{\glslink{latex}{LaTeX}}), assicurando un output professionale e coerente.
    \begin{itemize}
    ...
    \item \textbf{Terminologia tecnica:} demandare le definizioni e le spiegazioni dei concetti tecnici al \textit{\glslink{glossario}{Glossario}}, evitando di ripeterle all'interno del corpo del documento.
    \item \textbf{Organizzazione logica:} separare fisicamente e visivamente (ad esempio con paragrafi distinti o elenchi) le parti puramente descrittive dalle parti valutative, dalle motivazioni o dalle conseguenze.
    \end{itemize}
    
    \item \textbf{Revisione:} Terminata la stesura, il documento viene sottoposto a una verifica asincrona. Il \glslink{verificatore}{verificatore} analizza il file tramite una \textit{\glslink{pull-request}{pull request}} su \glslink{github}{GitHub}, segnalando eventuali correzioni necessarie o suggerimenti direttamente nei commenti della piattaforma.
    
    \item \textbf{Integrazione e Approvazione:} Una volta superata la revisione e ottenuta l'approvazione formale dal \glslink{responsabile}{Responsabile}, il contenuto viene integrato nel \textit{\glslink{branch}{branch} main} del \glslink{repository}{repository}. In questa fase viene consolidata la versione finale nel registro delle modifiche.

\end{itemize}

\paragraph{Procedura Operativa}
La produzione della documentazione avviene mediante i seguenti passaggi:

\begin{enumerate}
    \item Identificazione del documento da produrre o aggiornare.
    \item Assegnazione dell'attività al membro \glslink{responsabile}{responsabile}.
    \item Redazione o modifica del documento.
    \item Verifica della conformità alle \glslink{norme-di-progetto}{Norme di Progetto}.
    \item Approvazione del documento da parte del \glslink{responsabile}{Responsabile}.
    \item Pubblicazione della versione approvata nel \glslink{repository}{repository}.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{\glslink{git}{Git}:} Software per il controllo di versione distribuito che permette di monitorare ogni variazione apportata ai file sorgente e alla documentazione.
    \item \textbf{\glslink{github}{GitHub}:} Servizio di hosting basato su cloud utilizzato per la gestione centralizzata del \glslink{repository}{repository}. Viene impiegato non solo per l'archiviazione, ma anche per la \glslink{pianificazione}{pianificazione} operativa tramite le \textit{\glslink{issue}{Issue}} e per la revisione del lavoro attraverso le \textit{\glslink{pull-request}{Pull Request}}.
\end{itemize}

\paragraph{Controllo della configurazione}

Il controllo della configurazione è l'attività preposta alla gestione, tracciabilità e autorizzazione di qualsiasi proposta di modifica del progetto. Per tracciare in modo controllato tutte le modifiche, che vengono approvate o meno, il gruppo \textit{aLittleByte} sfrutta le funzionalità native di \glslink{github}{GitHub}, strutturando il processo attraverso l'uso rigoroso di \textit{\glslink{issue}{Issue}} e \textit{\glslink{pull-request}{Pull Request}}:

\begin{itemize}
    \item \textbf{\glslink{issue}{Issue}:} Ogni singola attività (che si tratti della stesura di un documento, dello sviluppo di una funzionalità, della correzione di un errore o anche semplicemente contattare l'azienda) deve essere preventivamente tracciata tramite l'apertura di un'\textit{\glslink{issue}{Issue}}. Quest'ultima viene assegnata a uno o più membri incaricati di portarla a termine. Per facilitarne la classificazione, ogni \textit{\glslink{issue}{Issue}} è identificata da una nomenclatura univoca (ad esempio per la stesura dei documenti viene usato il tag \textit{documentazione}).
    
    \item \textbf{\glslink{pull-request}{Pull Request}:} Rappresenta il meccanismo esclusivo per l'integrazione di qualsiasi modifica all'interno del \glslink{repository}{repository}. Al fine di mantenere un elevato standard qualitativo e prevenire errori, il team ha stabilito che nessun componente è autorizzato a effettuare modifiche dirette sul ramo principale (\texttt{main}). Perciò ogni integrazione deve necessariamente passare per una \textit{\glslink{pull-request}{Pull Request}}, la quale richiede obbligatoriamente un processo di revisione incrociata e può essere inserita nel main solo a seguito di almeno una revisione positiva da parte di altri membri del gruppo. 
\end{itemize}

\paragraph{Procedura Operativa}
Il controllo della configurazione viene gestito come segue:

\begin{enumerate}
    \item Apertura di una \glslink{issue}{Issue} associata alla modifica.
    \item Creazione di un \glslink{branch}{branch} dedicato.
    \item Implementazione delle modifiche previste.
    \item Verifica delle modifiche effettuate.
    \item Apertura di una \glslink{pull-request}{Pull Request}.
    \item Revisione della \glslink{pull-request}{Pull Request} da parte del \glslink{verificatore}{Verificatore}.
    \item Approvazione della modifica.
    \item Integrazione nel \glslink{branch}{branch} principale.
\end{enumerate}
...
Per assicurare che la documentazione fornita agli \glslink{stakeholder}{stakeholder} sia costantemente aggiornata e pubblica, il gruppo ha implementato un processo di \textit{Continuous Deployment} tramite \glslink{github}{GitHub} Actions.
Il \glslink{workflow}{workflow} principale, nominato \texttt{Site Deploy}, regola il ciclo di vita della pubblicazione del sito web e si attiva in automatico ad ogni evento di push o \glslink{pull-request}{pull request} verso il \glslink{branch}{branch} \texttt{main} (ignorando i file non critici come \texttt{README.md} e \texttt{.gitignore}).
...
A differenza dell'utilizzo di identificativi numerici, il gruppo \textit{aLittleByte} adotta una convenzione di nomenclatura descrittiva per i \glslink{branch}{branch}, in modo da rendere immediatamente esplicito l'oggetto del lavoro.
...
L'alberatura della \glslink{repository}{repository} documentale è strutturata in modo logico e gerarchico per separare le fasi del progetto didattico. Le directory principali includono:
\begin{itemize}
    \item \texttt{.github}: Contiene i \glslink{workflow}{workflow} di automazione e gli script Python necessari al rilascio.
    \item \texttt{candidatura}: Contiene i documenti relativi alla fase di candidatura.
    \item \texttt{\glslink{rtb-requirements-and-technology-baseline}{RTB}}: Contiene la documentazione in sviluppo per la \textit{\glslink{rtb-requirements-and-technology-baseline}{Requirements and Technology Baseline}}. Al suo interno è prevista un'ulteriore categorizzazione rigida, come ad esempio la separazione dei verbali in \texttt{RTB/Verbali/Verbali Interni} e \texttt{RTB/Verbali/Verbali Esterni}.
\end{itemize}
...
L'intera reportistica riguardante i controlli effettuati, i parametri misurati e gli esiti dei collaudi è raccolta e analizzata all'interno del \textbf{\glslink{piano-di-qualifica}{Piano di Qualifica}}. 
...
In questa prima parte del lavoro, il gruppo \textit{aLittleByte} si è dedicato principalmente alla verifica della documentazione. Tramite revisioni incrociate tra i membri, ci siamo assicurati che i contenuti fossero corretti e che i testi rispettassero le regole grammaticali, sintattiche e di formattazione che ci siamo dati nelle presenti \glslink{norme-di-progetto}{Norme di Progetto}.

Per quanto riguarda invece la verifica del codice sorgente e del prodotto software, ce ne occuperemo nel dettaglio una volta superata la revisione \textit{\glslink{rtb-requirements-and-technology-baseline}{RTB}}. In ogni caso, tutte le decisioni prese, i metodi di controllo utilizzati e i risultati di queste verifiche verranno sempre riportati in modo esaustivo nel documento \textbf{\glslink{piano-di-qualifica}{Piano di Qualifica}}.

\paragraph{Procedura Operativa}
Le attività di verifica vengono eseguite secondo il seguente processo:

\begin{enumerate}
    \item Individuazione dell'artefatto da verificare.
    \item Selezione delle tecniche di verifica applicabili.
    \item Esecuzione delle verifiche previste.
    \item Registrazione delle anomalie riscontrate.
    \item Correzione delle anomalie.
    \item Riesecuzione delle verifiche.
    \item Approvazione dell'artefatto verificato.
\end{enumerate}

\paragraph{Analisi Statica}
L'\glslink{analisi-statica}{analisi statica} è una tecnica di verifica che viene applicata all'oggetto di studio senza richiederne l'esecuzione. Questo approccio ci permette di individuare anomalie e difetti fin dalle primissime fasi del ciclo di vita del software, ben prima che la scrittura del codice sia ultimata.
...
\begin{itemize}
    \item \textbf{\glslink{walkthrough}{Walkthrough}:} Consiste in una revisione estensiva e senza preconcetti. Il \glslink{verificatore}{verificatore} analizza il prodotto passo dopo passo, simulandone mentalmente la logica e verificando che le regole prestabilite siano rispettate. È un approccio molto approfondito ma dispendioso in termini di tempo, e richiede un intervento interamente manuale.
    
    \item \textbf{Inspection:} Si tratta di un controllo mirato e guidato dall'esperienza. Il \glslink{verificatore}{verificatore} sa già quali sono le criticità più comuni e utilizza delle liste di controllo (\textit{checklist}) specifiche per scovarle. Pur essendo meno estesa del \textit{\glslink{walkthrough}{walkthrough}}, l'ispezione è più rapida e permette di standardizzare la ricerca dei difetti.
\end{itemize}

Al termine di ogni controllo, le attività svolte e i problemi riscontrati vengono opportunamente documentati. Gli errori vengono discussi in modo costruttivo tra il \glslink{verificatore}{verificatore} e l'autore del documento (o del codice); tuttavia, l'applicazione materiale delle correzioni rimane sempre di competenza dell'autore originale, al fine di garantire coerenza e responsabilità. Tutto ciò viene documentato all'interno della \textit{\glslink{pull-request}{Pull Request}}, che in caso di errori non viene approvata, e nel relativo documento.

\subparagraph{Analisi Statica Automatica}
In affiancamento alle tecniche manuali, il team adotta \textbf{Larastan} (PHPStan per \glslink{laravel}{Laravel}) come strumento di \glslink{analisi-statica}{analisi statica} automatica del codice. Larastan viene eseguito al livello di analisi \textbf{5} e si occupa di rilevare automaticamente i \textit{code smell} tracciati dalla metrica \textbf{MPD12} (es. metodi troppo complessi, variabili non tipizzate, chiamate a metodi inesistenti). L'esecuzione avviene in locale prima di ogni \textit{commit} e nella pipeline di \textit{Continuous Integration}: il codice che non supera l'analisi non può essere integrato nel \glslink{branch}{branch} principale.

\paragraph{Analisi Dinamica}
A differenza della tecnica statica, l'\glslink{analisi-dinamica}{analisi dinamica} presuppone l'esecuzione dell'oggetto sottoposto a verifica. Tale approccio esamina le reazioni di specifiche porzioni del software applicandovi un numero definito di "casi prova". Questi test devono possedere i requisiti di ripetibilità e automazione.
...
\subparagraph{Ambiente e Procedura Operativa}
Il framework adottato è \textbf{\glslink{pest}{Pest} PHP}, incluso nativamente in \glslink{laravel}{Laravel} 12. I test risiedono nella directory \texttt{tests/Unit/} del progetto.

Il \glslink{programmatore}{programmatore} è tenuto a seguire questa procedura prima di aprire una \textit{\glslink{pull-request}{Pull Request}}:
\begin{enumerate}
    \item Scrivere i \glslink{test-di-unita}{test di unità} che coprono la logica del codice prodotto.
    \item Verificare che tutti i test passino in locale con il comando \texttt{./vendor/bin/pest}.
    \item Calcolare la \textit{code coverage} con \texttt{./vendor/bin/pest -{}-coverage} (richiede il driver \textbf{Xdebug} attivo nel container PHP-FPM). La copertura minima richiesta per ogni unità è dell'\textbf{80\%}, in conformità con la metrica \textbf{MPC-CC}.
...
L'esecuzione dei \glslink{test-di-regressione}{test di regressione} è finalizzata ad accertare che interventi correttivi o evolutivi sul codice non abbiano introdotto nuovi difetti o problemi nelle funzionalità già esistenti.

Non richiedono la scrittura di test ad hoc: sono garantiti dall'esecuzione automatica dell'intera suite \glslink{pest}{Pest} PHP (unità + integrazione) nella pipeline di CI ad ogni \textit{push} o apertura di \textit{\glslink{pull-request}{Pull Request}}. Un test precedentemente verde che diventa rosso segnala una regressione e blocca il merge fino alla risoluzione.
...
I \glslink{test-di-integrazione}{test di integrazione} sono scritti con \textbf{\glslink{pest}{Pest} PHP} e risiedono in \texttt{tests/Feature/}. A differenza dei \glslink{test-di-unita}{test di unità}, questi test eseguono richieste HTTP reali contro l'applicazione \glslink{laravel}{Laravel} utilizzando il database \glslink{postgresql}{PostgreSQL} di test (definito in \texttt{.env.testing}), verificando l'interazione tra controller, service layer e database. Anche per questi test vale il requisito di copertura minima dell'\textbf{80\%} e l'obbligo di esecuzione prima di aprire una \textit{\glslink{pull-request}{Pull Request}}.

\paragraph{Test di Sistema}
I \glslink{test-di-sistema}{test di sistema} hanno l'obiettivo di accertare che il comportamento globale del prodotto rispetti integralmente i requisiti software stabiliti durante l'\glslink{analisi-dei-requisiti-adr}{analisi dei requisiti}.

Vengono eseguiti sull'applicazione deployata nell'ambiente Docker Compose completo (Nginx, PHP-FPM, \glslink{postgresql}{PostgreSQL}, \glslink{redis}{Redis}, \glslink{minio}{MinIO}), simulando scenari end-to-end che attraversano tutti i livelli dello stack. Ogni \glslink{test-di-sistema}{test di sistema} è tracciato a un \glslink{requisito-funzionale}{requisito funzionale} dell'\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti} tramite il codice \glslink{caso-d-uso-uc}{UC} corrispondente. I risultati e lo stato di copertura dei requisiti sono riportati nel \textit{\glslink{piano-di-qualifica}{Piano di Qualifica}}.
...
Mentre la verifica analizza la correttezza tecnica dell'esecuzione, la validazione si sposta sul piano dell'efficacia, garantendo che le funzionalità sviluppate siano effettivamente idonee a risolvere le problematiche operative individuate e siano allineate con le aspettative del \glslink{committente-proponente}{committente}.

\paragraph{Processo di Validazione}
Il gruppo \textit{aLittleByte} ha provveduto a raccogliere, analizzare e formalizzare tutte le necessità espresse dall'azienda \glslink{committente-proponente}{proponente} \textit{Eggon} all'interno del documento di \textit{\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti}}. Questa operazione di classificazione è fondamentale per garantire un tracciamento rigoroso delle specifiche e per predisporre i futuri \glslink{test-di-accettazione}{test di accettazione}, i quali avranno il compito di accertare che il prodotto software sviluppato rispecchi le richieste del \glslink{committente-proponente}{committente}.

Gli esiti derivanti da queste procedure di validazione saranno riportati e discussi in modo esaustivo nel \textit{\glslink{piano-di-qualifica}{Piano di Qualifica}}, all'interno della specifica sezione dedicata ai \glslink{test-di-accettazione}{Test di Accettazione}.

Si precisa che, in concomitanza con la pubblicazione dei documenti per l'\textit{\glslink{rtb-requirements-and-technology-baseline}{RTB}}, il progetto si trova in una fase antecedente alla codifica e al collaudo del software. Per tale ragione, lo stato di avanzamento dei test riporterà la dicitura temporanea ''Non Implementato''. I dati e i risultati effettivi dei test verranno integrati in questa sezione solamente in vista della consegna della \textit{\glslink{pb-product-baseline}{PB}}.

\paragraph{Procedura Operativa}
La validazione del prodotto avviene attraverso i seguenti passaggi:

\begin{enumerate}
    \item Raccolta dei risultati delle attività di verifica.
    \item Controllo del soddisfacimento dei requisiti.
    \item Verifica del raggiungimento degli obiettivi di qualità.
    \item Esecuzione dei \glslink{test-di-sistema}{test di sistema} previsti.
    \item Analisi delle eventuali non conformità.
    \item Approvazione del prodotto.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Interventi correttivi:} Prontezza e reattività nell'intervenire qualora le metriche di controllo evidenzino deviazioni significative rispetto al \glslink{piano-di-progetto}{Piano di Progetto}.
...
Come già citato in precedenza, il gruppo ha deciso di adottare un approccio iterativo, suddividendo il ciclo di vita del progetto in \glslink{sprint}{sprint} della durata fissa di due settimane. In concomitanza con l'avvio di ogni nuovo \glslink{sprint}{sprint}, il team provvede alla rotazione e all'assegnazione dei ruoli che ciascun membro dovrà ricoprire per l'intera durata dello \glslink{sprint}{sprint}.

L'organizzazione di ogni singola iterazione è scandita da una riunione, organizzata nel seguente modo:

\begin{itemize}
    \item \textbf{\glslink{pianificazione}{Pianificazione}:} Il team seleziona gli obiettivi operativi da raggiungere, scompone il lavoro in \textit{task} gestibili e li assegna ai vari componenti, cercando di bilanciare equamente i carichi di lavoro.
    \item \textbf{\glslink{sprint}{Sprint} Review:} Si analizza lo \glslink{sprint}{sprint} appena concluso per la verifica concreta dei risultati prodotti. Costituisce l'occasione per valutare il lavoro completato, discutere le criticità tecniche emerse e misurare l'avanzamento effettivo rispetto al piano generale. In questa sede, il team deve obbligatoriamente confrontare le ore preventivate con le ore effettivamente consuntivate: gli scostamenti principali (sottostime, attività impreviste, dipendenze esterne) devono essere motivati e registrati, al fine di aggiornare sistematicamente le previsioni di costo a finire (EAC).
    \item \textbf{\glslink{sprint}{Sprint} Retrospective:} Un momento di auto-valutazione finale, essenziale per il miglioramento continuo del gruppo. Il team si confronta in modo trasparente sulle dinamiche relazionali e metodologiche, individuando i punti di forza da mantenere e le inefficienze organizzative da correggere in vista del ciclo successivo. In questa fase è obbligatorio revisionare e aggiornare il Registro dei Rischi: per ogni criticità emersa o \glslink{rischio}{rischio} inatteso manifestatosi durante lo \glslink{sprint}{sprint}, il gruppo deve documentare formalmente nel registro la causa scatenante, l'impatto effettivo sul progetto e l'azione di \glslink{mitigazione-dei-rischi}{mitigazione} prevista e assegnare. 
\end{itemize}

\paragraph{Procedura Operativa}
La gestione degli \glslink{sprint}{Sprint} avviene secondo il seguente processo:

\begin{enumerate}
    \item Definizione degli obiettivi dello \glslink{sprint}{Sprint}.
    \item Individuazione delle attività da svolgere.
    \item Assegnazione dei ruoli e delle responsabilità.
    \item Stima dell'effort richiesto.
    \item Inserimento delle attività nello \glslink{sprint}{Sprint} Backlog.
    \item Monitoraggio dell'avanzamento.
    \item Esecuzione della \glslink{sprint}{Sprint} Review.
    \item Esecuzione della \glslink{sprint}{Sprint} Retrospective.
\end{enumerate}

\paragraph{Gestione del Flusso di Lavoro tramite Kanban}
Per garantire un controllo visivo e immediato sullo stato di avanzamento delle singole \glslink{issue}{Issue} associate al progetto, il gruppo gestisce i task tramite la board \glslink{github}{GitHub} Projects ''aLittleByte - \glslink{dashboard}{Dashboard}''.
...
\begin{itemize}
    ...
    \item \textbf{In Progress:} Attività su cui il team sta lavorando attivamente (le \glslink{issue}{issue} sono formalmente assegnate ai membri tramite il campo \textit{Assignee}).
    \item \textbf{In Review:} Task completati nello sviluppo o nella stesura e attualmente in attesa di \textit{Peer Review} tramite l'approvazione della relativa \textit{\glslink{pull-request}{Pull Request}}.
    \item \textbf{Done:} Lavoro verificato, approvato e unito definitivamente al \glslink{branch}{branch} principale.
\end{itemize}

\paragraph{Procedura Operativa}
Il flusso di lavoro tramite Kanban viene gestito come segue:

\begin{enumerate}
    \item Creazione della \glslink{issue}{Issue} associata all'attività.
    \item Inserimento della \glslink{issue}{Issue} nella colonna "To Do".
    \item Spostamento nella colonna "In Progress" all'avvio dei lavori.
    \item Aggiornamento dello stato durante l'esecuzione.
    \item Spostamento nella colonna "Review" al termine delle attività.
    \item Verifica del lavoro svolto.
    \item Spostamento nella colonna "Done" a seguito dell'approvazione.
\end{enumerate}

\subsubsection{Ruoli}

\paragraph{Responsabile}
Il membro che assume il ruolo di \glslink{responsabile}{Responsabile} agisce come vero e proprio \textit{Project Manager} del gruppo. Ha la responsabilità di coordinare il lavoro del team, pianificare le iterazioni di sviluppo, ottimizzare l'uso delle risorse e fungere da referente principale per le comunicazioni ufficiali con l'azienda \glslink{committente-proponente}{proponente} e il corpo docente.

Dal punto di vista strettamente operativo, il \glslink{responsabile}{Responsabile} si fa carico di distribuire e assegnare i compiti (tramite le \textit{\glslink{issue}{Issue}}) ai vari componenti del gruppo. Inoltre, è sua competenza diretta la stesura e il costante aggiornamento dei seguenti documenti:

\begin{itemize}
    \item \textbf{Verbali (Interni ed Esterni)}
    \item \textbf{\glslink{diario-di-bordo}{Diario di Bordo}}
    \item \textbf{\glslink{piano-di-progetto}{Piano di Progetto}}
    \item \textbf{\glslink{piano-di-qualifica}{Piano di Qualifica}}
\end{itemize}

\paragraph{Amministratore}
L'\glslink{amministratore}{Amministratore} rappresenta la figura di supporto metodologico e tecnologico del team. Il suo scopo principale è definire, ottimizzare e mantenere sempre operativi i processi e gli strumenti di lavoro, assicurando che l'intero gruppo possa procedere con le attività in modo fluido e senza impedimenti di natura tecnica o organizzativa. 

Oltre a monitorare le scadenze amministrative e a fornire supporto costante ai colleghi, l'\glslink{amministratore}{Amministratore} si fa carico delle seguenti responsabilità operative:
...
\begin{itemize}
    ...
    \item \textbf{\glslink{norme-di-progetto}{Norme di Progetto}:} Curare la stesura, la revisione e il continuo aggiornamento del presente documento (\textit{\glslink{norme-di-progetto}{Norme di Progetto}}), assicurandosi che le regole interne e il \textit{\glslink{way-of-working-wow}{Way of Working}} siano sempre allineati alle reali necessità del gruppo.
\end{itemize}

\paragraph{Progettista}

Il \glslink{progettista}{Progettista} è la mente tecnica dietro alla struttura del prodotto software. Il suo compito principale è ideare l'architettura del sistema, assicurandosi che sia in grado di soddisfare tutti i requisiti (sia funzionali che di qualità) concordati con il \glslink{committente-proponente}{proponente}. Lavora a stretto contatto con il resto del gruppo per trasformare le richieste astratte in soluzioni pratiche, efficienti e facili da mantenere nel tempo.
...
L'\glslink{analista}{Analista} rappresenta il punto di incontro tra le esigenze dell'azienda \glslink{committente-proponente}{proponente} e il lavoro tecnico del team di sviluppo. Il suo compito principale è comprendere a fondo il problema, raccogliere le richieste e trasformarle in regole chiare, complete e ben definite, evitando qualsiasi ambiguità. In pratica, lavora per assicurarsi che tutti sappiano esattamente \textit{cosa} deve essere costruito, prima ancora di decidere \textit{come} farlo.

Nello specifico, chi ricopre questo ruolo si fa carico di:

\begin{itemize}
    \item \textbf{Stesura dell'\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti}:} Redigere e mantenere costantemente aggiornato il documento che cataloga tutti i casi d'uso e le specifiche del progetto.
    
    \item \textbf{Gestione dei dubbi e delle modifiche:} Interfacciarsi direttamente con il \glslink{committente-proponente}{proponente} per risolvere eventuali incertezze, chiedere chiarimenti o concordare variazioni ai requisiti iniziali.
...
Il \glslink{programmatore}{Programmatore} è la figura operativa che materialmente "dà vita" al prodotto software. Il suo compito principale è tradurre l'architettura e le specifiche pensate dal \glslink{progettista}{Progettista} in codice reale e funzionante. Lavora con l'obiettivo di produrre un software non solo corretto, ma anche pulito, efficiente e facile da mantenere o aggiornare in futuro.
...
Il \glslink{verificatore}{Verificatore} è il "garante della qualità" all'interno del team. Il suo compito è assicurarsi che ogni elemento prodotto, dal singolo documento di testo fino al codice finale, rispetti fedelmente i requisiti stabiliti e gli standard prefissati.

Nello specifico, chi ricopre questo ruolo si fa carico di:

\begin{itemize}
    \item \textbf{Revisione documentale:} Leggere e controllare i documenti redatti dagli altri membri del team (tramite le \textit{\glslink{pull-request}{Pull Request}}), assicurandosi che siano corretti, completi e privi di ambiguità.
...
Il gruppo \textit{aLittleByte} adotta un sistema di rendicontazione oraria basato su un \textbf{foglio di calcolo condiviso su Google Fogli}, accessibile a tutti i membri del team.

\paragraph{Strumento}
Le ore vengono registrate all'interno di un foglio Google Fogli condiviso, organizzato per membro e per ruolo ricoperto. Per ciascun task svolto, il membro indica le ore previste e le ore effettivamente impiegate, consentendo il calcolo automatico degli scostamenti e del saldo ore rimanente.

\paragraph{Frequenza di aggiornamento}
Ogni membro del gruppo aggiorna il proprio registro orario \textbf{entro la fine di ogni settimana}. Il \glslink{responsabile}{Responsabile} di turno effettua una revisione complessiva del bilancio orario al \textbf{termine di ogni \glslink{sprint}{Sprint}}, verificando che i dati siano completi e coerenti con le attività svolte.

\paragraph{Catalogazione delle ore}
Le ore sono catalogate secondo due dimensioni:
\begin{itemize}
    \item \textbf{Per ruolo:} \glslink{responsabile}{Responsabile}, \glslink{amministratore}{Amministratore}, \glslink{analista}{Analista}, \glslink{progettista}{Progettista}, \glslink{programmatore}{Programmatore}, \glslink{verificatore}{Verificatore}.
    \item \textbf{Per membro:} ciascun componente del gruppo registra separatamente le ore svolte in ogni ruolo, permettendo di tenere traccia del monte ore residuo individuale per ciascuna figura professionale.
\end{itemize}

\paragraph{Monitoraggio}
Il \glslink{responsabile}{Responsabile} ha il compito di controllare, a fine \glslink{sprint}{Sprint}, che nessun membro abbia esaurito prematuramente le ore assegnate a un determinato ruolo. In caso di criticità, provvede a ridistribuire i task nelle iterazioni successive.

\subsubsection{Tracciamento delle Azioni}

Al termine di ogni riunione, il membro designato redige il verbale seguendo la struttura definita nella sezione~\ref{sec:documentazione}. Ogni compito emerso viene immediatamente trasformato in una \textit{\glslink{issue}{Issue}} su \glslink{github}{GitHub} e assegnato al membro o ai membri incaricati. Qualora il componente assegnato riscontri difficoltà, può utilizzare la sezione commenti della \textit{\glslink{issue}{Issue}} per chiedere chiarimenti al resto del team.

\paragraph{Procedura Operativa}
Il tracciamento delle azioni viene effettuato mediante i seguenti passaggi:

\begin{enumerate}
    \item Registrazione dell'attività nel sistema di tracciamento.
    \item Assegnazione dell'attività a un membro del gruppo.
    \item Aggiornamento periodico dello stato.
    \item Verifica del completamento dell'attività.
    \item Chiusura dell'azione registrata.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Discussioni \glslink{github}{GitHub}:} Viene utilizzato per discussioni all'interno delle \textit{\glslink{issue}{Issue}} e delle \textit{\glslink{pull-request}{Pull Request}}, per garantire il tracciamento delle decisioni. Sono utili in quanto permettono di comunicare solo con i membri del gruppo relativi alla discussione. 
\end{itemize}

\paragraph{Comunicazioni Esterne}

Per interagire con l'azienda \glslink{committente-proponente}{proponente} e con il corpo docente, il gruppo ha definito dei canali di comunicazione specifici, differenziati in base al livello di formalità e all'urgenza della richiesta:
...
\begin{itemize}
    ...
    \item \textbf{Telegram:} Impiegato per scambi di messaggi rapidi, chiarimenti immediati e per il coordinamento logistico direttamente con il referente dell'azienda \glslink{committente-proponente}{proponente}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Luoghi e piattaforme:} Per comodità organizzativa e per ottimizzare i tempi, le riunioni si svolgono prevalentemente online utilizzando i canali vocali della piattaforma \textbf{\glslink{discord}{Discord}}.
    
    \item \textbf{Verbalizzazione:} A garanzia della massima trasparenza operativa, al termine di ogni incontro viene redatto un \glslink{verbale-interno}{Verbale Interno}.
\end{itemize}

\paragraph{Riunioni Esterne}

I colloqui con l'azienda \glslink{committente-proponente}{proponente} sono essenziali per garantire il continuo allineamento sugli obiettivi e raccogliere \textit{feedback} sul lavoro svolto.
Tutte le sedute si tengono in via telematica sfruttando la piattaforma \textbf{Google Meet}. A conclusione di ogni colloquio, il team si occupa di redigere un \glslink{verbale-esterno}{Verbale Esterno} formale che riepiloga puntualmente le tematiche affrontate, le decisioni prese e le future azioni concordate. Tale documento viene infine inoltrato al referente aziendale per un controllo incrociato e, previa conferma di aderenza con quanto discusso, sottoposto alla sua firma per approvazione definitiva.

\subsubsection{Processo di Miglioramento Continuo}
In conformità con i principi agili e gli standard di qualità adottati, il gruppo aLittleByte applica sistematicamente un ciclo di miglioramento continuo del proprio \textit{\glslink{way-of-working-wow}{Way of Working}}. Al termine di ogni iterazione o fase di progetto, il team analizza le misurazioni raccolte nel cruscotto di valutazione all'interno del \textbf{\glslink{piano-di-qualifica}{Piano di Qualifica}}. 

Qualora emergano metriche sotto la soglia di accettabilità o inefficienze operative, il gruppo elabora specifiche ``Iniziative di miglioramento'', documentandole nel \glslink{piano-di-qualifica}{Piano di Qualifica} stesso. Tali iniziative non rimangono semplici osservazioni, ma vengono obbligatoriamente recepite e formalizzate all'interno delle presenti \textbf{\glslink{norme-di-progetto}{Norme di Progetto}} sotto forma di nuove regole operative e procedurali per gli \glslink{sprint}{sprint} successivi. Questo meccanismo garantisce che il metodo di lavoro si evolva costantemente, riducendo i rischi, correggendo le anomalie e massimizzando l'efficienza complessiva del team.

\paragraph{Procedura Operativa}
Il processo di miglioramento continuo viene svolto secondo i seguenti passaggi:

\begin{enumerate}
    \item Raccolta dei dati relativi allo \glslink{sprint}{Sprint} concluso.
    \item Analisi dei risultati ottenuti.
    \item Individuazione delle criticità emerse.
    \item Definizione delle misure correttive.
    \item Aggiornamento dell'\glslink{analisi-dei-rischi}{analisi dei rischi}.
    \item Aggiornamento della \glslink{pianificazione}{pianificazione} futura.
    \item Verifica dell'efficacia delle azioni intraprese.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Descrizione:} Percentuale di rischi effettivamente manifestatisi che non erano stati identificati nel registro dei rischi del \glslink{piano-di-progetto}{Piano di Progetto}. Valori bassi indicano una \glslink{pianificazione}{pianificazione} del \glslink{rischio}{rischio} efficace e un registro dei rischi completo.
    \item \textbf{Formula:}
