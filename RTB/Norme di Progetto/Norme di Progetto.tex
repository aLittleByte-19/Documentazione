...
\begin{center}
    ...
    \begin{tabularx}{\textwidth}{|p{1.4cm}|p{1.8cm}|X|p{2.4cm}|p{2.3cm}|p{2.3cm}|}
        ...
        \hline
        0.14.0 & 20/05/2026 & Aggiunta sezione metriche & Diego Belluz & Eleonora Bellet & - \\
        
        \hline 
        0.13.0 & 18/05/2026 & Aggiunta sezione qualità del software & Diego Belluz & Eleonora Bellet & - \\
        \hline 
        0.12.0 & 8/05/2026 & Struttura dei diari di bordo & Eleonora Bellet & Diego Belluz & - \\
        \hline
        0.11.2 & 7/05/2026 & Corretti errori di sintassi & Eleonora Bellet & Diego Belluz & - \\
        \hline 
        0.11.1 & 22/04/2026 & Aggiornate e corrette alcune sezioni delle norme & Diego Belluz & Eleonora Bellet & - \\
        \hline 
...
\begin{itemize}
    \item \textbf{Copertina:} Riporta le informazioni identificative di base, ovvero il logo del team, la data della presentatione, il numero del gruppo e i nomi dei componenti.
...
\section{Metriche}
Questa sezione definisce in modo formale tutte le metriche adottate dal gruppo per quantificare la qualità del processo di sviluppo e del prodotto software finale. I valori di soglia (accettabili e ottimali) e i risultati effettivi delle misurazioni sono demandati e documentati all'interno del \textit{Piano di Qualifica}.

\subsection{Metriche per la qualità di processo}
Queste metriche (identificate dalla sigla MPC) misurano l'efficacia, l'efficienza e il controllo delle attività necessarie per sviluppare il prodotto.

\subsubsection{Processi di Fornitura e Sviluppo}
\begin{itemize}
    \item \textbf{MPC-EV - Earned Value}: Rappresenta il valore del lavoro effettivamente completato.
    \item \textbf{MPC-PV - Planned Value}: Rappresenta il valore pianificato del lavoro da completare entro ciascuno sprint.
    \item \textbf{MPC-AC - Actual Cost}: Rappresenta il costo effettivo sostenuto per il lavoro completato in ciascuno sprint.
    \item \textbf{MPC-CPI - Cost Performance Index}: Misura l'efficienza dei costi. Si calcola come: 
    $$CPI = \frac{EV_{cum}}{AC_{cum}}$$
    \item \textbf{MPC-SPI - Schedule Performance Index}: Misura l'efficienza del tempo. Si calcola come: 
    $$SPI = \frac{EV_{cum}}{PV_{cum}}$$
    \item \textbf{MPC-EAC - Estimated At Completion}: Stima il costo totale del progetto proiettando la performance corrente. Si calcola come: 
    $$EAC = \frac{BAC}{CPI}$$
    \item \textbf{MPC-ETC - Estimate To Complete}: Rappresenta la stima dei costi rimanenti al completamento. Si calcola come: 
    $$ETC = EAC - AC_{cum}$$ 
    \item \textbf{MPC-TEAC - Time Estimate At Completion}: Stima la durata totale del progetto a partire dalla performance di avanzamento. Si calcola come: 
    $$TEAC = \frac{\text{Durata pianificata}}{SPI}$$
    \item \textbf{MPC-RSI - Requirements Stability Index}: Calcolato confrontando il numero di requisiti aggiunti, modificati o eliminati rispetto alla versione iniziale. Formula: 
    $$RSI = \left(1 - \frac{N_{modificati} + N_{eliminati} + N_{aggiunti}}{N_{totali\_iniziali}}\right) \times 100\%$$
\end{itemize}

\subsubsection{Processi di Supporto}
\begin{itemize}
    \item \textbf{MPC-IG - Indice di Gulpease}: Misura la leggibilità dei documenti in lingua italiana. Formula: 
    $$G = 89 - \frac{10 \cdot L}{P} + \frac{300 \cdot F}{P}$$
    \item \textbf{MPC-CO - Correttezza ortografica}: Analisi automatica della correttezza ortografica per individuare errori di battitura o sintassi.
    \item \textbf{MPC-CC - Code Coverage}: Misura la percentuale di codice sorgente eseguita durante l'esecuzione dei test automatici.
    \item \textbf{MPC-TSR - Test Success Rate}: Percentuale di test automatici che hanno restituito un esito positivo in fase di esecuzione.
    \item \textbf{MPC-QMS - Quality Metrics Satisfied}: Misura la percentuale di metriche di processo che soddisfano la soglia accettabile. Formula: 
    $$QMS = \frac{\text{Metriche soddisfatte}}{\text{Metriche misurate}} \times 100\%$$ 
\end{itemize}

\subsubsection{Processi Organizzativi}
\begin{itemize}
    \item \textbf{MPC-TE - Time Efficiency}: Calcolato confrontando le ore produttive effettivamente rendicontate con il totale delle ore disponibili. Formula: 
    $$TE = \frac{\text{Ore produttive}}{\text{Ore totali disponibili}} \times 100\%$$ 
    \item \textbf{MPC-PRI - Percentuale Rischi Inattesi}: Confronta i rischi effettivamente manifestatisi con quelli identificati nel registro dei rischi. Formula: 
    $$PRI = \frac{N_{rischi\_inattesi}}{N_{rischi\_totali\_manifestati}} \times 100\%$$ 
    \item \textbf{MPC-LE - Labor Efficiency}: Confronta le ore dedicate ad attività a valore aggiunto con il totale delle ore lavorate. Formula: 
    $$LE = \frac{\text{Ore a valore aggiunto}}{\text{Ore totali lavorate}} \times 100\%$$ 
\end{itemize}

\subsection{Metriche per la qualità di prodotto}
Queste metriche (identificate dalla sigla MPD) misurano le proprietà interne ed esterne del software finale per garantire che il sistema rispetti i requisiti.

\begin{itemize}
    \item \textbf{Funzionalità}:
    \begin{itemize}
        \item \textbf{MPD01}: Requisiti obbligatori soddisfatti
        \item \textbf{MPD02}: Requisiti opzionali soddisfatti.
        \item \textbf{MPD03}: AI Acceptance Rate (rating $\ge 3/5$)
    \end{itemize}
    \item \textbf{Affidabilità}:
    \begin{itemize}
        \item \textbf{MPD04}: Branch Coverage
        \item \textbf{MPD05}: Defect Density (Densità dei difetti per KLOC).
    \end{itemize}
    \item \textbf{Efficienza}:
    \begin{itemize}
        \item \textbf{MPD06}: Latenza interfaccia utente.
        \item \textbf{MPD07}: Latenza generazione testo AI Assistant.
        \item \textbf{MPD08}: Latenza generazione immagine AI Assistant.
        \item \textbf{MPD09}: Latenza analisi documentale AI Co-Pilot.
    \end{itemize}
    \item \textbf{Usabilità}:
    \begin{itemize}
        \item \textbf{MPD10}: Profondità Massima di Navigazione (numero di livelli).
        \item \textbf{MPD11}: Learning Time (tempo di apprendimento per le funzioni principali).
\end{itemize}
\newpage

    \item \textbf{Mantenibilità}:
    \begin{itemize}
        \item \textbf{MPD12}: Code Smell.
        \item \textbf{MPD13}: Coefficient of Coupling.
        \item \textbf{MPD14}: Cyclomatic Complexity (complessità per singolo metodo).
    \end{itemize}
    \item \textbf{Portabilità}:
    \begin{itemize}
        \item \textbf{MPD15}: Compatibilità Cross-Browser.
    \end{itemize}
\end{itemize}
