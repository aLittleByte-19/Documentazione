...
Al fine di prevenire qualsiasi ambiguità linguistica o concettuale tra i membri del team e gli \glslink{stakeholder}{stakeholder}, è stato redatto un documento separato denominato \glslink{glossario}{Glossario}. Esso contiene la spiegazione puntuale di tutti i termini tecnici, di dominio (es. \glslink{llm-large-language-model}{LLM}, \glslink{ocr-riconoscimento-ottico-dei-caratteri}{OCR}, \glslink{dispaccio}{Dispaccio}, ecc.) e degli acronimi utilizzati all'interno della documentazione di progetto.
...
\begin{itemize}
    \item \textbf{\glslink{glossario}{Glossario}:} Documento interno del progetto contenente la definizione dei termini tecnici, di dominio e degli acronimi utilizzati nella documentazione.
    \begin{itemize}
        \item \url{https://github.com/aLittleByte-19/Documentazione/blob/main/RTB/Glossario/Glossario.pdf}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item UC-[Numero] identifica i casi d'uso relativi all'\textbf{\glslink{ai-assistant-generativo}{AI Assistant Generativo}} (es. \textit{UC-1}).
        \item UC-CO-[Numero] identifica i casi d'uso relativi all'\textbf{\glslink{intelligenza-artificiale-ai}{AI} Co-Pilot} (es. \textit{UC-CO-1}).
...
\begin{itemize}
    \item \textbf{\glslink{git}{Git}:} Software per il controllo di versione distribuito che permette di monitorare ogni variazione apportata \glslink{intelligenza-artificiale-ai}{ai} file sorgente e alla documentazione.
...
\begin{enumerate}
    ...
    \item L'esecuzione dello script proprietario \texttt{.\glslink{github}{github}/site-src/update\_index.py}, che genera dinamicamente la struttura statica del sito (HTML, CSS e asset associati).
...
\begin{itemize}
    \item \texttt{.\glslink{github}{github}}: Contiene i \glslink{workflow}{workflow} di automazione e gli script Python necessari al rilascio.
...
Le attività di verifica hanno lo scopo di garantire l'assenza di difetti e la totale aderenza di ogni componente prodotta (sia essa documentale o codice sorgente) rispetto alle specifiche e \glslink{intelligenza-artificiale-ai}{ai} vincoli stabiliti. Applicando i principi dell'Ingegneria del Software, questa pratica metodologica mira a soddisfare la seguente domanda: \textit{''Did I build the system right?''}.
...
\begin{enumerate}
    ...
    \item Calcolare la \textit{code coverage} con \texttt{./vendor/bin/\glslink{pest}{pest} -{}-coverage} (richiede il driver \textbf{Xdebug} attivo nel container PHP-FPM). La copertura minima richiesta per ogni unità è dell'\textbf{80\%}, in conformità con la metrica \textbf{MPC-CC}.
...
La validazione rappresenta il processo finale che conferma in modo definitivo che le caratteristiche del prodotto software siano pienamente conformi \glslink{intelligenza-artificiale-ai}{ai} bisogni dell'utilizzo finale e agli obiettivi d'uso previsti. L'obiettivo principale è rispondere al quesito: \textit{''Did I build the right system?''} 
...
Gli esiti derivanti da queste procedure di validazione saranno riportati e discussi in modo esaustivo nel \textit{\glslink{piano-di-qualifica}{Piano di Qualifica}}, all'interno della specifica sezione dedicata \glslink{intelligenza-artificiale-ai}{ai} \glslink{test-di-accettazione}{Test di Accettazione}.
...
\begin{itemize}
    ...
    \item \textbf{Prevenzione e gestione \glslink{mitigazione-dei-rischi}{dei rischi}:} Identificazione delle potenziali criticità tecnologiche o organizzative, unita all'adozione di protocolli strutturati per governare le modifiche e per prevenire i possibili imprevisti.
    ...
    \item \textbf{Organizzazione del team:} Assegnazione trasparente di ruoli e responsabilità \glslink{intelligenza-artificiale-ai}{ai} membri del gruppo, garantendo al contempo il mantenimento di canali di comunicazione efficaci.
\end{itemize}
...
\begin{itemize}
    \item \textbf{\glslink{pianificazione}{Pianificazione}:} Il team seleziona gli obiettivi operativi da raggiungere, scompone il lavoro in \textit{task} gestibili e li assegna \glslink{intelligenza-artificiale-ai}{ai} vari componenti, cercando di bilanciare equamente i carichi di lavoro.
...
\begin{itemize}
    ...
    \item \textbf{In Progress:} Attività su cui il team sta lavorando attivamente (le \glslink{issue}{issue} sono formalmente assegnate \glslink{intelligenza-artificiale-ai}{ai} membri tramite il campo \textit{Assignee}).
...
Dal punto di vista strettamente operativo, il \glslink{responsabile}{Responsabile} si fa carico di distribuire e assegnare i compiti (tramite le \textit{\glslink{issue}{Issue}}) \glslink{intelligenza-artificiale-ai}{ai} vari componenti del gruppo. Inoltre, è sua competenza diretta la stesura e il costante aggiornamento dei seguenti documenti:
...
Oltre a monitorare le scadenze amministrative e a fornire supporto costante \glslink{intelligenza-artificiale-ai}{ai} colleghi, l'\glslink{amministratore}{Amministratore} si fa carico delle seguenti responsabilità operative:
...
\begin{itemize}
    ...
    \item \textbf{Gestione dei dubbi e delle modifiche:} Interfacciarsi direttamente con il \glslink{committente-proponente}{proponente} per risolvere eventuali incertezze, chiedere chiarimenti o concordare variazioni \glslink{intelligenza-artificiale-ai}{ai} requisiti iniziali.
...
Al termine di ogni riunione, il membro designato redige il verbale seguendo la struttura definita nella sezione~\ref{sec:documentazione}. Ogni compito emerso viene immediatamente trasformato in una \textit{\glslink{issue}{Issue}} su \glslink{github}{GitHub} e assegnato al membro o \glslink{intelligenza-artificiale-ai}{ai} membri incaricati. Qualora il componente assegnato riscontri difficoltà, può utilizzare la sezione commenti della \textit{\glslink{issue}{Issue}} per chiedere chiarimenti al resto del team.
...
Lo standard ISO/IEC 12207:1997 viene adattato al contesto di un team accademico che sviluppa un \glslink{mvp-minimum-viable-product}{MVP} stand-alone con moduli \glslink{intelligenza-artificiale-ai}{AI}. Sono mantenuti i processi di \textbf{Fornitura} e \textbf{Sviluppo} (adattati con \glslink{agile}{Agile}/SCRUM), mentre Acquisizione, Operazione e Manutenzione sono esclusi. La documentazione è ridotta \glslink{intelligenza-artificiale-ai}{ai} soli artefatti richiesti dalla baseline; la gestione della configurazione è delegata a \glslink{github}{GitHub}. L'efficacia del tailoring è monitorata tramite la metrica \textbf{MPC-QMS}: un valore sotto soglia segnala la necessità di rivedere le scelte nell'iterazione successiva.
...
\begin{itemize}
    \item \textbf{Descrizione:} Percentuale di requisiti opzionali correttamente implementati e verificati rispetto al totale dei requisiti opzionali rilevati. Un requisito è considerato soddisfatto quando il relativo \glslink{test-di-sistema}{test di sistema} risulta superato.
    \item \textbf{Formula:}
...
\begin{itemize}
    \item \textbf{Descrizione:} Tempo di risposta del sistema a un'interazione dell'utente sull'interfaccia, misurato come intervallo tra l'invio della richiesta e la ricezione della risposta completa per le operazioni generali dell'applicazione (escluse le chiamate \glslink{intelligenza-artificiale-ai}{ai} moduli \glslink{intelligenza-artificiale-ai}{AI}).
    \item \textbf{Formula:}
