...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \hline
        \glslink{earned-value-mpc-ev}{MPC-EV} & \glslink{earned-value-mpc-ev}{Earned Value} & $\geq 0$ & $\leq$ EAC \\
        \glslink{planned-value-mpc-pv}{MPC-PV} & \glslink{planned-value-mpc-pv}{Planned Value} & $\geq 0$ & $\leq$ BAC \\
        \glslink{actual-cost-mpc-ac}{MPC-AC} & \glslink{actual-cost-mpc-ac}{Actual Cost} & $\geq 0$ & $\leq$ EAC \\
        \glslink{cost-performance-index-mpc-cpi}{MPC-CPI} & \glslink{cost-performance-index-mpc-cpi}{Cost Performance Index} & $\geq 0,9$ & $1$ \\
        \glslink{schedule-performance-index-mpc-spi}{MPC-SPI} & \glslink{schedule-performance-index-mpc-spi}{Schedule Performance Index} & $\geq 0,9$ & $1$ \\
        \glslink{estimated-at-completion-mpc-eac}{MPC-EAC} & \glslink{estimated-at-completion-mpc-eac}{Estimated At Completion} & $\pm 5\%$ rispetto al BAC & BAC \\
        \glslink{estimate-to-complete-mpc-etc}{MPC-ETC} & \glslink{estimate-to-complete-mpc-etc}{Estimate To Complete} & $\geq 0$ & $\leq$ EAC \\
        \glslink{time-estimate-at-completion-mpc-teac}{MPC-TEAC} & \glslink{time-estimate-at-completion-mpc-teac}{Time Estimate At Completion} & $\geq 0$ & $\leq$ Durata pianificata \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \textbf{ID} & \textbf{Nome Metrica} & \textbf{Accettabile} & \textbf{Ottimo} \\
        \hline
        \glslink{requirements-stability-index-mpc-rsi}{MPC-RSI} & \glslink{requirements-stability-index-mpc-rsi}{Requirements Stability Index} & $\geq 70\%$ & $100\%$ \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \textbf{ID} & \textbf{Nome Metrica} & \textbf{Accettabile} & \textbf{Ottimo} \\
        \hline
        MPC-CC & \glslink{code-coverage}{Code Coverage} & $\geq 80\%$ & $100\%$ \\
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \textbf{ID} & \textbf{Nome Metrica} & \textbf{Accettabile} & \textbf{Ottimo} \\
        \hline
        \glslink{quality-metrics-satisfied-mpc-qms}{MPC-QMS} & \glslink{quality-metrics-satisfied-mpc-qms}{Quality Metrics Satisfied} & $\geq 80\%$ & $100\%$ \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \textbf{ID} & \textbf{Nome Metrica} & \textbf{Accettabile} & \textbf{Ottimo} \\
        \hline
        \glslink{time-efficiency-mpc-te}{MPC-TE} & \glslink{time-efficiency-mpc-te}{Time Efficiency} & $\geq 50\%$ & $100\%$ \\
        MPC-PRI & Percentuale Rischi Inattesi & $\leq 20\%$ & $0\%$ \\
        \glslink{labor-efficiency-mpc-le}{MPC-LE} & \glslink{labor-efficiency-mpc-le}{Labor Efficiency} & $\geq 70\%$ & $\geq 100\%$ \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \glslink{ai-acceptance-rate-mpd03}{MPD03} & \glslink{ai-acceptance-rate-mpd03}{AI Acceptance Rate} (\glslink{rating}{rating} $\geq 3/5$) & $\geq 60\%$ & $\geq 80\%$ \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \textbf{ID} & \textbf{Nome Metrica} & \textbf{Accettabile} & \textbf{Ottimo} \\
        \hline
        \glslink{branch-coverage-mpd04}{MPD04} & \glslink{branch-coverage-mpd04}{Branch Coverage} & $\geq 70\%$ & $\geq 85\%$ \\
        MPD05 & \glslink{defect-density}{Defect Density} & $\leq 3$ / \glslink{kloc}{KLOC} & $\leq 1$ / \glslink{kloc}{KLOC} \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        MPD07 & Latenza generazione testo \glslink{ai-intelligenza-artificiale}{AI} Assistant & $\leq 5$ sec & $\leq 3$ sec \\
        MPD08 & Latenza generazione immagine \glslink{ai-intelligenza-artificiale}{AI} Assistant & $\leq 10$ sec & $\leq 5$ sec \\
        MPD09 & Latenza analisi documentale \glslink{ai-co-pilot}{AI Co-Pilot} & $\leq 10$ sec & $\leq 5$ sec \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \glslink{learning-time-mpd11}{MPD11} & \glslink{learning-time-mpd11}{Learning Time} (funzioni principali) & $\leq 30$ min & $\leq 15$ min \\
        \hline
    \end{tabularx}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{2.4cm}|X|p{3cm}|p{3cm}|}
        ...
        \glslink{coefficient-of-coupling-mpd13}{MPD13} & \glslink{coefficient-of-coupling-mpd13}{Coefficient of Coupling} & $\leq 0{,}7$ & $\leq 0{,}4$ \\
        \glslink{cyclomatic-complexity-mpd14}{MPD14} & \glslink{cyclomatic-complexity-mpd14}{Cyclomatic Complexity} (per metodo) & $\leq 15$ & $\leq 10$ \\
        \hline
    \end{tabularx}
...
La copertura del codice (\glslink{code-coverage}{Code Coverage}) misura la percentuale di codice sorgente eseguita durante l'esecuzione dei test automatici. Tale metrica consente di valutare il grado di verifica del software ed è direttamente collegata alla metrica \textbf{MPC-CC}.
Il valore minimo accettabile è fissato all'80\%.

\subsection{Test unitari}
I test unitari verificano il corretto funzionamento delle singole unità software in isolamento. L'attenzione è rivolta alle componenti critiche del backend: logica di elaborazione dei \glslink{prompt}{prompt}, validazione degli input, gestione dei \glslink{metadati}{metadati} documentali e algoritmi di classificazione \glslink{ai-intelligenza-artificiale}{AI}. I test sono realizzati con il framework \glslink{pest}{Pest} e contribuiscono al miglioramento delle metriche \textbf{MPC-TSR} e \textbf{MPC-CC}.
...
\begin{longtable}{|p{2.2cm}|p{8.5cm}|p{3.8cm}|p{1.5cm}|}
    ...
    \hline
    TI-002 & Verifica che il sistema invii il \glslink{prompt}{prompt} e riceva la risposta generata dal servizio \glslink{ai-intelligenza-artificiale}{AI}. & Backend $\leftrightarrow$ \glslink{amazon-bedrock}{Amazon Bedrock} & NI \\
    \hline
    ...
    \hline
    TI-005 & Verifica che il sistema gestisca correttamente l'upload, lo storage e l'accesso \glslink{ai-intelligenza-artificiale}{ai} file. & Backend $\leftrightarrow$ \glslink{minio}{MinIO} (S3) & NI \\
    \hline
    TI-006 & Verifica che il sistema esegua correttamente i job in modalità asincrona tramite le code. & Backend $\leftrightarrow$ \glslink{redis-queue-rq}{Redis Queue} & NI \\
    \hline
\end{longtable}
...
\begin{longtable}{|p{2.2cm}|p{9.5cm}|p{2.3cm}|p{1.5cm}|}
    ...
    \hline
    TS-F-009 & Verifica che il sistema notifichi il \glslink{redattore}{Redattore} in caso di errore interno o \glslink{timeout}{timeout} durante la generazione iniziale del contenuto tramite \glslink{ai-intelligenza-artificiale}{AI}. & RF9-OB & NI \\
    \hline
    TS-F-010 & Verifica che il sistema notifichi il \glslink{redattore}{Redattore} in caso di errore interno o \glslink{timeout}{timeout} durante la rigenerazione del contenuto tramite \glslink{ai-intelligenza-artificiale}{AI}. & RF10-OB & NI \\
    \hline
    ...
    \hline
    TS-F-040 & Verifica che il sistema fornisca all'\glslink{analista-dati}{Analista dati} una \glslink{dashboard}{dashboard} dedicata con metriche di utilizzo del modulo \glslink{ai-intelligenza-artificiale}{AI} Assistant Generativo. & RF40-OB & NI \\
    \hline
    TS-F-041 & Verifica che il sistema visualizzi nella \glslink{dashboard}{dashboard} una panoramica dell'attività complessiva del modulo \glslink{ai-intelligenza-artificiale}{AI} Assistant. & RF41-OB & NI \\
    \hline
    TS-F-042 & Verifica che il sistema visualizzi nella \glslink{dashboard}{dashboard} le statistiche di utilizzo del modulo \glslink{ai-intelligenza-artificiale}{AI} Assistant. & RF42-OB & NI \\
    \hline
    ...
    \hline
    TS-F-046 & Verifica che il sistema permetta al \glslink{consulente-del-lavoro-cdl}{Consulente del lavoro} di caricare singoli documenti o lotti nel repository dedicato. & RF46-OB & NI \\
    \hline
    ...
    \hline
    TS-F-051 & Verifica che il sistema notifichi l'utente quando nessun documento corrisponde \glslink{ai-intelligenza-artificiale}{ai} criteri di visualizzazione dello storico. & RF51-OB & NI \\
    \hline
    ...
    \hline
    TS-F-055 & Verifica che il sistema permetta il filtraggio dello storico documenti per livello di \glslink{confidenza-confidence-score}{confidenza} dell'analisi \glslink{ai-intelligenza-artificiale}{AI}. & RF55-OB & NI \\
    \hline
    ...
    \hline
    TS-F-098 & Verifica che il sistema fornisca all'\glslink{analista-dati}{Analista dati} una \glslink{dashboard}{dashboard} dedicata con metriche di monitoraggio del modulo \glslink{ai-co-pilot}{AI Co-Pilot}. & RF98-OB & NI \\
    \hline
    ...
    \hline
    TS-Q-007 & Verifica tramite sessione di user testing che gli utenti siano in grado di apprendere le funzionalità principali del sistema in meno di 30 minuti. & \glslink{learning-time-mpd11}{MPD11} & NI \\
    \hline
    \multicolumn{4}{|l|}{\cellcolor{gray!12}\textbf{Requisiti di vincolo}} \\
    \hline
    TS-V-001 & Verifica che il codice sorgente sia versionato tramite \glslink{git}{Git} su un repository condiviso. & RVC1-OB & NI \\
    \hline
    ...
    \hline
    TS-V-011 & Verifica che i modelli di \glslink{ai-intelligenza-artificiale}{intelligenza artificiale} generativa possano essere erogati tramite \glslink{amazon-bedrock}{Amazon Bedrock}. & RVC11-OP & NI \\
    \hline
    ...
    \hline
    TS-V-016 & Verifica che l'utilizzo delle funzionalità \glslink{ai-intelligenza-artificiale}{AI} richieda credenziali AWS valide e accesso outbound ad \glslink{amazon-bedrock}{Amazon Bedrock}. & RVC16-OB & NI \\
    \hline
\end{longtable}

\subsection{Test di accettazione}
I test di accettazione verificano che il sistema soddisfi i requisiti utente relativi al \glslink{capitolato}{capitolato} d'appalto. Ogni test è associato al \glslink{caso-d-uso-uc}{caso d'uso} di riferimento.
...
\begin{longtable}{|p{2.2cm}|p{9.5cm}|p{2.3cm}|p{1.5cm}|}
    ...
    \hline
    \endlastfoot
    TA-001 & Verifica che il sistema permetta al \glslink{redattore}{Redattore} di configurare completamente i parametri del \glslink{prompt}{prompt} (tono, stile, lunghezza) e avviare la generazione \glslink{ai-intelligenza-artificiale}{AI} con successo. & UC-1 & NI \\
    \hline
    ...
    \hline
    TA-014 & Verifica che il sistema esponga all'\glslink{analista-dati}{Analista dati} la \glslink{dashboard}{dashboard} \glslink{ai-intelligenza-artificiale}{AI} Assistant con panoramica dell'attività complessiva e statistiche di utilizzo. & UC-6 & NI \\
    \hline
    TA-015 & Verifica che il sistema visualizzi nella \glslink{dashboard}{dashboard} \glslink{ai-intelligenza-artificiale}{AI} Assistant il \glslink{rating}{rating} medio delle bozze e permetta la consultazione dei feedback testuali. & UC-6 & NI \\
    \hline
    TA-016 & Verifica che il sistema permetta al \glslink{consulente-del-lavoro-cdl}{Consulente del lavoro} di caricare un singolo documento nel repository con classificazione automatica tramite \glslink{ai-analyst}{AI Analyst}. & UC-7, UC-8 & NI \\
    \hline
    ...
    \hline
    TA-023 & Verifica che il sistema permetta il filtraggio dello storico documenti per livello di \glslink{confidenza-confidence-score}{confidenza} dell'analisi \glslink{ai-intelligenza-artificiale}{AI}. & UC-13 & NI \\
    \hline
...
\begin{longtable}{|p{2.5cm}|p{2.2cm}|p{2.5cm}|p{2.2cm}|}
    ...
    \hline
    TS-Q-007 & \glslink{learning-time-mpd11}{MPD11} & & \\
    \hline
...
Le misurazioni si riferiscono \glslink{ai-intelligenza-artificiale}{ai} quattro \glslink{sprint}{sprint} del periodo \glslink{rtb-requirements-and-technology-baseline}{RTB}:
...
Il \glslink{planned-value-mpc-pv}{Planned Value} (PV) rappresenta il valore pianificato del lavoro da completare entro ciascuno \glslink{sprint}{sprint}, estratto dai preventivi del Piano di Progetto. L'\glslink{earned-value-mpc-ev}{Earned Value} (EV) rappresenta il valore del lavoro effettivamente completato. Nella fase \glslink{rtb-requirements-and-technology-baseline}{RTB} si assume che gli obiettivi di ogni \glslink{sprint}{sprint} siano stati completamente raggiunti (come risulta dalle retrospettive nel Piano di Progetto), pertanto $\text{EV} = \text{PV}$ per tutti gli \glslink{sprint}{sprint}.
...
\begin{tikzpicture}
\begin{axis}[
    title={Andamento \glslink{earned-value-mpc-ev}{MPC-EV} e \glslink{planned-value-mpc-pv}{MPC-PV} Cumulativi},
...
\noindent\textbf{Valutazione:} Entrambi i valori sono $\geq 0$ per tutti gli \glslink{sprint}{sprint} (\emph{soglia accettabile} raggiunta). Il PV cumulativo è $\leq \text{BAC} = \EUR{12.575,00}$ (\emph{soglia ottimale per \glslink{planned-value-mpc-pv}{MPC-PV}} raggiunta); l'EV cumulativo è $\leq \text{EAC}$ (\emph{soglia ottimale per \glslink{earned-value-mpc-ev}{MPC-EV}} raggiunta).

% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{MPC-AC — Actual Cost}

L'\glslink{actual-cost-mpc-ac}{Actual Cost} (AC) rappresenta il costo effettivo sostenuto per il lavoro completato in ciascuno \glslink{sprint}{sprint}, ricavato dai consuntivi del Piano di Progetto.
...
\begin{tikzpicture}
\begin{axis}[
    title={Andamento \glslink{actual-cost-mpc-ac}{MPC-AC} Cumulativo},
    xlabel={\glslink{sprint}{Sprint}},
...
Il \glslink{cost-performance-index-mpc-cpi}{Cost Performance Index} misura l'efficienza dei costi:
\[
  \text{CPI} = \frac{\text{EV}_\text{cum}}{\text{AC}_\text{cum}}
\]
Lo \glslink{schedule-performance-index-mpc-spi}{Schedule Performance Index} misura l'efficienza del tempo:
\[
...
L'\glslink{estimated-at-completion-mpc-eac}{Estimated At Completion} stima il costo totale del progetto proiettando la performance corrente:
\[
  \text{EAC} = \frac{\text{BAC}}{\text{CPI}}
\]
L'\glslink{estimate-to-complete-mpc-etc}{Estimate To Complete} rappresenta la stima dei costi rimanenti al completamento:
\[
...
\noindent\textbf{Valutazione:} L'EAC stimato supera il BAC di \EUR{12.575,00} in tutti gli \glslink{sprint}{sprint} e si attesta a \EUR{13.906,00} a S4, con uno scostamento del $+10{,}6\%$ rispetto al BAC — superiore alla soglia accettabile di $\pm 5\%$ (\emph{soglia non raggiunta} per \glslink{estimated-at-completion-mpc-eac}{MPC-EAC}). L'ETC è positivo per tutti gli \glslink{sprint}{sprint} (\emph{soglia accettabile} raggiunta per \glslink{estimate-to-complete-mpc-etc}{MPC-ETC}). Il trend crescente dell'EAC richiede un monitoraggio rigoroso dei costi nelle fasi successive.

% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{MPC-TEAC — Time Estimate At Completion}

Il \glslink{time-estimate-at-completion-mpc-teac}{Time Estimate At Completion} stima la durata totale del progetto a partire dalla performance di avanzamento. Calcolato come $\text{TEAC} = \text{Durata pianificata} / \text{SPI}$: poiché $\text{SPI} = 1{,}000$ per tutti gli \glslink{sprint}{sprint}, il progetto procede esattamente in linea con il calendario.
...
\begin{tikzpicture}
\begin{axis}[
    title={Andamento \glslink{time-estimate-at-completion-mpc-teac}{Time Estimate At Completion}},
    xlabel={\glslink{sprint}{Sprint}},
    ylabel={Settimane},
    symbolic x coords={S1, S2, S3, S4},
    xtick=data,
...
\begin{tikzpicture}
\begin{axis}[
    title={Andamento \glslink{requirements-stability-index-mpc-rsi}{Requirements Stability Index}},
    xlabel={\glslink{sprint}{Sprint}},
    ylabel={RSI (\%)},
    symbolic x coords={S1, S2, S3, S4},
    xtick=data,
...
Il \glslink{quality-metrics-satisfied-mpc-qms}{Quality Metrics Satisfied} misura la percentuale di metriche di processo che soddisfano la soglia accettabile nel periodo di riferimento:
\[
  \text{QMS} = \frac{\text{Metriche soddisfatte}}{\text{Metriche misurate}} \times 100\%
\]

La tabella seguente riassume l'esito di tutte le metriche misurabili nella fase \glslink{rtb-requirements-and-technology-baseline}{RTB}. Con l'\glslink{inclusione}{inclusione} delle metriche \glslink{requirements-stability-index-mpc-rsi}{MPC-RSI} e MPC-PRI, le metriche misurate passano a 12.
...
\begin{tabular}{|l|p{3.5cm}|p{3cm}|c|}
    ...
    \hline
    \glslink{earned-value-mpc-ev}{MPC-EV}  & $\geq 0$ tutti gli \glslink{sprint}{sprint} & $\geq 0$ & \textcolor{green!60!black}{Soddisfatta} \\
    \glslink{planned-value-mpc-pv}{MPC-PV}  & $\geq 0$ tutti gli \glslink{sprint}{sprint} & $\geq 0$ & \textcolor{green!60!black}{Soddisfatta} \\
    \glslink{actual-cost-mpc-ac}{MPC-AC}  & $\geq 0$ tutti gli \glslink{sprint}{sprint} & $\geq 0$ & \textcolor{green!60!black}{Soddisfatta} \\
    \glslink{cost-performance-index-mpc-cpi}{MPC-CPI} & 0,904 (S4)               & $\geq 0{,}9$ & \textcolor{green!60!black}{Soddisfatta} \\
    \glslink{schedule-performance-index-mpc-spi}{MPC-SPI} & 1,000 (tutti gli \glslink{sprint}{sprint}) & $\geq 0{,}9$ & \textcolor{green!60!black}{Soddisfatta} \\
    \glslink{estimated-at-completion-mpc-eac}{MPC-EAC} & $+10{,}6\%$ dal BAC (S4) & $\pm 5\%$ dal BAC & \textcolor{red!70!black}{Non soddisfatta} \\
    \glslink{estimate-to-complete-mpc-etc}{MPC-ETC} & $\geq 0$ tutti gli \glslink{sprint}{sprint} & $\geq 0$ & \textcolor{green!60!black}{Soddisfatta} \\
    \glslink{time-estimate-at-completion-mpc-teac}{MPC-TEAC} & 8 settimane (SPI = 1,000) & $\geq 0$ & \textcolor{green!60!black}{Soddisfatta} \\
    ...
    \glslink{requirements-stability-index-mpc-rsi}{MPC-RSI} & 96,9\% (S4)              & $\geq 70\%$ & \textcolor{green!60!black}{Soddisfatta} \\
...
\begin{itemize}
    \item \textbf{\glslink{estimated-at-completion-mpc-eac}{MPC-EAC}:} il costo stimato a completamento supera il BAC del 10,6\%. Nelle fasi successive è necessario un controllo più rigoroso dei costi, con revisione periodica delle stime e delle allocazioni orarie.
    \item \textbf{MPC-PRI:} il verificarsi di imprevisti bloccanti (50\% rispetto \glslink{ai-intelligenza-artificiale}{ai} rischi emersi) suggerisce che il registro dei rischi necessiti di un ampliamento e aggiornamento in vista dello sviluppo operativo \glslink{pb-product-baseline}{PB}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|c|}
        ...
        \hline
        S2 & Stallo per attesa file di esempio dal \glslink{committente-o-proponente}{proponente}. & Previsto (RR1 / RO5) \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.1cm}|X|X|}
        ...
        \hline
        \glslink{pianificazione}{Pianificazione} & \glslink{estimated-at-completion-mpc-eac}{MPC-EAC} non soddisfatta, con EAC superiore al BAC del 10,6\%. & Raffinare le stime e controllare con maggiore frequenza le previsioni a finire. \\
        \hline
        ...
        \hline
        Tracciamento operativo & \glslink{time-efficiency-mpc-te}{MPC-TE} e \glslink{labor-efficiency-mpc-le}{MPC-LE} non misurabili in \glslink{rtb-requirements-and-technology-baseline}{RTB} per assenza di tracciamento granulare. & Distinguere le ore per tipologia di attività, ruolo, \glslink{sprint}{sprint} e \glslink{issue}{issue} di riferimento. \\
        \hline
...
Lo scostamento rilevato tramite \glslink{estimated-at-completion-mpc-eac}{MPC-EAC} evidenzia che alcune attività sono state stimate con un livello di dettaglio non sufficiente rispetto all'impegno effettivo richiesto. La criticità riguarda in particolare le attività tecniche e documentali, che nella fase \glslink{rtb-requirements-and-technology-baseline}{RTB} hanno richiesto più tempo rispetto a quanto preventivato.
...
\paragraph*{Criticità rilevata:} \glslink{estimated-at-completion-mpc-eac}{MPC-EAC} non soddisfatta, con EAC superiore al BAC del 10,6\%.
...
\paragraph*{Metriche monitorate:} \glslink{estimated-at-completion-mpc-eac}{MPC-EAC}, con supporto delle metriche \glslink{actual-cost-mpc-ac}{MPC-AC}, \glslink{cost-performance-index-mpc-cpi}{MPC-CPI} e \glslink{estimate-to-complete-mpc-etc}{MPC-ETC}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.2cm}|X|X|}
        ...
        \hline
        Dipendenze esterne & File, servizi o informazioni provenienti dal \glslink{committente-o-proponente}{proponente} possono introdurre ritardi non controllabili dal gruppo. & Associare alle dipendenze esterne un impatto previsto e una possibile attività alternativa in caso di blocco. \\
        \hline
    \end{tabularx}
...
\paragraph*{Azione prevista:} Aggiornamento del registro rischi durante le retrospettive, indicando causa, impatto, mitigazione e \glslink{responsabile}{responsabile} per ciascun rischio significativo.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.1cm}|X|X|}
        ...
        \hline
        Attività produttive & Sviluppo, documentazione, configurazione e attività direttamente collegate agli obiettivi dello \glslink{sprint}{sprint}. & Permette di stimare le ore a valore aggiunto necessarie per \glslink{time-efficiency-mpc-te}{MPC-TE} e \glslink{labor-efficiency-mpc-le}{MPC-LE}. \\
        \hline
...
\paragraph*{Criticità rilevata:} \glslink{time-efficiency-mpc-te}{MPC-TE} e \glslink{labor-efficiency-mpc-le}{MPC-LE} non misurabili in \glslink{rtb-requirements-and-technology-baseline}{RTB} per mancanza di tracciamento granulare.
...
\paragraph*{Metriche monitorate:} \glslink{time-efficiency-mpc-te}{MPC-TE} e \glslink{labor-efficiency-mpc-le}{MPC-LE}, con supporto dei dati consuntivi utilizzati anche per il controllo di \glslink{estimated-at-completion-mpc-eac}{MPC-EAC}.

\subsection{Miglioramento della verifica e del testing}

Durante la fase \glslink{rtb-requirements-and-technology-baseline}{RTB} molti test risultano ancora NI o non pienamente misurabili, poiché la PoC ha avuto finalità dimostrativa e ha consentito di verificare la fattibilità dei flussi principali. La baseline stabile di prodotto verrà sviluppata e consolidata nella fase \glslink{pb-product-baseline}{PB}, prendendo come riferimento lo stack effettivamente adottato: Laravel con viste Blade, CSS e JavaScript custom. Le tecnologie valutate inizialmente ma non adottate nella realizzazione \glslink{rtb-requirements-and-technology-baseline}{RTB} non saranno considerate nel consolidamento previsto per la \glslink{pb-product-baseline}{PB}.
...
In \glslink{pb-product-baseline}{PB} il gruppo intende collegare il tracciamento dei test \glslink{ai-intelligenza-artificiale}{ai} quality gate automatici, così che gli esiti non restino informazioni isolate. Il consolidamento dovrà riguardare sia i test che verificano il comportamento applicativo, sia i controlli di integrazione sui servizi previsti dalla baseline di riferimento.
...
\noindent\textbf{Sintesi conclusiva:} Le iniziative descritte hanno lo scopo di trasformare le criticità emerse durante la fase \glslink{rtb-requirements-and-technology-baseline}{RTB} in azioni operative per la fase \glslink{pb-product-baseline}{PB}. In particolare, il gruppo intende migliorare la precisione della \glslink{pianificazione}{pianificazione}, rendere più sistematica la gestione dei rischi, aumentare la qualità della documentazione e consolidare il processo di verifica del codice. Tali interventi saranno monitorati attraverso le metriche già definite nel presente documento, con particolare attenzione a \glslink{estimated-at-completion-mpc-eac}{MPC-EAC}, MPC-PRI, MPC-IG, \glslink{time-efficiency-mpc-te}{MPC-TE}, \glslink{labor-efficiency-mpc-le}{MPC-LE}, MPC-CC e MPC-TSR.
