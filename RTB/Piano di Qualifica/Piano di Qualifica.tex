...
Il Requirements Stability Index valuta la stabilità dei requisiti nel tempo. La baseline dei requisiti è stata consolidata al termine dello Sprint 3 con un totale di 131 requisiti (109 funzionali, 6 di qualità, 16 di vincolo). Durante lo Sprint 4 sono state apportate correzioni ad alcuni requisiti esistenti.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|c|c|}
        \hline
        \rowcolor{gray!35}
        \textbf{Sprint} & \textbf{Baseline} & \textbf{Aggiunti} & \textbf{Modificati} & \textbf{Eliminati} & \textbf{RSI (\%)} & \textbf{Esito} \\
        \hline
...
\noindent\textbf{Valutazione:} La misurazione inizia formalmente dallo Sprint 3, momento di consolidamento della baseline. Nello Sprint 4 sono state effettuate 4 correzioni formali. Il valore si attesta al 96,9\%, mantenendosi ampiamente sopra la soglia accettabile del 70\% (\emph{soglia accettabile} raggiunta).
...
Le metriche MPC-CC (Code Coverage) e MPC-TSR (Test Success Rate) non sono ancora considerate pienamente applicabili nella fase RTB, poiché il codice realizzato in RTB ha natura dimostrativa e non coincide con una baseline stabile di prodotto. La misurazione sistematica di tali metriche è prevista a partire dalla fase PB (Product Baseline), quando verrà sviluppata e consolidata la baseline di riferimento e i test automatici potranno essere utilizzati come criterio continuativo di verifica.
...
La tabella seguente riassume l'esito di tutte le metriche misurabili nella fase RTB. Con l'inclusione delle metriche MPC-RSI e MPC-PRI, le metriche misurate passano a 12.
...
\begin{tabular}{|l|p{3.5cm}|p{3cm}|c|}
    ...
    MPC-PRI & 50\% (fase RTB)          & $\leq 20\%$ & \textcolor{red!70!black}{Non soddisfatta} \\
    \hline
    \rowcolor{gray!25}
...
\noindent\textbf{Valutazione:} Il QMS complessivo al termine della fase RTB è pari al 75\%, al di sotto della \emph{soglia accettabile} ($\geq 80\%$). Le tre metriche non soddisfatte richiedono interventi correttivi:
\begin{itemize}
    ...
    \item \textbf{MPC-PRI:} il verificarsi di imprevisti bloccanti (50\% rispetto ai rischi emersi) suggerisce che il registro dei rischi necessiti di un ampliamento e aggiornamento in vista dello sviluppo operativo PB.
    \item \textbf{MPC-IG:} i documenti tecnici presentano indici di leggibilità al di sotto della soglia di 60. Si raccomanda una revisione stilistica nelle prossime versioni, semplificando la struttura delle frasi.
\end{itemize}
...
Durante la fase RTB, il tracciamento orario si è concentrato esclusivamente sullo scostamento tra le ore previste e le ore effettivamente impiegate per il completamento dei task. Non è stata effettuata una misurazione granulare per distinguere le ore produttive da quelle di \textit{overhead}. Pertanto, questa metrica risulta \textbf{N/A} per gli sprint S1-S4.

\noindent\textbf{Azioni di miglioramento:} Il team introdurrà un sistema di \textit{time-tracking} granulare a partire dalla fase di Product Baseline (PB).

% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{MPC-PRI — Percentuale Rischi Inattesi}

La metrica valuta la capacità del team di prevedere e mitigare i rischi. I dati sono stati estratti dalle retrospettive dei quattro sprint di progetto.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|c|}
        \hline
        \rowcolor{gray!35}
        \textbf{Sprint} & \textbf{Problema Emerso} & \textbf{Classificazione} \\
        \hline
        S1 & Difficoltà nell'apprendimento di nuovi tool grafici per il mock-up. & Previsto (RT1) \\
        \hline
        S2 & Stallo per attesa file di esempio dal proponente. & Previsto (RR1 / RO5) \\
        \hline
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|}
        \hline
        \rowcolor{gray!35}
        \textbf{Sprint} & \textbf{Rischi Totali} & \textbf{Rischi Inattesi} & \textbf{PRI (\%)} & \textbf{Esito} \\
        \hline
        Fase RTB & 4 & 2 & 50\% & \textcolor{red!70!black}{Non soddisfatta} \\
        \hline
    \end{tabular}
    \caption{Calcolo MPC-PRI per la fase RTB}
\end{table}

\noindent\textbf{Valutazione:} Durante la fase RTB sono emersi 4 problemi principali. Due di questi non erano stati esplicitamente previsti. Il valore PRI si attesta al 50\%, superando la soglia accettabile del $\leq 20\%$ (\emph{soglia non raggiunta}). Sarà necessaria un'integrazione del registro dei rischi per la fase PB.

% ─────────────────────────────────────────────────────────────────────────────
\subsubsection{MPC-LE — Labor Efficiency}

Come per la metrica MPC-TE, l'assenza di un tracciamento puntuale delle ore spese in attività a valore aggiunto rispetto al tempo totale lavorato (incluso overhead) rende impossibile il calcolo per la fase RTB. Il valore è quindi \textbf{N/A}. Le misurazioni inizieranno formalmente con la fase PB.

\section{Iniziative di miglioramento}

Le valutazioni riportate nel cruscotto di qualità evidenziano alcuni ambiti sui quali il gruppo intende intervenire nella fase PB. Il valore finale di MPC-QMS è pari al 75\%, inferiore alla soglia accettabile, e segnala la necessità di trasformare le criticità rilevate in azioni operative verificabili. Gli interventi previsti non introducono nuove metriche, ma utilizzano quelle già definite nel presente documento come riferimento per controllare l'efficacia delle correzioni.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.1cm}|X|X|}
        \hline
        \rowcolor{gray!35}
        \textbf{Ambito} & \textbf{Evidenza rilevata in RTB} & \textbf{Obiettivo per la fase PB} \\
        \hline
        Pianificazione & MPC-EAC non soddisfatta, con EAC superiore al BAC del 10,6\%. & Raffinare le stime e controllare con maggiore frequenza le previsioni a finire. \\
        \hline
        Gestione dei rischi & MPC-PRI non soddisfatta, con 2 rischi inattesi su 4 e valore pari al 50\%. & Rendere più sistematica l'identificazione dei rischi tecnici, organizzativi ed esterni. \\
        \hline
        Qualità documentale & MPC-IG sotto soglia per la leggibilità dei documenti. & Ridurre la densità testuale e migliorare la distinzione tra definizioni, motivazioni e conseguenze. \\
        \hline
        Tracciamento operativo & MPC-TE e MPC-LE non misurabili in RTB per assenza di tracciamento granulare. & Distinguere le ore per tipologia di attività, ruolo, sprint e issue di riferimento. \\
        \hline
        Verifica e testing & MPC-CC e MPC-TSR non ancora misurabili su una baseline stabile di prodotto, da sviluppare in PB. & Introdurre quality gate e tracciamento continuativo dei test a partire dai componenti stabilizzati. \\
        \hline
    \end{tabularx}
    \caption{Sintesi delle criticità RTB e degli obiettivi di miglioramento per PB}
\end{table}

Le azioni descritte nelle sottosezioni seguenti sono state definite a partire dagli esiti delle metriche e dalle evidenze emerse durante gli sprint RTB. Per ciascun ambito sono indicati la criticità rilevata, l'intervento previsto e il criterio con cui il gruppo intende verificarne l'efficacia nella fase successiva.

\subsection{Miglioramento della pianificazione e del controllo dei costi}

Lo scostamento rilevato tramite MPC-EAC evidenzia che alcune attività sono state stimate con un livello di dettaglio non sufficiente rispetto all'impegno effettivo richiesto. La criticità riguarda in particolare le attività tecniche e documentali, che nella fase RTB hanno richiesto più tempo rispetto a quanto preventivato.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3cm}|X|X|}
        \hline
        \rowcolor{gray!35}
        \textbf{Aspetto} & \textbf{Criticità rilevata} & \textbf{Intervento previsto in PB} \\
        \hline
        ...
        \hline
        Controllo dello sprint & Lo scostamento è emerso soprattutto a consuntivo, quando l'impatto sul costo stimato a completamento era già visibile. & Confrontare a fine sprint ore preventivate e consuntivate, motivando gli scostamenti principali. \\
        \hline
...
Per evitare che il controllo dei costi resti limitato al solo consuntivo, il gruppo intende applicare un controllo regolare sulle stime ancora aperte. Tale controllo sarà svolto in corrispondenza della chiusura degli sprint e dovrà rendere esplicite le cause dello scostamento, distinguendo tra sottostima iniziale, attività non previste, dipendenze esterne e riallocazione delle risorse.
...
Il valore di MPC-PRI mostra una capacità di previsione dei rischi ancora incompleta. In RTB alcuni eventi sono stati gestiti nel corso dello sprint, ma non erano stati descritti in modo sufficientemente esplicito nel registro dei rischi. Per la fase PB il gruppo intende quindi rendere più puntuale la classificazione dei rischi e collegarla alle retrospettive.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.2cm}|X|X|}
        \hline
        \rowcolor{gray!35}
        \textbf{Rischio emerso} & \textbf{Lettura del problema} & \textbf{Trattamento previsto} \\
        \hline
        Attività tecniche sottostimate & Alcune attività implementative e di integrazione hanno richiesto approfondimenti non completamente previsti. & Esplicitare nel registro i rischi legati alla complessità tecnica e indicare una mitigazione tramite spike o task preliminari. \\
        \hline
        Attese per credenziali & L'accesso a risorse esterne può incidere direttamente sulla continuità delle attività tecniche. & Registrare le dipendenze da credenziali e ambienti esterni, indicando responsabile del sollecito e data limite interna. \\
        \hline
        Dipendenze esterne & File, servizi o informazioni provenienti dal proponente possono introdurre ritardi non controllabili dal gruppo. & Associare alle dipendenze esterne un impatto previsto e una possibile attività alternativa in caso di blocco. \\
        \hline
    \end{tabularx}
    \caption{Rischi da esplicitare nel registro per la fase PB}
\end{table}

L'aggiornamento del registro non sarà limitato all'aggiunta di nuove voci, ma dovrà migliorare la qualità informativa dei rischi già presenti. In particolare, durante le retrospettive il gruppo dovrà verificare se gli eventi emersi nello sprint erano stati previsti, se la mitigazione era applicabile e se l'impatto rilevato corrispondeva a quello stimato.

\paragraph*{Criticità rilevata:} MPC-PRI non soddisfatta, con presenza di rischi inattesi durante la fase RTB.

\paragraph*{Azione prevista:} Aggiornamento del registro rischi durante le retrospettive, indicando causa, impatto, mitigazione e responsabile per ciascun rischio significativo.

\paragraph*{Metriche monitorate:} MPC-PRI, con attenzione alla riduzione dei rischi inattesi e alla tempestività delle azioni di mitigazione.

\subsection{Miglioramento della qualità documentale}

L'indice di Gulpease sotto soglia non implica necessariamente la presenza di documenti errati, ma segnala una leggibilità inferiore a quella attesa. La criticità principale riguarda la densità dei periodi e l'uso frequente di terminologia tecnica, specialmente nelle sezioni in cui vengono motivate scelte architetturali o descritte metriche di controllo.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.2cm}|X|X|}
        ...
        \hline
        Terminologia tecnica & Alcuni concetti vengono spiegati più volte all'interno dei documenti, aumentando la complessità di lettura. & Usare il Glossario per alleggerire le spiegazioni ripetute e mantenere nei documenti solo il contesto necessario. \\
        \hline
...
\paragraph*{Criticità rilevata:} MPC-IG sotto soglia, con indice di Gulpease inferiore al valore accettabile previsto.

\paragraph*{Azione prevista:} Revisione mirata dei periodi più complessi, migliore separazione tra definizioni, motivazioni e conseguenze, uso più sistematico del Glossario.

\paragraph*{Metriche monitorate:} MPC-IG, verificando l'andamento dell'indice nelle revisioni successive dei documenti principali.

\subsection{Miglioramento del tracciamento operativo}

Le metriche MPC-TE e MPC-LE non sono state calcolabili in RTB perché il tracciamento orario non distingue ancora in modo sufficiente le diverse categorie di lavoro. Il gruppo ha registrato le ore necessarie al controllo dei costi, ma non ha separato con granularità adeguata attività produttive, verifica, riunioni, studio, overhead e blocchi.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.1cm}|X|X|}
        ...
        \hline
        Attività produttive & Sviluppo, documentazione, configurazione e attività direttamente collegate agli obiettivi dello sprint. & Permette di stimare le ore a valore aggiunto necessarie per MPC-TE e MPC-LE. \\
        \hline
        ...
        \hline
        Riunioni e coordinamento & Incontri interni, retrospettive, pianificazione e coordinamento operativo tra membri del gruppo. & Rende visibile il tempo di organizzazione necessario al completamento delle attività. \\
        \hline
...
La rendicontazione prevista per PB dovrà collegare ogni registrazione allo sprint, all'issue di riferimento, al ruolo coinvolto, alle ore preventivate e alle ore effettive. Questo consentirà di analizzare gli scostamenti non solo a livello complessivo, ma anche per tipologia di attività e per responsabilità operative.

\paragraph*{Criticità rilevata:} MPC-TE e MPC-LE non misurabili in RTB per mancanza di tracciamento granulare.

\paragraph*{Azione prevista:} Introduzione di una rendicontazione collegata a sprint, issue, ruolo, categoria dell'attività, ore preventivate e ore effettive.
...
Durante la fase RTB molti test risultano ancora NI o non pienamente misurabili, poiché la PoC ha avuto finalità dimostrativa e ha consentito di verificare la fattibilità dei flussi principali. La baseline stabile di prodotto verrà sviluppata e consolidata nella fase PB, prendendo come riferimento lo stack effettivamente adottato: Laravel con viste Blade, CSS e JavaScript custom. Le tecnologie valutate inizialmente ma non adottate nella realizzazione RTB non saranno considerate nel consolidamento previsto per la PB.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{3.3cm}|X|X|}
        ...
        \hline
        Elaborazione asincrona & Esecuzione tramite worker e coda Redis, con gestione degli stati intermedi. & Test sui passaggi di stato e sui casi di elaborazione fallita o ritentata. \\
        \hline
        Integrazione con MinIO & Gestione dei file e coerenza tra metadati applicativi e oggetti salvati. & Controlli su caricamento, lettura e assenza di riferimenti non validi. \\
        \hline
...
In PB il gruppo intende collegare il tracciamento dei test ai quality gate automatici, così che gli esiti non restino informazioni isolate. Il consolidamento dovrà riguardare sia i test che verificano il comportamento applicativo, sia i controlli di integrazione sui servizi previsti dalla baseline di riferimento.

\paragraph*{Criticità rilevata:} MPC-CC e MPC-TSR non ancora pienamente applicabili in RTB, poiché la baseline stabile di prodotto verrà sviluppata e consolidata in PB.
...
\noindent\textbf{Sintesi conclusiva:} Le iniziative descritte hanno lo scopo di trasformare le criticità emerse durante la fase RTB in azioni operative per la fase PB. In particolare, il gruppo intende migliorare la precisione della pianificazione, rendere più sistematica la gestione dei rischi, aumentare la qualità della documentazione e consolidare il processo di verifica del codice. Tali interventi saranno monitorati attraverso le metriche già definite nel presente documento, con particolare attenzione a MPC-EAC, MPC-PRI, MPC-IG, MPC-TE, MPC-LE, MPC-CC e MPC-TSR.
