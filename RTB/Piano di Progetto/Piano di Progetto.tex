...
\centering
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|p{2cm}|p{2.5cm}|p{2.5cm}|}
       ...
       \hline
        ...
        \hline
        0.3.3 & 2026-04-29 & Inizio stesura della sezione \textbf{\hyperref[sec:pianificazione]{§4}} & A.Oloni & -\\
        \hline
        0.3.2 & 2026-04-26 & Stesura della sezione \textbf{\hyperref[sec:modello_di_sviluppo]{§3}} & A.Oloni & - \\
        \hline
        0.3.1 & 2026-04-23 & Stesura delle sezioni \textbf{\hyperref[sec:introduzione]{§1}} e \textbf{\hyperref[sec:analisi_dei_rischi]{§2}} & A. Oloni & - \\
        \hline
        0.3.0 & 2026-04-21 & Aggiunta delle sezioni: \textbf{\hyperref[sec:scopo_del_documento]{§1.2}, \hyperref[sec:glossario]{§1.3}, \hyperref[sec:riferimenti]{§1.4}, \hyperref[sec:analisi_dei_rischi]{§2}, \hyperref[sec:modello_di_sviluppo]{§3}}, Preventivo ed aggiunta specifiche sulla sezione Introduzione e Pianificazione & A. Oloni & -\\
        \hline
...
\begin{itemize}
    ...
    \item \textbf{R}: Requisiti, legato a possibili modifiche, aggiunte o incomprensioni delle richieste del proponente
\end{itemize}
...
Nel Capitolato C5 \textbf{\textit{'Nexum'}} richiede varie tecnologie richieste dall'azienda proponente che escono dal nostro campo di competenza che abbiamo sviluppato durante il periodo di Università, andando ad imparare tecnologie di cui i membri non hanno confidenza e padronanza.
...
La comunicazione con l'azienda proponente è fondamentale per far si che il prodotto finale sia in linea con le richieste, se questa viene a mancare o le risposte non sono abbastanza tempestive, possono causare ritardi e fraintendimenti su questioni tecnologiche ed organizzative.
\vspace{0.5cm}
\hrule
\vspace{0.5em}
\textbf{RO5: Mancata o carenza di comunicazione col proponente}
\vspace{0.5em}
\hrule
\vspace{1cm}
...
\begin{itemize}
    ...
    \item \textbf{RTB \textit{(Requirements and Technology Baseline)}}: Questa fase avviene dopo l'aggiudicazione dell'appalto. Rappresenta il punto di controllo iniziale, fissato al termine della prima milestone di revisione. Essa certifica il consolidamento dei requisiti, la stabilità della documentazione normativa e la validazione delle scelte tecnologiche avvenuta tramite il \textit{Proof of Concept}.
    \item \textbf{PB: \textit{(Product Baseline)}}: In questo ultimo periodo, viene elaborato e sviluppato il prodotto software. È la baseline finale stabilita a conclusione della milestone di accettazione, volta a validare la conformità dell'MVP, l'avvenuta esecuzione di tutti i piani di test di sistema e la sottomissione finale del prodotto.
\end{itemize}
...
\begin{table}[H]
    ...
    \hline
    \textit{Requirements and Technology Baseline - RTB} & 2026/05/29\\
    \hline
...
con tutti i propri interessi, dubbi ed incertezze su ogni capitolato. Dopo aver trovato i capitolati di maggior interesse e fattibilità, il team ha fissato dei colloqui con le aziende, per esporre le domande emerse durante le riunioni interne e maggiori specifiche organizzative e tecnologiche.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|X|X|X|}
    \hline \rowcolor{Dandelion!50}
    \textbf{Sprint} & \textbf{Data Inizio} & \textbf{Data Fine}\\
    \hline
...
\begin{itemize}
    ...
    \item \textbf{Attività Pianificate (Sprint Backlog):} Una tabella di dettaglio che elenka i task assegnati ai vari ruoli, confrontando le ore preventivate con quelle effettivamente consumate a consuntivo. Ogni attività è accompagnata dal relativo stato di avanzamento, indicato con le diciture ``\textit{Completata}'' o ``\textit{In corso}''.
    \begin{quote}
        \textit{Nota bene:} È fondamentale precisare che tali stati si riferiscono \textbf{esclusivamente agli incrementi e agli aggiornamenti pianificati per lo specifico Sprint}, e non al completamento assoluto dell'intera attività o del documento a cui fanno riferimento. (A titolo di esempio, lo stato ``Completata'' per la voce ``Stesura Analisi dei Requisiti'' indica unicamente che il team ha portato a termine il carico di lavoro prefissato per quella specifica iterazione, non che il documento sia da considerarsi concluso o in versione definitiva).
    \end{quote}
    
    \item \textbf{Considerazioni (Sprint Review e Retrospective):} Una sezione conclusiva in cui vengono analizzati criticamente i risultati raggiunti, evidenziando eventuali problematiche riscontrate durante lo sviluppo e formalizzando le azioni correttive adottate per le iterazioni successive.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Problematiche Riscontrate:} Lieve rallentamento fisiologico per i ruoli di Analista e Progettista, dovuto alla curva di apprendimento nell'utilizzo dei tool grafici e alla fase di rodaggio nella stesura dei requisiti.
    \item \textbf{Azioni Correttive:} Maggiore accortezza nelle stime per le attività di Analisi e Progettazione durante i prossimi Sprint Planning; introduzione dell'obbligo di avviso tempestivo al Responsabile in caso di ritardi per consentire una rapida redistribuzione del carico.
\end{itemize}
\newpage
...
Durante il secondo Sprint, l'obiettivo principale del team è stato il consolidamento e l'allineamento della documentazione per prevenire future incomprensioni sui requisiti. Si è proceduto con l'avvio della stesura del \textit{Piano di Progetto} e con il continuo aggiornamento dell'\textit{Analisi dei Requisiti} (tramite l'estensione dei casi d'uso) e del \textit{Glossario}. Sul fronte progettuale, il mock-up è stato ultimato e presentato all'azienda proponente. A seguito del feedback ricevuto, sono state recepite le richieste per alcune modifiche minori; la revisione finale e la relativa approvazione del mock-up sono state riprogrammate per lo Sprint successivo.\\
...
\paragraph{Considerazioni (Sprint Review e Retrospective)}
\begin{itemize}
    \item \textbf{Analisi dei Risultati:} Ultimazione e presentazione del mock-up grafico all'azienda proponente; buon avanzamento nel consolidamento e nell'allineamento della documentazione per prevenire ambiguità.
    \item \textbf{Problematiche Riscontrate:} Emersione di richieste per modifiche minori sul mock-up da parte del proponente e incremento imprevisto del carico di lavoro per i ruoli di Analista (estensione casi d'uso) e Verificatore. Le attività di Analisi dei Requisiti hanno subito un momentaneo stallo a causa della necessità di acquisire dal proponente dei file di esempio (es. cedolini, comunicazioni). Tali documenti si sono rivelati un prerequisito indispensabile per poter estrarre e mappare correttamente i dati necessari alla descrizione dettagliata dei casi d'uso, generando un rallentamento rispetto alle stime iniziali.
    ...
    \item \textbf{Azioni Correttive:} Ottimizzazione della pianificazione interna per riassorbire lo sforzo orario extra e impegno del team nell'affinare le metriche di stima a partire dall'iterazione successiva.
\end{itemize}
\newpage
...
\paragraph{Considerazioni (Sprint Review e Retrospective)}
\begin{itemize}
    \item \textbf{Analisi dei Risultati:} Approvazione finale del mock-up da parte del proponente, forte progressione nella stesura dei casi d'uso (AdR) e avvio efficace della fase di autoformazione sui framework tecnologici Laravel e Filament.
    \item \textbf{Problematiche Riscontrate:} Sottostima iniziale dell'intensità del lavoro richiesto per la diagrammazione dei requisiti ed emersione di attività tecniche non preventivate (ruolo del Programmatore) necessarie alla rifinitura del mock-up.
    \item \textbf{Azioni Correttive:} Gestione flessibile e tempestiva del budget orario, attuando una riduzione mirata sulle ore del Progettista per supportare i ruoli che hanno portato via più tempo del necessario (Analista e Programmatore).
\end{itemize}
\newpage
...
\paragraph{Considerazioni (Sprint Review e Retrospective)}
\begin{itemize}
    \item \textbf{Analisi dei Risultati:} Chiusura definitiva dell'Analisi dei Requisiti e sviluppo completato del Proof of Concept (PoC) utilizzando le tecnologie designate; Completati Norme di progetto, piano di qualifica, piano di progetto e glossario in vista dell'RTB.
    \item \textbf{Problematiche Riscontrate:} Sul piano tecnico, lo sviluppo del PoC ha subito un momentaneo stallo a causa di una dipendenza esterna legata all'infrastruttura cloud di \textbf{AWS}. Il team si è trovato bloccato in attesa di poter richiedere e ottenere le credenziali e le risorse necessarie direttamente dall'azienda proponente. Questo periodo di attesa forzata ha costretto i ruoli tecnici (Progettista, Programmatore e Verificatore) a condensare l'intera fase di implementazione e collaudo in un arco di tempo molto più ristretto rispetto a quanto pianificato. Per recuperare il tempo latente e garantire la consegna del PoC entro i termini dello Sprint, il team ha dovuto svolgere sessioni di lavoro straordinarie e intensive, generando così lo scostamento complessivo di +7 ore.
    \item \textbf{Azioni Correttive:} Trattandosi dell'ultimo Sprint della fase RTB, il gruppo utilizzerà le criticità per il preventivo delle ore del PoC per stilare una pianificazione oraria post-RTB estremamente accurata, prevedendo \textit{buffer} di rischio più ampi per la fase di coding.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Obiettivo:} Rilasciare un \textit{Minimum Viable Product} (MVP) stabile, in grado di dimostrare l'interoperabilità di base tra le interfacce utente e i servizi di backend.
...
\begin{itemize}
    \item \textbf{Obiettivo:} Raggiungere la piena copertura funzionale del capitolato, ottimizzando i costi operativi legati alle chiamate AI e perfezionando le logiche di automazione.
    \item \textbf{Attività chiave:} Estensione delle funzionalità AI (raffinamento dei prompt e dei meccanismi di smistamento automatico), implementazione di sistemi di caching per minimizzare le richieste esterne, definizione granulare del controllo accessi basato sui ruoli (RBAC) e rifinitura grafica delle interfacce \textit{Nexum}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Obiettivo:} Certificare la conformità totale del prodotto rispetto ai requisiti qualitativi, prestazionali e di sicurezza concordati con la proponente.
    \item \textbf{Attività chiave:} Esecuzione intensiva dei \textit{System Test} e dei test di integrazione, sessioni di \textit{User Acceptance Test} (UAT) in coordinamento con Eggon, stesura e validazione della manualistica finale (Manuale Utente e Manuale Amministratore) e rilascio del codice in ambiente di produzione.
\end{itemize}
...
Al termine del primo Sprint, viene effettuato il calcolo del \textit{Preventivo a Finire} (EAC - \textit{Estimate at Completion}) per monitorare proattivamente l'andamento economico del progetto. Questa metrica proietta il costo totale finale sulla base degli scostamenti registrati nell'iterazione appena conclusa.
...
\begin{table}[H]
    ...
    \hline
    Responsabile & 56 & +0 & 56 & 1.680,00 \euro & 0,00 \euro \\
    \hline
    Amministratore & 52 & +0 & 52 & 1.020,00 \euro & 0,00 \euro\\
    \hline
    Analista & 59 & +1 & 60 & 1.500,00 \euro & -25,00 \euro\\
    \hline
    Progettista & 143 & +1 & 144 & 3.575,00 \euro & -25,00 \euro\\
    \hline
    Programmatore & 160 & +0 & 160 & 2.400,00 \euro & 0,00 \euro\\
    \hline
    Verificatore & 160 & -1 & 159 & 2.400,00 \euro & +15,00 \euro \\
    \hline
    \rowcolor{green!50!black!20!white}
...
Al termine del secondo Sprint, il \textit{Preventivo a Finire} (EAC) è stato ricalcolato tenendo conto degli scostamenti cumulativi registrati nelle prime due iterazioni di progetto. 
...
\begin{table}[H]
    ...
    Responsabile & 56 & +0 & 56 & 1.680,00 \euro & 0,00 \euro\\
    \hline
    Amministratore & 52 & -1 & 51 & 1.020,00 \euro & +20,00 \euro \\
    \hline
    Analista & 59 & +3 & 62 & 1.550,00 \euro & -75,00 \euro \\
    \hline
    Progettista & 143 & +1 & 144 & 3.600,00 \euro & -25,00 \euro\\
    \hline
    Programmatore & 160 & +0 & 160 & 2.400,00 \euro & 0,00 \euro \\
    \hline
    Verificatore & 160 & +1 & 161 & 2.415,00 \euro & -15,00 \euro\\
    \hline
    \rowcolor{green!50!black!20!white}
...
Al termine del terzo Sprint, il Responsabile ha proceduto al ricalcolo del \textit{Preventivo a Finire}. Questa analisi predittiva valuta l'impatto degli scostamenti cumulativi registrati fin dall'inizio del progetto rispetto al budget aggiornato in fase di avvio.
...
\begin{table}[H]
    ...
    Amministratore & 52 & -1 & 51 & 1.020,00 \euro & +20,00 \euro \\
    \hline
    Analista & 59 & +6 & 65 & 1.625,00 \euro & -150,00 \euro \\
    \hline
    Progettista & 143 & +0 & 143 & 3.575,00 \euro & 0,00 \euro \\
    \hline
    Programmatore & 160 & +2 & 162 & 2.430,00 \euro & -30,00 \euro\\
    \hline
    Verificatore & 160 & +1 & 161 & 2.415,00 \euro & -15,00 \euro \\
    \hline
    \rowcolor{green!50!black!20!white}
...
Al termine del quarto e ultimo Sprint della fase di pre-qualifica, viene rendicontato il \textit{Preventivo a Finire} (EAC) finale per la \textit{Requirements and Technology Baseline} (RTB).

La realizzazione e il collaudo del Proof of Concept (PoC) hanno richiesto un effort tecnico superiore alle stime (complessive +7 ore sui ruoli di Progettazione, Programmazione e Verifica). Questo scostamento, sommato a quelli delle iterazioni precedenti, porta la Varianza a Finire complessiva a -300,00 \euro. Trattandosi della chiusura di una macro-fase, il team ha assorbito tale varianza all'interno del margine di rischio concordato; la futura pianificazione della fase di \textit{Product Baseline} (PB) sarà calibrata su metriche più conservative per garantire l'assoluto rispetto del tetto di spesa.
...
\begin{table}[H]
    ...
    Analista & 59 & +6 & 65 & 1.625,00 \euro & -150,00 \euro\\
    \hline
    Progettista & 143 & +2 & 145 & 3.625,00 \euro & -50,00 \euro \\
    \hline
    Programmatore & 160 & +4 & 164 & 2.460,00 \euro & -60,00 \euro \\
    \hline
    Verificatore & 160 & +4 & 164 & 2.460,00 \euro & -60,00 \euro\\
    \hline
    \rowcolor{green!50!black!20!white}
