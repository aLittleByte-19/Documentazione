...
\begin{longtable}{|l|l|p{4.5cm}|p{2cm}|p{2.5cm}|p{2.5cm}|}
    ...
    \endfoot
    \endlastfoot
    2.2.0 & 19/07/2026 & Introduzione delle attività di miglioramento continuo negli Sprint. & Eleonora Bellet & Alex Oloni & - \\ \hline
    2.1.0 & 06/07/2026 & Aggiunto settimo sprint & Diego Belluz & Alex Oloni & - \\ \hline
...
\begin{itemize}
    \item Maggiore accuratezza nelle stime delle attività di Analisi e Progettazione durante gli \glslink{sprint}{Sprint} Planning.
    \item Obbligo di comunicazione tempestiva al \glslink{responsabile}{Responsabile} in caso di ritardi.
    \item Prediligere tecnologie già note nella fase iniziale del progetto
\end{itemize}

\paragraph{Aggiornamento analisi dei rischi}
\begin{itemize}
    \item RT1 si è manifestato come previsto durante la fase iniziale di apprendimento.
    \item Il \glslink{rischio}{rischio} è stato mitigato mediante l'utilizzo di tecnologie note e attività di studio mirate.
\end{itemize}

\paragraph{Miglioramento della pianificazione}
\begin{itemize}
    \item Allocare un margine temporale aggiuntivo per le attività di analisi e progettazione nei successivi \glslink{sprint}{sprint}.
\end{itemize}
    \newpage
...
\paragraph{Misure correttive}
\begin{itemize}
    \item Riassegnazione temporanea degli Analisti ad attività documentali durante l'attesa del materiale fornito dal \glslink{committente-proponente}{proponente}.
    \item Affinamento delle metriche di stima per gli \glslink{sprint}{sprint} successivi.
\end{itemize}

\paragraph{Aggiornamento analisi dei rischi}
\begin{itemize}
    \item RR1 si è manifestato attraverso l'estensione dei casi d'uso.
    \item Il \glslink{rischio}{rischio} è stato contenuto senza impatti sulle \glslink{milestone}{milestone} previste.
\end{itemize}

\paragraph{Miglioramento della pianificazione}
\begin{itemize}
    \item Inserire dipendenze esterne (documentazione del \glslink{committente-proponente}{proponente}) tra i fattori considerati nello \glslink{sprint}{Sprint} Planning.
\end{itemize}
\newpage
...
\paragraph{Misure correttive}
\begin{itemize}
    \item Migliorare la stima delle attività legate alla documentazione, in particolare per \glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti}.
    \item Considerare esplicitamente, in fase di \glslink{sprint}{Sprint} Planning, eventuali attività di rifinitura richieste dal \glslink{committente-proponente}{proponente}.
\end{itemize}

\paragraph{Miglioramento della pianificazione}
\begin{itemize}
    \item Allocare un numero maggiore di ore alle attività di analisi e verifica documentale negli \glslink{sprint}{sprint} successivi.
\end{itemize}
\newpage
...
\paragraph{Misure correttive}
\begin{itemize}
    \item Prevedere buffer temporali per attività dipendenti da soggetti esterni.
    \item Pianificare con maggiore anticipo le richieste di accesso alle infrastrutture.
\end{itemize}

\paragraph{Aggiornamento analisi dei rischi}
\begin{itemize}
    \item RT1 si è manifestato durante lo sviluppo del PoC
    \item Il \glslink{rischio}{rischio} è stato mitigato mediante attività di studio e consultazione della documentazione tecnica.
\end{itemize}

\paragraph{Miglioramento della pianificazione}
\begin{itemize}
    \item Incrementare i buffer di \glslink{rischio}{rischio} nelle future attività di sviluppo.
    \item Considerare esplicitamente le dipendenze infrastrutturali nella \glslink{pianificazione}{pianificazione} post-\glslink{rtb-requirements-and-technology-baseline}{RTB}.
\end{itemize}
...
\paragraph{Misure correttive}
\begin{itemize}
    \item Revisione approfondita dell'\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti} e del PoC.
    \item Recepimento sistematico delle osservazioni ricevute durante la revisione \glslink{rtb-requirements-and-technology-baseline}{RTB}.
\end{itemize}

\paragraph{Aggiornamento analisi dei rischi}
\begin{itemize}
    \item RR2 (errata interpretazione dei requisiti) ha mostrato una rilevanza superiore rispetto alle attese.
    \item È stato necessario rivalutare la qualità della documentazione prodotta.
\end{itemize}

\paragraph{Miglioramento della pianificazione}
\begin{itemize}
    \item Destinare uno \glslink{sprint}{sprint} dedicato alla correzione delle criticità evidenziate in sede di revisione.
    \item Incrementare le verifiche interne prima delle future \glslink{milestone}{milestone}.
\end{itemize}
\newpage
...
\paragraph{Misure correttive}
\begin{itemize}
    \item Mantenere un elevato livello di verifica incrociata della documentazione e del codice prodotto durante le future iterazioni.
    \item Evitare la concentrazione delle attività di chiusura negli ultimi giorni dello \glslink{sprint}{sprint}, al fine di garantire il rispetto degli standard qualitativi definiti dal gruppo.
\end{itemize}

\paragraph{Aggiornamento analisi dei rischi}
\begin{itemize}
    \item Il \glslink{rischio}{rischio} RR2 (Ambiguità o errata interpretazione dei requisiti) si è manifestato in maniera significativa nel periodo precedente ed è stato mitigato mediante una rilettura approfondita dei feedback ricevuti e una revisione completa dell'\glslink{analisi-dei-requisiti-adr}{Analisi dei Requisiti} e del PoC.
    \item Il \glslink{rischio}{rischio} RP2 (Lavoro svolto non conforme rispetto agli impegni dichiarati) è stato affrontato introducendo un processo di validazione più rigoroso per documentazione e codice.
    \item Le azioni intraprese hanno consentito di riallineare il progetto agli standard qualitativi richiesti per la \glslink{milestone}{milestone} \glslink{rtb-requirements-and-technology-baseline}{RTB}.
\end{itemize}

\paragraph{Miglioramento della pianificazione}
\begin{itemize}
    \item Mantenere una quota di ore dedicata alle attività di verifica anche nelle fasi avanzate del progetto.
    \item Pianificare le future iterazioni con particolare attenzione al rispetto degli standard qualitativi richiesti per il conseguimento della \glslink{pb-product-baseline}{Product Baseline} (\glslink{pb-product-baseline}{PB}).
\end{itemize}
\newpage
...
\paragraph{Misure correttive}
\begin{itemize}
    \item Richiedere a ciascun membro del gruppo la dichiarazione esplicita dei propri impegni.
    \item Definire chiaramente attività e scadenze anche in periodi caratterizzati da ridotto carico operativo.
\end{itemize}

\paragraph{Aggiornamento analisi dei rischi}
\begin{itemize}
    \item RO2 si è manifestato durante il periodo di attesa del riscontro esterno.
    \item Il \glslink{rischio}{rischio} è stato mitigato mediante una più chiara assegnazione delle responsabilità.
\end{itemize}

\paragraph{Miglioramento della pianificazione}
\begin{itemize}
    \item Mantenere una \glslink{pianificazione}{pianificazione} minima anche nei periodi di attesa.
    \item Predisporre attività secondarie da eseguire in caso di dipendenze esterne.
\end{itemize}
