...
\begin{itemize}
    \item Lo stack tecnologico include Redis per metadati e cache, e Minio per lo storage di documenti originali e suddivisi.
    \item È stata sviluppata un'interfaccia admin per la gestione delle credenziali e per simulare risposte AI, testando workflow di keytab.
    \item L'elaborazione dei PDF viene delegata a un worker tramite coda Redis, senza bloccare la richiesta, e i documenti vengono salvati su Minio.
    ...
    \item Sono emerse limitazioni nell'estrazione dati dai PDF, potenzialmente legate all'uso di Bedrock rispetto a un OCR più preciso, e si è discusso della necessità di identificare l'utente destinatario.
\end{itemize}

\subsection{Conclusioni e Richiesta del Bucket}
La dimostrazione ha avuto successo nel mostrare le funzionalità dell'applicazione e l'integrazione delle tecnologie. È stata confermata la disponibilità a fornire il bucket S3 richiesto per facilitare i test futuri. Ulteriori sviluppi potrebbero includere l'identificazione dell'utente e l'affinamento dei prompt per migliorare l'estrazione dei dati dai documenti.
