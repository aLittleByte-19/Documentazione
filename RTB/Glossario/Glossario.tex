...
Servizio interamente gestito da Amazon Web Services (AWS) che rende disponibili modelli di fondazione (FM) delle principali aziende di AI tramite API.

\subsection{Amministratore}
Ruolo responsabile della configurazione dell'ambiente standalone, della gestione dei privilegi degli account e della definizione delle policy globali.

\subsection{Analisi dei Requisiti (AdR)}
Documento che raccoglie e formalizza i requisiti funzionali, di qualità e di vincolo, inclusi i casi d'uso UML.

\subsection{Analisi dei Rischi}
Processo continuo e strutturato volto a identificare, classificare e valutare i potenziali eventi critici (tecnologici, organizzativi o personali) che potrebbero compromettere lo svolgimento del progetto.
...
\subsection{Code Coverage (MPC-CC)}
...
\subsection{Commit}
Operazione di salvataggio delle modifiche nel repository Git.
...
\subsection{Defect Density (MPD05)}
...
\subsection{Docker}
Piattaforma per la containerizzazione di applicazioni che permette di standardizzare l'ambiente di sviluppo e produzione.

\subsection{Docker Compose}
Strumento per definire ed eseguire applicazioni multi-container.
...
\subsection{Filament}
Framework PHP per la creazione rapida di interfacce amministrative basato su Laravel.
...
\subsection{GitHub}
Piattaforma web per l'hosting e la gestione collaborativa dei repository tramite Pull Request e Issue.

\subsection{GitHub Actions}
Servizio di automazione per l'esecuzione di workflow CI/CD.
...
\subsection{Laravel}
Framework PHP utilizzato per lo sviluppo del backend.
...
\subsection{MinIO}
Server di object storage open-source ad alte prestazioni, compatibile a livello di API con il servizio cloud Amazon S3.
...
Approccio metodologico e framework operativo adottato dal team per scandire le fasi del ciclo di vita del software.
...
Documento che definisce il Way of Working (WoW): ruoli, strumenti, procedure e standard.
...
\subsection{Penpot}
Strumento open source per la progettazione di mockup e prototipi.

\subsection{Pest}
Framework di testing moderno per il linguaggio di programmazione PHP, focalizzato sulla semplicità e sulla leggibilità dei test unitari.
...
Processo di calendarizzazione e allocazione temporale delle attività, delle pietre miliari (Milestone) e delle consegne ufficiali del progetto.

\subsection{Piano di Progetto}
Documento che contiene pianificazione, analisi dei rischi, preventivi e consuntivi.

\subsection{Piano di Qualifica}
Documento che definisce metriche, test e obiettivi di qualità.
...
Sistema di gestione di database relazionali (RDBMS) open-source a oggetti.
...
\subsection{Prompt}
L'istruzione o il testo che scrivi all'intelligenza artificiale per dirle cosa deve generare.
...
Applicazione web sviluppata per offrire un'esperienza utente simile a quella di un'applicazione nativa.
...
\section{R}
\subsection{Rating}
Sistema di valutazione (punteggio e commento) che l'utente può assegnare al contenuto generato dall'AI.

\subsection{RBAC (Role-Based Access Control)}
Un sistema di sicurezza che decide cosa può fare un utente in base al suo ruolo.

\subsection{Redattore}
Utente aziendale delegato alla creazione di comunicazioni interne tramite il modulo AI Assistant.

\subsection{Redis}
Sistema per code asincrone e caching.

\subsection{Redis Queue (RQ)}
Sistema di gestione delle code di messaggi basato su Redis.

\subsection{Requirements Stability Index (MPC-RSI)}
Indice che misura il grado di stabilità dei requisiti nel tempo.

\subsection{Responsabile}
Ruolo incaricato di approvare formalmente i contenuti e coordinare il team.

\subsection{Repository Documentale}
Un archivio digitale dove vengono caricati e organizzati tutti i documenti.

\subsection{Requisito (Obbligatorio, Desiderabile, Opzionale)}
Classificazione delle funzionalità del sistema in base alla loro priorità.

\subsection{Requisito Funzionale}
Caratteristica che descrive un'azione specifica che il software deve essere in grado di eseguire.

\subsection{Requisito Non Funzionale}
Vincolo qualitativo o prestazionale che definisce come il sistema deve operare.

\subsection{Resilienza}
Capacità del sistema di gestire errori e anomalie mantenendo il funzionamento.

\subsection{Retention Policy}
La regola che stabilisce per quanto tempo un documento deve essere conservato nel sistema.

\subsection{Riferimento Normativo}
Legge, standard ufficiale o documento interno a cui il team deve conformarsi.
...
Categoria di rischi legati a problematiche individuali dei membri del team.

\subsection{RT (Rischio Tecnologico)}
Categoria di rischi legati alla complessità, all'apprendimento o al malfunzionamento delle tecnologie.
...
Un ambiente di prova isolato dove i programmatori possono testare nuove funzioni.

\subsection{Schedule Performance Index (MPC-SPI)}
Indicatore dell'efficienza temporale di un progetto; rapporto tra Earned Value e Planned Value.
...
Il valore percentuale limite (fissato ad esempio all'80\%) di sicurezza richiesto all'AI.

\subsection{Sottocaso}
Scomposizione logica e strutturale di un caso d'uso complesso in porzioni più piccole.

\subsection{Split Automatico}
Funzionalità che permette di suddividere documenti multi-destinatario in singole "copie atomiche".

\subsection{Sprint}
Un periodo di tempo definito (nel caso di NEXUM è di 10 giorni lavorativi) in cui il team si concentra su un set specifico di compiti.

\subsection{Stakeholder}
Qualsiasi individuo, gruppo o organizzazione che ha un interesse diretto nel progetto.

\subsection{Standalone}
Caratteristica di un'applicazione che può funzionare in modo indipendente.

\subsection{Stile Editoriale}
Parametro di configurazione (es. formale, empatico, tecnico) che calibra il registro dell'AI Assistant.

\section{T}
\subsection{Tenant}
Un'istanza isolata e dedicata a un singolo cliente o organizzazione all'interno della piattaforma software.

\subsection{Test di Accettazione}
Test che validano il sistema rispetto ai requisiti utente e al capitolato.
...
Test end-to-end su tutto il sistema in ambiente simulato.

\subsection{Test di Unità}
Test volti a verificare il corretto funzionamento di singole parti (unità) di codice.

\subsection{Time Efficiency (MPC-TE)}
Rapporto percentuale tra le ore produttive effettivamente rendicontate e il totale delle ore nominalmente disponibili.

\subsection{Time Estimate At Completion (MPC-TEAC)}
Stima della durata temporale totale complessiva richiesta per completare il progetto.

\subsection{Timeout}
Interruzione forzata di un'operazione che si verifica quando un servizio non risponde entro un tempo massimo prestabilito.

\subsection{Token API}
Una sorta di "chiave digitale" segreta che permette a diversi programmi di parlare tra loro in modo sicuro.
...
Tipologia di utente autorizzato a utilizzare le funzionalità ordinarie del software, privo di privilegi di configurazione sistemistica.
...
Metodo di revisione informale e di lettura approfondita dei documenti o del codice sorgente.
